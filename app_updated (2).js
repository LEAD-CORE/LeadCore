...
(() => {
  ...
  // SLA scheduling: date + time (stored as "YYYY-MM-DDTHH:MM")
  const DEFAULT_DUE_TIME = "09:00";

  const isISODateOnly = (s) => /^\d{4}-\d{2}-\d{2}$/.test(safeTrim(s));

  const normalizeDueAt = (dueAt) => {
    const s = safeTrim(dueAt);
    if (!s) return "";
    if (isISODateOnly(s)) return `${s}T${DEFAULT_DUE_TIME}`;
    return s;
  };

  const splitDueAt = (dueAt) => {
    const s = normalizeDueAt(dueAt);
    if (!s) return { date: "", time: "" };
    if (s.includes("T")) {
      const [d, t0] = s.split("T");
      const t = (t0 || "").slice(0,5);
  ...
  const composeDueAt = (dateStr, timeStr) => {
    const d = safeTrim(dateStr);
    const t = safeTrim(timeStr) || DEFAULT_DUE_TIME;
    if (!d) return "";
    return `${d}T${t.slice(0,5)}`;
  };

  const fmtDueDateTimeHe = (dueAt) => {
    const d = new Date(normalizeDueAt(dueAt));
...
if (!out.agents.length) {
  ...
  out.agents = (base.agents || []).map((a, idx) => ({
    ...
    out.customers = (out.customers || []).map((c) => ({
      ...
      dueAt: normalizeDueAt(safeTrim(c?.dueAt)),
...
this.els.btnUserEditSave = $("#btnUserEditSave");
      ...
      const _updateSlaBadge = (customer) => {
        const due = (safeTrim(this.els.cfDueAt?.value) ? composeDueAt(safeTrim(this.els.cfDueAt?.value), safeTrim(this.els.cfDueTime?.value)) : normalizeDueAt(safeTrim(customer?.dueAt)));
      ...
      on(this.els.cfNextAction, "change", () => {
        try {
          ...
          const computedDate = computeDueAtFromAction(action, new Date());
          if (this.els.cfDueAt) this.els.cfDueAt.value = computedDate;
          if (this.els.cfDueTime && !safeTrim(this.els.cfDueTime.value)) this.els.cfDueTime.value = DEFAULT_DUE_TIME;
          _updateSlaBadge(c);
        } catch (_) {}
      });
      ...
      on(this.els.cfSlaSaveBtn, "click", async () => {
        try {
          ...
          const action = safeTrim(this.els.cfNextAction?.value) || "מעקב לקוח";
          const dueDate = safeTrim(this.els.cfDueAt?.value) || computeDueAtFromAction(action, new Date());
          const due = composeDueAt(dueDate, safeTrim(this.els.cfDueTime?.value));
...
// Search (live filter on customers view)
      ...
      try {
        ...
        if (this.els.cfDueAt) {
          const cur = safeTrim(c.dueAt) ? normalizeDueAt(safeTrim(c.dueAt)) : composeDueAt(computeDueAtFromAction(safeTrim(c.nextAction) || "מעקב לקוח", new Date()), DEFAULT_DUE_TIME);
          const parts = splitDueAt(cur);
          this.els.cfDueAt.value = parts.date || "";
          if (this.els.cfDueTime) this.els.cfDueTime.value = parts.time || DEFAULT_DUE_TIME;
          // keep normalized back on model (no save yet)
          c.dueAt = cur;
        }
        ...
        if (this.els.cfSlaBadge) {
          ...
          const label = this._slaLabel(bucket);
          this.els.cfSlaBadge.textContent = `תזמון: ${label}`;
    ...
    renderMyFlow(){
      ...
      const items = customers
        ...
        .map(c => {
          ...
          if (!dueAt) {
            dueAt = composeDueAt(computeDueAtFromAction(action, new Date()), DEFAULT_DUE_TIME);
            c.nextAction = action;
            c.dueAt = dueAt;
            c.updatedAt = nowISO();
            this._slaAutofillDirty = true;
      ...
      const fmtDue = (v) => fmtDueDateTimeHe(v);
            ...
            <div class="lcFlowCard__body">
              <div class="lcFlowCard__top">
                <div>
                  ...
                  <div class="lcFlowCard__meta">${escapeHtml(it.nextAction)} • יעד: ${escapeHtml(fmtDue(it.dueAt))}${(it.dueTs <= Date.now()) ? " • ⏰ עכשיו" : ""}</div>
                </div>
...
openPolicyActionModal(action, policyId, policyLabel) {
    ...
    renderDashboard() {
      ...
      // Due schedules ("pop") for active agent
      const active = this._getActiveAgentName();
      const nowTs = Date.now();
      const dueNow = (State.data.customers || [])
        .filter(c => safeTrim(c.assignedAgent) && safeTrim(c.assignedAgent) === active)
        .map(c => {
          const fullName = safeTrim(`${c.firstName || ""} ${c.lastName || ""}`) || "לקוח ללא שם";
          const due = this._parseDateSafe(normalizeDueAt(c.dueAt));
          return {
            id: c.id,
            name: fullName,
            task: safeTrim(c.nextAction) || "מעקב לקוח",
            dueAt: normalizeDueAt(c.dueAt),
            dueTs: due ? due.getTime() : Number.POSITIVE_INFINITY
          };
        })
        .filter(it => Number.isFinite(it.dueTs) && it.dueTs <= nowTs)
        .sort((a,b) => a.dueTs - b.dueTs);

      if (this.els.kpiUpdated) this.els.kpiUpdated.textContent = String(dueNow.length || 0);

      if (this.els.kpiDueList) {
        if (!dueNow.length) {
          this.els.kpiDueList.innerHTML = `<div class="kpiDueItem" style="cursor:default; opacity:.75">
            <div class="kpiDueLeft">
              <div class="kpiDueName">אין לקוחות לקפיצה כרגע</div>
              <div class="kpiDueMeta">כשתזמון יגיע — הוא יופיע כאן אוטומטית</div>
            </div>
            <div class="kpiDueTime">—</div>
          </div>`;
        } else {
          this.els.kpiDueList.innerHTML = dueNow.slice(0,4).map(it => `
            <button class="kpiDueItem" type="button" data-open-customer="${escapeHtml(it.id)}">
              <div class="kpiDueLeft">
                <div class="kpiDueName">${escapeHtml(it.name)}</div>
                <div class="kpiDueMeta">${escapeHtml(it.task)}</div>
              </div>
              <div class="kpiDueTime">${escapeHtml(fmtDueDateTimeHe(it.dueAt).split(" ").slice(-1)[0] || "")}</div>
            </button>
          `).join("");
        }
      }
