...
// ---------------------------
// Force Google Sheets (no Local mode)
// ---------------------------
const FORCE_GSHEETS = true;
const FORCE_GS_URL = "https://script.google.com/macros/s/AKfycbwHDJlsn3TiTrPXpIVdOc9b0sy7cIlkF9cQnKJ4__19vvr-OUjvwzpEuSFhSBEItjB9Iw/exec";
  ...
  // ---------------------------
  const Storage = {
    mode: "sheets", // "local" | "sheets"
    gsUrl: "",

    // מצב סנכרון אחרון (לחיווי אמיתי ולא רק "יש URL")
    last: { ok: null, at: null, error: "" },
    ...
    async saveSheets(state) {
      ...
      // אנחנו מנסים כמה פורמטים כדי להתאים לסקריפט (payload/state/raw)
      const baseUrl = new URL(this.gsUrl);

      const attempts = [
        // (1) action=put + {payload: state, state: state}
        {
          label: "put(payload+state)",
          url: (() => { const u=new URL(baseUrl); u.searchParams.set("action","put"); return u; })(),          body: JSON.stringify({ payload: state, state })
        },
        // (2) action=put + raw state
        {
          label: "put(raw)",
          url: (() => { const u=new URL(baseUrl); u.searchParams.set("action","put"); return u; })(),          body: JSON.stringify(state)
        },
        // (3) action=save (לפעמים כך נקרא בסקריפטים אחרים)
        {
          label: "save(payload+state)",
          url: (() => { const u=new URL(baseUrl); u.searchParams.set("action","save"); return u; })(),          body: JSON.stringify({ payload: state, state })
        }
      ];
      ...
      for (const a of attempts) {
        try {
          const res = await fetch(a.url.toString(), {
            method: "POST",
            body: a.body
          });
          ...
          if (json && json.ok === true) return { ok: true, at: json.at || nowISO(), via: a.label };

          // אם קיבלנו תשובה אבל היא לא ok
          if (json && json.error) lastErr = String(json.error);
          else lastErr = `שגיאת שמירה (${a.label})`;
        } catch (e) {
          lastErr = String(e?.message || e);
        }
      }

      return { ok: false, error: lastErr || "שגיאת put" };
    },

    async load() {
      let r;
      if (this.mode === "sheets") r = await this.loadSheets();
      else r = { ok: true, payload: this.loadLocal(), at: nowISO() };
      this.last.ok = !!r.ok;
      this.last.at = r.at || nowISO();
      this.last.error = r.ok ? "" : (r.error || "");
      return r;
    },

    async save(state) {
      let r;
      if (this.mode === "sheets") r = await this.saveSheets(state);
      else r = this.saveLocal(state);
      this.last.ok = !!r.ok;
      this.last.at = r.at || nowISO();
      this.last.error = r.ok ? "" : (r.error || "");
      return r;
...
on(this.els.btnSearch, "click", () => {
  ...
  this.renderCustomers();
      ...
      on(this.els.btnAddPolicyInline, "click", async () => {
        ...
        c.updatedAt = nowISO();
        this.renderPolicies();
        await App.save("נוסף ביטוח ללקוח");
        this.renderAll();
      });
...
on(this.els.gsUrl, "change", () => {
  if(FORCE_GSHEETS){
    // locked
    try{ this.els.gsUrl.value = FORCE_GS_URL; }catch(_){}
    return;
  }
...
on(this.els.btnResetLocal, "click", () => {
    ...
    renderSyncStatus() {
      ...
      // במערכת הזו אנחנו עובדים תמיד מול Google Sheets
      txt.textContent = "מצב: Google Sheets";

      if (dot) {
        dot.classList.remove("ok", "warn", "err");
        if (Storage.last.ok === true) dot.classList.add("ok");
        else if (Storage.last.ok === false) dot.classList.add("err");
        else dot.classList.add("warn"); // עדיין לא נעשה סנכרון/שמירה מאז עלייה
      }

      const at = Storage.last.at || State.data?.meta?.updatedAt;
      const when = at ? new Date(at).toLocaleString("he-IL") : null;

      if (Storage.last.ok === false && Storage.last.error) {
        last.textContent = `שגיאת סנכרון: ${Storage.last.error}`;
      } else {
        last.textContent = when ? ("סנכרון אחרון: " + when) : "טרם בוצע סנכרון";
      }
    }
  };
  ...
  const App = {
    async boot() {
  // Force Google Sheets always (no browser-stored settings)
  if (FORCE_GSHEETS) {
    Storage.mode = "sheets";
    Storage.gsUrl = FORCE_GS_URL;
  } else {
    // restore settings (legacy)
    Storage.gsUrl = localStorage.getItem("LEAD_CORE_GS_URL") || "";
    Storage.mode = localStorage.getItem("LEAD_CORE_MODE") || "local";
  }

  // reflect in UI (if settings view exists)
  try {
    if (UI.els.gsUrl) {
      UI.els.gsUrl.value = Storage.gsUrl;
      if (FORCE_GSHEETS) {
        UI.els.gsUrl.disabled = true;
        UI.els.gsUrl.setAttribute("readonly", "true");
      }
    }
    if (UI.els.modeLocal) {
      UI.els.modeLocal.classList.toggle("is-active", Storage.mode === "local");
      if (FORCE_GSHEETS) UI.els.modeLocal.disabled = true;
    }
    if (UI.els.modeSheets) {
      UI.els.modeSheets.classList.toggle("is-active", Storage.mode === "sheets");
      if (FORCE_GSHEETS) UI.els.modeSheets.disabled = true;
    }
  } catch (_) {}

  // Always load from Sheets when forced
  if (FORCE_GSHEETS) {
    const r = await Storage.loadSheets();
    if (!r.ok) {
      document.body.innerHTML = `
        <div style="max-width:760px;margin:70px auto;padding:18px;border:1px solid rgba(0,0,0,.12);border-radius:14px;font-family:Arial;line-height:1.6">
          <h2 style="margin:0 0 10px">שגיאת חיבור ל‑Google Sheets</h2>
          <div style="opacity:.9;margin-bottom:10px">
            המערכת מוגדרת לעבוד אך ורק עם Google Sheets, אבל החיבור נכשל.
          </div>
          <ul style="margin:0 0 10px;padding-inline-start:20px">
            <li>ודא שב‑Apps Script: Deploy → Web app</li>
            <li>Execute as: <b>Me</b></li>
            <li>Who has access: <b>Anyone</b> / <b>Anyone with the link</b></li>
            <li>ודא שה‑URL הוא של ה‑Deploy ומסתיים ב‑/exec</li>
          </ul>
          <div style="opacity:.75;font-size:13px">פרטים טכניים: ${escapeHtml(r.error || "unknown")}</div>
        </div>`;
      return;
    }
    State.set(r.payload);
...
async save(activityText) {
      ...
      // חיווי אמיתי: לשמור סטטוס סנכרון אחרון
      Storage.last.ok = !!r.ok;
      Storage.last.at = r.at || nowISO();
      Storage.last.error = r.ok ? "" : (r.error || "שמירה נכשלה");
      ...
      // אימות שמירה: קורא בחזרה מה-Sheets ומוודא שהרשומה האחרונה קיימת
      if (Storage.mode === "sheets") {
        try {
          const v = await Storage.loadSheets();
          if (v.ok && v.payload) {
            const newest = (State.data.customers && State.data.customers[0]) ? State.data.customers[0].id : "";
            if (newest) {
              const exists = (v.payload.customers || []).some(x => String(x.id) === String(newest));
              if (!exists) {
                Storage.last.ok = false;
                Storage.last.error = "נראה שהשמירה לא נקלטה בגליון (בדוק Deploy/הרשאות)";
                State.data.activity.unshift({ at: nowISO(), text: "אזהרה: שמירה לא נקלטה בגליון (בדוק Deploy/הרשאות)." });
              }
            }
          } else if (!v.ok) {
            Storage.last.ok = false;
            Storage.last.error = "שמירה הצליחה אך קריאה חזרה נכשלה";
          }
        } catch (_) {
          // לא מפילים את המערכת על אימות
        }
      }
    ...
    async setMode(mode) {
      // Local disabled
      notify("מצב Local בוטל. המערכת עובדת תמיד מול Google Sheets.", "warn");
      Storage.mode = "sheets";
      UI.renderSyncStatus();
    },
