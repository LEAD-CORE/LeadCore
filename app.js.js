...
(() => {
  ...
  const Storage = {
    ...
    // ---------------------------
    // Live Sync (polling) - update across PCs without refresh
    // ---------------------------
    _liveTimer: null,
    _busy: false,
    _lastRemoteAt: null,
    ...
    _applyRemote(payload, at) {
      // Do NOT stamp updatedAt locally – we are receiving remote truth
      State.data = normalizeState(payload || {});
      State.data.meta ||= {};
      // Keep remote meta.updatedAt if exists; otherwise use server 'at'
    ...
    startLiveSync() {
      ...
      if (Storage.mode !== "sheets" || !Storage.gsUrl) return;
...
on(this.els.btnSearch, "click", () => {
  ...
  this.renderCustomers();
      ...
      this.els.gsUrl.addEventListener("change", () => {
        ...
        if (Storage.mode === "sheets") App.startLiveSync();
      });
  ...
  const App = {
    async boot() {
      // restore settings (default: Google Sheets + default URL)
      const savedUrl = localStorage.getItem("LEAD_CORE_GS_URL");
      const savedMode = localStorage.getItem("LEAD_CORE_MODE");

      Storage.gsUrl = (savedUrl && String(savedUrl).trim()) ? String(savedUrl).trim() : DEFAULT_GS_URL;
      Storage.mode  = (savedMode && String(savedMode).trim()) ? String(savedMode).trim() : "sheets";

      // persist defaults once
      if (!savedUrl || !String(savedUrl).trim()) localStorage.setItem("LEAD_CORE_GS_URL", Storage.gsUrl);
      if (!savedMode || !String(savedMode).trim()) localStorage.setItem("LEAD_CORE_MODE", Storage.mode);
      ...
      // start live sync across computers (no refresh needed)
      this.startLiveSync();
    },
    ...
    async save(activityText) {
      ...
      this._busy = false;
      if (r && r.ok && r.at) this._lastRemoteAt = r.at;
      if (!r.ok) {
        State.data.activity.unshift({ at: nowISO(), text: "שמירה נכשלה (בדוק חיבור/URL)." });
      }
      UI.renderSyncStatus();

      // restart live sync based on mode
      if (mode === "sheets") this.startLiveSync(); else this.stopLiveSync();
      return r;
    },
