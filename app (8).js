...
(() => {
  ...
  const normPolicyStatus = (s) => {
    ...
    if (v === "swapped" || v === "שוחלף") return "swapped";
    if (v === "pending_cancel" || v === "ממתין לביטול") return "pending_cancel";
    return v;
  };
  const isActivePolicy = (p) => {
    const st = normPolicyStatus(p?.status);
    return st === "active" || st === "pending_cancel"; // pending still counts until done
  };
  ...
  const UI = {
    ...
    init() {
      ...
      // cancelled accordion
      this.els.btnToggleCancelled = $("#btnToggleCancelled");
      this.els.cfCancelledPanel = $("#cfCancelledPanel");
      this.els.cfCancelledTbody = $("#cfCancelledTbody");
      this.els.cfCancelledCount = $("#cfCancelledCount");
      this.els.btnCancelledPolicies = $("#btnCancelledPolicies");
      this.els.modalCancelledPolicies = $("#modalCancelledPolicies");
      this.els.cancelledPoliciesTbody = $("#cancelledPoliciesTbody");

      // cancel policy modal
      this.els.modalCancelPolicy = $("#modalCancelPolicy");
      this.els.cancelPolicyForm = $("#cancelPolicyForm");
      this.els.cancelPolicyList = $("#cancelPolicyList");
      this.els.cancelChosenLine = $("#cancelChosenLine");
      ...
      // cancelled policies
      on(this.els.btnCancelledPolicies, "click", () => this.openCancelledPoliciesModal());

      // Cancelled accordion toggle
      on(this.els.btnToggleCancelled, "click", () => this.toggleCancelledPanel());
      ...
      on(this.els.btnAddPolicyInline, "click", async () => {
        ...
        c.policies ||= [];
        c.policies.unshift({ id: "p_" + uid(), type, company, premium, status: "פעיל", renewAt });
      ...
      on(this.els.cancelPolicyForm, "submit", async (e) => {
        e.preventDefault();

        const custId = safeTrim(this.els.modalCancelPolicy?.dataset?.customerId);
        const c = (State.data.customers || []).find(x => x.id === custId);
        if (!c) return;

        const chosenId = safeTrim(this.els.modalCancelPolicy?.dataset?.policyId);
        if (!chosenId) { notify("נא לבחור פוליסה לביטול.", "warn"); return; }

        const p = (c.policies || []).find(x => x.id === chosenId);
        if (!p) return;

        const fd = new FormData(this.els.cancelPolicyForm);
        const reason = safeTrim(fd.get("reason"));
        const postponeUntil = safeTrim(fd.get("postponeUntil"));
        const template = safeTrim(fd.get("template"));

        p.cancelReason = reason;
        p.cancelTemplate = template;

        if (postponeUntil) {
          p.status = "pending_cancel";
          p.cancelRequestedAt = nowISO();
          p.cancelPostponeUntil = postponeUntil;
          State.data.activity.unshift({ at: nowISO(), text: `בקשת ביטול נדחתה ל-${postponeUntil} עבור ${c.firstName} ${c.lastName}` });
        } else {
          p.status = "cancelled";
          p.cancelledAt = nowISO();
          p.cancelledBy = safeTrim(UI.els.cfAgentSelect?.value || c.assignedAgent || "");
          State.data.activity.unshift({ at: nowISO(), text: `בוטלה פוליסה עבור ${c.firstName} ${c.lastName} (${p.type} • ${p.company})` });
        }
        ...
        const r = await App.save("עודכן ביטול פוליסה");
        if (!r.ok) notify("שמירה נכשלה: " + (r.error || "שגיאה"), "error");
        this.renderAll();
      });
...
// Policy modal submit
      ...
      try{ this.closeCancelPolicyModal?.(); }catch(_){ }
      try{ this.closeCancelledPoliciesModal?.(); }catch(_){ }
        this.renderPolicies();
    ...
    renderPolicies() {
      ...
      const active = list.filter(p => isActivePolicy(p));
      ...
      this.els.cfPoliciesTbody.innerHTML = list.map(p => {
        ...
        return `
          <tr>
            <td>${escapeHtml(p.type || "")}${(p.status && normPolicyStatus(p.status)!=="active") ? ('<div class="muted small">סטטוס: ' + escapeHtml(normPolicyStatus(p.status)==="cancelled"?"בוטל":"שוחלף") + '</div>') : ""}</td>
      ...
      $$("button[data-cancelpol]", this.els.cfPoliciesTbody).forEach(btn => {
        on(btn, "click", () => {
          ...
          this.openCancelPolicyModal(pid);
        });
      });
      ...
      $$("button[data-swapol]", this.els.cfPoliciesTbody).forEach(btn => {
        on(btn, "click", async () => {
          ...
          const old = (c.policies || []).find(x => x.id === pid);
          if (!old) return;

          // mark old as swapped
          old.status = "swapped";
          old.swappedAt = nowISO();
          ...
          c.updatedAt = nowISO();
          this.renderPolicies();
          const r = await App.save("בוצע שחלוף פוליסה");
          if (!r.ok) notify("שמירה נכשלה: " + (r.error || "שגיאה"), "error");
          this.renderAll();
        });
      });
  ...
  // ---------------------------
  // LeadCore • Extra UI helpers (Cancelled Policies + robust modals)
  // ---------------------------

  const normalizePolicyStatus = (s) => {
    const v = safeTrim(s).toLowerCase();
    if (!v) return "active";
    if (v === "פעיל") return "active";
    if (v === "בוטל") return "cancelled";
    if (v === "שוחלף") return "swapped";
    if (v === "ממתין לביטול") return "pending_cancel";
    return v; // active/cancelled/swapped/pending_cancel
  };

  const isActivePolicy = (p) => normalizePolicyStatus(p?.status) === "active";
  const isCancelledPolicy = (p) => normalizePolicyStatus(p?.status) === "cancelled";

  UI.openCancelPolicyModal = function(policyId){
    try{
      const custId = safeTrim(this.els.customerFull?.dataset?.customerId);
      const c = (State.data.customers || []).find(x => x.id === custId);
      if (!c) return;

      const list = (c.policies || []).slice();
      if (!list.length){ notify("אין פוליסות לביטול.", "warn"); return; }

      // fill list of policies
      if (this.els.cancelPolicyList){
        this.els.cancelPolicyList.innerHTML = list.map(p => {
          const pid = escapeHtml(p.id);
          const title = `${escapeHtml(p.type || "")} • ${escapeHtml(p.company || "")}`;
          const prem = fmtMoney(p.premium || 0);
          return `
            <label class="radioRow">
              <input type="radio" name="policyPick" value="${pid}">
              <span class="radioRow__title">${title}</span>
              <span class="badge">${prem}</span>
            </label>
          `;
        }).join("");
      }
      ...
      // default selection
      const chosen = policyId && list.find(x=>x.id===policyId) ? policyId : list[0].id;
      this.els.modalCancelPolicy.dataset.customerId = custId;
      this.els.modalCancelPolicy.dataset.policyId = chosen;

      // set radio checked and chosen line
      const radios = $$("input[type='radio'][name='policyPick']", this.els.cancelPolicyList || document);
      radios.forEach(r => { r.checked = (r.value === chosen); });

      const p0 = list.find(x=>x.id===chosen);
      if (this.els.cancelChosenLine){
        this.els.cancelChosenLine.textContent = p0 ? `${p0.type || ""} • ${p0.company || ""} (#${p0.id})` : "—";
      }

      // listen change
      radios.forEach(r => on(r, "change", () => {
        this.els.modalCancelPolicy.dataset.policyId = r.value;
        const px = (c.policies||[]).find(x=>x.id===r.value);
        if (this.els.cancelChosenLine){
          this.els.cancelChosenLine.textContent = px ? `${px.type || ""} • ${px.company || ""} (#${px.id})` : "—";
        }
      }));

      // reset form
      this.els.cancelPolicyForm?.reset?.();

      // show modal
      this.els.modalCancelPolicy.classList.add("is-open");
      this.els.modalCancelPolicy.setAttribute("aria-hidden","false");
    }catch(e){ notify("שגיאה בפתיחת מסך ביטול פוליסה", "error"); }
  };

  UI.closeCancelPolicyModal = function(){
    if (!this.els.modalCancelPolicy) return;
    this.els.modalCancelPolicy.classList.remove("is-open");
    this.els.modalCancelPolicy.setAttribute("aria-hidden","true");
    try{
      this.els.modalCancelPolicy.dataset.customerId = "";
      this.els.modalCancelPolicy.dataset.policyId = "";
    }catch(_){}
  };

  UI.openCancelledPoliciesModal = function(){
    try{
      const custId = safeTrim(this.els.customerFull?.dataset?.customerId);
      const c = (State.data.customers || []).find(x => x.id === custId);
      if (!c) return;
      const cancelled = (c.policies || []).filter(isCancelledPolicy);

      if (this.els.cancelledPoliciesTbody){
        this.els.cancelledPoliciesTbody.innerHTML = cancelled.map(p => {
          const d = safeTrim(p.cancelledAt);
          const when = d ? new Date(d).toLocaleString("he-IL") : "—";
          return `
            <tr>
              ...
              <td>${escapeHtml(p.cancelReason || "")}</td>
              <td>${escapeHtml(p.cancelledBy || "")}</td>
            </tr>
          `;
        }).join("") || `<tr><td colspan="6" class="muted" style="padding:18px">אין פוליסות שבוטלו</td></tr>`;
      }

      if (this.els.modalCancelledPolicies){
        this.els.modalCancelledPolicies.classList.add("is-open");
        this.els.modalCancelledPolicies.setAttribute("aria-hidden","false");
      }
    }catch(e){ notify("שגיאה בפתיחת פוליסות שבוטלו", "error"); }
  };

  UI.closeCancelledPoliciesModal = function(){
    if (!this.els.modalCancelledPolicies) return;
    this.els.modalCancelledPolicies.classList.remove("is-open");
    this.els.modalCancelledPolicies.setAttribute("aria-hidden","true");
  };

  // Make renderPolicies robust with status normalization + correct monthly premium
  const _origRenderPolicies = UI.renderPolicies.bind(UI);
  UI.renderPolicies = function(){
    // call original to rebuild table
    _origRenderPolicies();

    // After original, recompute totals using normalized statuses
    const id = safeTrim(this.els.customerFull?.dataset?.customerId);
    const c = (State.data.customers || []).find(x => x.id === id);
    if (!c) return;

    const list = (c.policies || []).slice();
    const active = list.filter(isActivePolicy);
    const total = active.reduce((s,p)=> s + Number(p.premium||0), 0);
    c.monthlyPremium = total;
