...
on(this.els.btnSearch, "click", () => {
  ...
  UI.renderSyncStatus();
    ...
    openDrawer(customerId) {
      ...
      if(!this.els.drawerCustomer) return;
    ...
    renderSyncStatus() {
      if (!this.els.syncDot || !this.els.syncText || !this.els.lastSyncText) return;
...
async testConnection() {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      try{const t=document.getElementById("syncText"); if(t) t.textContent="שגיאה בטעינת המערכת (בדוק קונסול)";}catch(_e){}}
  });
