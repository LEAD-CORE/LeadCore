  ...
  function openWizard({ customerId=null, proposalId=null }){
    ...
    if(!proposal){
      ...
      proposal = {
        ...
        status: 'טיוטה',
        internalStatus: 'פתוח',
        proposalPurpose: '', // הוזלה | הרחבה | רכישת ביטוח חדש
        title: 'הצעה',
        pdfGenerated: false,
...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="hr"></div>
      <div class="sectionTitle">מטרת ההצעה</div>
      <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:8px">
        ${[
          {key:'הוזלה', label:'הוזלה'},
          {key:'הרחבה', label:'הרחבה'},
          {key:'רכישת ביטוח חדש', label:'רכישת ביטוח חדש'}
        ].map(opt => {
          const checked = (p.proposalPurpose||'')===opt.key ? 'checked' : '';
          return `<label style="display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid rgba(15,15,20,.12);border-radius:12px;background:#fff">
            <input type="radio" name="proposalPurpose" data-proposal-purpose="${opt.key}" ${checked}/>
            <span>${escapeHtml(opt.label)}</span>
          </label>`;
        }).join('')}
      </div>
      <div class="sideHint" style="margin-top:10px">ℹ️ מטרת ההצעה תופיע בדו״ח ה־PDF תחת "פוליסות קיימות".</div>
...
;
    ...
    // Proposal purpose (single choice)
    $$('[data-proposal-purpose]').forEach(r => r.addEventListener('change', () => {
      const v = r.getAttribute('data-proposal-purpose') || '';
      p.proposalPurpose = v;
      markDraft();
    }));
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="tableWrap">
        <table>
          <thead><tr><th>חברה</th><th>מוצר</th><th>מס׳ פוליסה</th><th>פרמיה</th><th>סכום ביטוח</th><th>הנחות</th><th>אחרי הנחה</th><th>כיסויים</th><th>שיעבוד</th><th></th></tr></thead>
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => `
              <tr>
                <td>${renderCompanyCell(x.company)}</td>
    ...
    $('#addNewBtn').addEventListener('click', () => {
      ...
      const policyNo = ($('#new_policyNo') ? String($('#new_policyNo').value||'').trim() : '');
      ...
      if(isCompProduct(product) && compensation <= 0){ toast('שגיאה', 'חובה למלא סכום פיצוי למחלות קשות/סרטן'); return; }
      const rec = { insuredId, company, product, policyNo, premium };
...
`;
  ...
  function renderPoliciesByInsured(p, kind){
    ...
    const blocks = list.map(ins => {
      ...
      const head = (kind === 'new')
        ? `<tr><th>חברה</th><th>מוצר</th><th>מס׳ פוליסה</th><th>פרמיה</th><th>הנחות</th><th>אחרי הנחה</th><th>סכום ביטוח/פיצוי</th><th>כיסויים</th><th>שיעבוד</th></tr>`
        : `<tr><th>חברה</th><th>מוצר</th><th>מס׳ פוליסה</th><th>פרמיה</th><th>סטטוס</th><th>נספחים</th></tr>`;
              ...
              <tbody>
                ${rows.map(x => {
                  if(kind === 'new'){
                    ...
                    const discText = summarizeDiscount(x.discount) || '—';
                    ...
                    const sumOrComp = (prod==='ריסק' || prod==='משכנתא') ? money(Number(x.sumInsured)||0)
                      : ((prod==='מחלות קשות' || prod==='מחלות סרטן') ? money(Number(x.compensation)||0) : '—');
                    return `<tr>
                      <td>${renderCompanyCell(x.company||'')}</td>
                      <td>${escapeHtml(x.product||'')}</td>
                      <td>${escapeHtml(x.policyNo||'')}</td>
                      <td>${money(x.premium||0)}</td>
                      <td>${escapeHtml(discText)}</td>
                      <td>${after}</td>
                      <td>${sumOrComp}</td>
                      <td>${cov}</td>
                      <td>${lien}</td>
                  ...
                  return `<tr>
                    ...
                    <td>${escapeHtml(x.policyNo||'')}</td>
                    <td>${money(x.premium||0)}</td>
                    <td>${chip(statusLabel, (dec==='keep')?'ok':(dec==='cancel_partial')?'warn':'warn'}</td>
                    <td>${annex ? `<div style="white-space:pre-wrap">${escapeHtml(annex)}</div>` : '—'}</td>
                  </tr>`;
                }).join('')}
              </tbody>
            </table>
  ...
  function openPrintView(proposal){
    ...
    try {
      ...
      const productToInsuranceType = (prod) => {
        const p = String(prod||'').trim();
        if(p === 'משכנתא') return 'משכנתא';
        if(['ריסק','אובדן כושר עבודה'].includes(p)) return 'חיים';
        if(['בריאות','תאונות אישיות','מחלות קשות','מחלות סרטן'].includes(p)) return 'בריאות';
        // fallback: use product if it matches a known branch label, otherwise "אחר"
        if(['חיים','בריאות'].includes(p)) return p;
        return 'אחר';
      };

      const purposeText = String(p.proposalPurpose || '').trim() || '—';
      const proposalStatusText = String(p.status || '').trim() || '—';
      ...
      const oldRows = oldPolicies.map(x => {
        ...
        const statusLabel = (dec==='keep') ? 'להשאיר ללא שינוי'
          ...
          : 'ביטול מלא';
        const annex = (dec==='cancel_partial') ? String(x.partialCancelAnnexes||'').trim() : '';
        const insType = productToInsuranceType(prod);
        return `
        <tr>
      ...
      `;
      }).join('') || `<tr><td colspan="12" class="mutedCell">אין פוליסות</td></tr>`;
      ...
      function medicalSection(){
        ...
        for(const ins of insuredList){
          const insId = ins.id;
          const bucketSet = p.medical && p.medical[insId];
          const hasNew = Array.isArray(p.newPolicies) && p.newPolicies.some(x => (x.insuredId||'main') === insId);

          // If there are new policies for this insured but no questionnaire filled yet – show a clear placeholder
          if(!bucketSet){
            if(hasNew){
              out.push(`
                <div class="box">
                  <div class="boxTitle">שאלון רפואי — ${esc(labelInsured(insId))}</div>
                  <div class="muted">לא מולא שאלון רפואי למבוטח זה.</div>
                </div>
              `);
            }
            continue;
      ...
      <body>
        <div class="page">
          ...
          <table class="t">
            <thead><tr><th>מבוטח</th><th>חברה</th><th>מוצר</th><th>מס׳ פוליסה</th><th>סוג ביטוח</th><th>מטרת ההצעה</th><th>סטטוס הצעה</th><th>פרמיה חודשית</th><th>סכום ביטוח</th><th>סכום פיצוי</th><th>סטטוס</th><th>נספחים לביטול</th></tr></thead>
            <tbody>
              ${oldRows}
              <tr class="sumRow">
                <td colspan="11">סה״כ</td><td class="sumGold">${money(sumOld)}</td>
              </tr>
            </tbody>
          </table>
