...
(() => {
  ...
  // Default Google Apps Script WebApp URL (fallback if none saved)
...
// ---------------------------
// Live Sync (Auto refresh without manual reload)
// Polls server every few seconds and refreshes UI when data changes.
// ---------------------------
let __liveTimer = null;
let __liveBusy = false;
let __lastServerAt = null;

async function liveSyncTick_(){
  try{
    if(Storage.mode !== "sheets") return;
    if(!Storage.gsUrl) return;
    if(__liveBusy) return;
    __liveBusy = true;

    const r = await Storage.loadSheets();
    if(!r.ok) return;

    const serverAt = safeTrim(r.at || r.payload?.meta?.updatedAt || "");
    if(__lastServerAt && serverAt && serverAt === __lastServerAt) return;

    // Don't spam initial load
    if(!__lastServerAt && serverAt){
      __lastServerAt = serverAt;
      return;
    }

    __lastServerAt = serverAt || nowISO();
    State.set(r.payload);
    UI.renderAll();
    UI.renderSyncStatus();
  } finally {
...
function startLiveSync(){
  stopLiveSync();
  __liveTimer = setInterval(liveSyncTick_, 4000);
  document.addEventListener("visibilitychange", () => {
    if(!document.hidden) liveSyncTick_();
  });
}
...
.replaceAll("<","&lt;")
  ...
  const App = {
    async boot() {
      // restore settings
            const __savedUrl = safeTrim(localStorage.getItem("LEAD_CORE_GS_URL") || "");
      Storage.gsUrl = __savedUrl || DEFAULT_GS_URL;
      if(!__savedUrl){ localStorage.setItem("LEAD_CORE_GS_URL", Storage.gsUrl); }
      ...
      if (r.ok) {
        ...
        State.data.activity.unshift({ at: nowISO(), text: "המערכת עלתה ב-Local (בעיה בחיבור ל-Sheets)." });
            ...
            UI.renderAll();
      UI.goView("dashboard");
      // start live sync after first render
      startLiveSync();
    },
    ...
    async save(activityText) {
      ...
      if(r.ok){ __lastServerAt = safeTrim(r.at || State.data.meta.updatedAt || "") || nowISO(); }
      UI.renderSyncStatus();
      return r;
    },
    ...
    async setMode(mode) {
      ...
      if (mode === "sheets") {
        ...
        if (r.ok) {
                    State.set(r.payload);
          __lastServerAt = safeTrim(r.at || State.data.meta.updatedAt || "") || nowISO();
