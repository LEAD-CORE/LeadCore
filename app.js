...
// Search (live filter on customers view)
    ...
    openCustomerFull(customerId) {
      ...
      // ===== Hotfix (JS only): hide top ID chip + keep ID inside contact/identity card =====
      try {
        const root = this.els.customerFull;
        const idNumber = safeTrim(c.idNumber);
        if (root) {
          // Fill any existing "ID number" targets (backup-safe)
          const targets = [
            this.els.cfId, // whatever it points to in this build
            root.querySelector("#cfIdContact"),
            root.querySelector("#cfIdLine"),
            root.querySelector("#cfIdNumber"),
            root.querySelector("[data-field='idNumber']"),
            root.querySelector("[data-role='idNumber']")
          ].filter(Boolean);

          targets.forEach(el => { try { el.textContent = idNumber || "—"; } catch(_){} });

          // Hide ONLY the top chip/badge that shows ID (if exists)
          const directChip = root.querySelector("#cfIdChip, #cfTopId, #cfIdTop");
          if (directChip) {
            const wrap = directChip.closest(".chip, .cfChip, .badge, .pill") || directChip;
            wrap.style.display = "none";
          } else if (idNumber) {
            // Fallback: find chip-like elements containing the ID number (outside details/cards)
            const isInsideDetails = (el) => !!el.closest(".detail, .detailRow, .profile__details, .customerCards, .contactCard, .cfDetails, .cfDetailsGrid");
            const candidates = Array.from(root.querySelectorAll(".chip, .cfChip, .badge, .pill, .metaChip, .lcChip"));
            candidates.forEach(el => {
              const t = safeTrim(el.textContent);
              if (!t) return;
              if (t.includes(idNumber) && !isInsideDetails(el)) {
