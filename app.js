...
function showSaveOverlay_({ title = "יוצר תיק לקוח…", sub = "אנא המתן, המערכת שומרת ומסנכרנת…", seconds = 6 } = {}){
  ...
  const tick = () => {
    ...
    if(left <= 0){
      clearInterval(t); t = null; // stop at 0 (spinner continues)
    }
  };
  ...
  return {
    setSub(txt){
      try{ if(subEl) subEl.textContent = String(txt||""); }catch(_){}
    },
    stop(){
      try{
...
const submitBtn = this.els.customerForm?.querySelector?.("button[type='submit']");
        ...
        if (editId) {
          ...
          if (!r.ok) {
            ...
            try{ saveOvCtl && saveOvCtl.setSub && saveOvCtl.setSub("שמירה נכשלה. בדוק חיבור ל-Google Sheets."); }catch(_){ }
            return;
          }

          this.closeModal();
          try{ saveOvCtl?.stop?.(); ovStopped = true; }catch(_){ }
        ...
        this.renderAll();
        return;        } finally {
          try{ if(!ovStopped) saveOvCtl?.stop?.(); }catch(_){ }
