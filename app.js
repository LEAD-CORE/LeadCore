...
// ---------------------------
// Customer-file Loading Overlay (clock) — min 6 seconds
// ---------------------------
function ensureGlobalLoading_(){
  let el = document.getElementById("globalLoading");
  if(el) return el;

  el = document.createElement("div");
  el.id = "globalLoading";
  el.className = "globalLoading";
  el.setAttribute("aria-hidden","true");

  el.innerHTML = `
    <div class="globalLoading__backdrop" aria-hidden="true"></div>
    <div class="globalLoading__panel" role="status" aria-live="polite">
      <div class="globalLoading__clock" aria-hidden="true"></div>
      <div class="globalLoading__text" id="globalLoadingText">טוען תיק לקוח…</div>
    </div>`;
  document.body.appendChild(el);
  return el;
}

function showGlobalLoading_(text){
  try{
    const el = ensureGlobalLoading_();
    const t = el.querySelector("#globalLoadingText");
    if(t) t.textContent = text || "טוען תיק לקוח…";
    el.classList.add("is-open");
    el.setAttribute("aria-hidden","false");
  }catch(_){}
}

function hideGlobalLoading_(){
  try{
...
async function withMinLoader_(promise, minMs=6000){
...
// Search (live filter on customers view)
      ...
      on(this.els.customerForm, "submit", async (e) => {
        ...
        if (editId) {
          ...
          this.closeModal();
          showGlobalLoading_("טוען תיק לקוח…");
          await withMinLoader_(Promise.resolve(true), 6000);
    ...
    openCustomerFull(customerId) {
      const c = State.data.customers.find(x => x.id === customerId);
      if (!c || !this.els.customerFull){ hideGlobalLoading_(); return; }
...
// ===== Customer creation loader (6s minimum) =====
function ensureCustomerLoader(){
  let el=document.getElementById("customerLoader");
  if(el) return el;
  el=document.createElement("div");
  el.id="customerLoader";
  el.className="globalLoading";
  el.innerHTML=`
    <div class="globalLoading__backdrop"></div>
    <div class="globalLoading__panel">
      <div class="globalLoading__clock"></div>
      <div style="font-weight:900;font-size:18px">טוען תיק לקוח…</div>
    </div>`;
  document.body.appendChild(el);
  return el;
}
function showCustomerLoader(){
  const el=ensureCustomerLoader();
  el.classList.add("is-open");
}
function hideCustomerLoader(){
  const el=document.getElementById("customerLoader");
