  ...
  const Auth = {
    ...
    init() {
      ...
      if (restored) {
        ...
        this.unlock();
        // Apply role-based UI + render after session restore
    ...
    renderAuthPill(){
      try {
        ...
        if (!Auth.current) {
          ...
          pill.classList.remove("is-premium");
          return;
        }

        const name = safeTrim(Auth.current.name) || "משתמש";
        const role = (Auth.current.role === "admin") ? "מנהל" : "נציג";

        pill.style.display = "inline-flex";
        pill.classList.add("is-premium");

        if (this.els.userPillText) {
          this.els.userPillText.textContent = `Premium • ${role}: ${name}`;
        }

        // Premium tag (if exists)
        const tag = pill.querySelector?.('.lcUserPill__tag');
