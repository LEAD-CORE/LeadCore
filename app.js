  ...
  const UI = {
    ...
    init() {
      ...
      this.els.drawerTabs.forEach(b => {
        b.addEventListener("click", () => this.setDrawerTab(b.dataset.tab));
      });

      // Drawer quick actions
      this.els.btnDrawerAddPolicy.addEventListener("click", () => { this.addPolicy(); this.setDrawerTab("policies"); });
      this.els.btnAddPolicyTop.addEventListener("click", () => this.addPolicy());

      this.els.btnDrawerAddTask.addEventListener("click", () => { this.setDrawerTab("tasks"); this.els.taskText.focus(); });
      this.els.btnAddTaskTop.addEventListener("click", () => { this.setDrawerTab("tasks"); this.els.taskText.focus(); });
      this.els.btnTaskAdd.addEventListener("click", () => this.addTask());

      this.els.btnDrawerAddLog.addEventListener("click", () => { this.setDrawerTab("timeline"); this.els.logText.focus(); });
      this.els.btnLogAdd.addEventListener("click", () => this.addLog());

      this.els.btnAddDocTop.addEventListener("click", () => { this.setDrawerTab("docs"); this.els.docName.focus(); });
      this.els.btnDocAdd.addEventListener("click", () => this.addDoc());

      // Drawer editable fields -> mark dirty
      this.els.drawerPhoneInput.addEventListener("input", () => this.markDrawerDirty());
      this.els.drawerIdInput.addEventListener("input", () => this.markDrawerDirty());

      // Delegation: policies edits + deletions
      this.els.policiesRows.addEventListener("input", (e) => {
      ...
      this.els.policiesRows.addEventListener("click", (e) => {
      ...
      this.els.timelineList.addEventListener("click", (e) => {
        const id = this.els.drawerCustomer.dataset.customerId;
        const c = State.data.customers.find(x => x.id === id);
        if (!c) return;
      ...
      this.els.docsList.addEventListener("click", (e) => {
        const id = this.els.drawerCustomer.dataset.customerId;
        const c = State.data.customers.find(x => x.id === id);
        if (!c) return;
