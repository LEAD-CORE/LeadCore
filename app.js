...
(() => {
  ...
  const waitWithTimeout = async (promise, timeoutMs = 9000) => {
    let to;
    try{
      return await Promise.race([
        promise,
        new Promise((_, rej) => { to = setTimeout(() => rej(new Error("timeout")), timeoutMs); })
      ]);
    } finally {
      clearTimeout(to);
    }
...
// ---------------------------
  ...
  const Auth = {
    ...
    _loginSubmit: async function() {
      ...
      try { if (App && App._bootPromise) await waitWithTimeout(App._bootPromise, 9000); } catch (_) {}

      // If we don't have users data, it's almost always a connection/backup issue (show a clear message)
      const hasUsersData = Array.isArray(State.data?.agents) && State.data.agents.some(a => safeTrim(a?.name)) && (
        State.data.agents.some(a => safeTrim(a?.username)) || State.data.agents.some(a => safeTrim(a?.pin))
      );
      ...
      if (!hasUsersData && !(State.data?.meta?.adminAuth)) {
        return this._setError("אין נתוני משתמשים. בדוק חיבור ל-Google Sheets או פתח 'הגדרות מערכת' ובצע 'בדיקת חיבור'.");
      }
...
openPolicyActionModal(action, policyId, policyLabel) {
    ...
    renderCustomers() {
      ...
      this.els.customersTbody.innerHTML = list.map(c => `
        <tr class="lcRowOpen" data-open="${escapeHtml(c.id)}">
      ...
      // Row click (open file) - avoids global table changes, scoped to customers tbody
      $$(".lcRowOpen", this.els.customersTbody).forEach(tr => {
        on(tr, "click", (e) => {
          // don't double-trigger when clicking on buttons/inputs/links
