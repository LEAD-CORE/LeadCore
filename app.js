...
(() => {
  ...
  function normalizeState(s) {
    ...
    out.customers = (out.customers || []).map((c) => ({
      ...
      policies: Array.isArray(c?.policies) ? c.policies.map((p) => ({
        ...
        secondary: (p && typeof p === "object" && p.secondary) ? {
          customerId: safeTrim(p.secondary?.customerId),
          idNumber: safeTrim(p.secondary?.idNumber),
          name: safeTrim(p.secondary?.name),
          phone: safeTrim(p.secondary?.phone)
        } : null,
        children: Array.isArray(p?.children) ? p.children.map(ch => ({
          firstName: safeTrim(ch?.firstName),
          lastName: safeTrim(ch?.lastName),
          idNumber: safeTrim(ch?.idNumber),
          birthDate: safeTrim(ch?.birthDate)
        })).filter(ch => ch.firstName || ch.lastName || ch.idNumber || ch.birthDate) : []
      })) : []
    }));
  ...
  const UI = {
    ...
    init() {
      ...
      // Policy extras (secondary insured + children)
      this.els.secondaryIdNumber = $("#secondaryIdNumber");
      this.els.secondaryDisplay = $("#secondaryDisplay");
      this.els.secondaryCustomerId = $("#secondaryCustomerId");
      this.els.btnAddChild = $("#btnAddChild");
      this.els.childrenList = $("#childrenList");
      ...
      // Policy extras: lookup secondary insured by ID number
      const lookupSecondary = () => {
        const idNum = safeTrim(this.els.secondaryIdNumber?.value).replace(/\D/g, "");
        if (!this.els.secondaryDisplay) return;
        if (!idNum) {
          this.els.secondaryDisplay.value = "";
          if (this.els.secondaryCustomerId) this.els.secondaryCustomerId.value = "";
          return;
        }
        const found = (State.data.customers || []).find(c => String(c.idNumber || "").replace(/\D/g, "") === idNum);
        if (!found) {
          this.els.secondaryDisplay.value = "לא נמצא לקוח במאגר";
          if (this.els.secondaryCustomerId) this.els.secondaryCustomerId.value = "";
          return;
        }
        const fullName = [found.firstName, found.lastName].filter(Boolean).join(" ").trim();
        const phone = safeTrim(found.phone);
        this.els.secondaryDisplay.value = (fullName ? fullName : "לקוח") + (phone ? (" • " + phone) : "") + " • ת"ז " + idNum;
        if (this.els.secondaryCustomerId) this.els.secondaryCustomerId.value = found.id;
      };

      if (this.els.secondaryIdNumber) {
        on(this.els.secondaryIdNumber, "input", () => {
          // lightweight debounce
          clearTimeout(this._secT);
          this._secT = setTimeout(lookupSecondary, 120);
        });
        on(this.els.secondaryIdNumber, "blur", lookupSecondary);
      }

      // Children: add rows
      if (this.els.btnAddChild) {
        on(this.els.btnAddChild, "click", () => this._addChildRow());
      }
...
// Search (live filter on customers view)
      ...
      // Policy modal submit
        ...
        const secIdNum = safeTrim(this.els.secondaryIdNumber?.value).replace(/\D/g, "");
        const secCustId = safeTrim(this.els.secondaryCustomerId?.value);
        let secondary = null;
        if (secIdNum) {
          const found = (State.data.customers || []).find(x => String(x.idNumber || "").replace(/\D/g, "") === secIdNum);
          if (!found) { notify("מבוטח משני לא נמצא במאגר לפי ת״ז.", "warn"); return; }
          const fullName = [found.firstName, found.lastName].filter(Boolean).join(" ").trim();
          secondary = { customerId: found.id, idNumber: secIdNum, name: fullName, phone: safeTrim(found.phone) };
        }

        const children = this._collectChildren();
        if (children === null) return;

        c.policies ||= [];
        c.policies.unshift({ id: "p_" + uid(), type, company, premium, status: "active", renewAt, secondary, children: children || [] });
        c.updatedAt = nowISO();
...
this.renderPolicies();
    ...
    openPolicyModal() {
      ...
      // reset extras
      if (this.els.secondaryIdNumber) this.els.secondaryIdNumber.value = "";
      if (this.els.secondaryDisplay) this.els.secondaryDisplay.value = "";
      if (this.els.secondaryCustomerId) this.els.secondaryCustomerId.value = "";
      if (this.els.childrenList) this.els.childrenList.innerHTML = "";
...
openPolicyActionModal(action, policyId, policyLabel) {
    ...
    _addChildRow() {
      if (!this.els.childrenList) return;
      const idx = (this.els.childrenList.querySelectorAll(".lcChildRow").length || 0) + 1;
      const row = document.createElement("div");
      row.className = "lcChildRow";
      row.innerHTML = `
        <div class="formGrid lcChildGrid">
          <div class="field">
            <label class="label">שם פרטי</label>
            <input class="input" data-ch="firstName" placeholder="שם פרטי" />
          </div>
          <div class="field">
            <label class="label">שם משפחה</label>
            <input class="input" data-ch="lastName" placeholder="שם משפחה" />
          </div>
          <div class="field">
            <label class="label">ת״ז</label>
            <input class="input" data-ch="idNumber" inputmode="numeric" placeholder="ת״ז" />
          </div>
          <div class="field">
            <label class="label">תאריך לידה</label>
            <input class="input" data-ch="birthDate" type="date" />
          </div>
        </div>
        <div class="row" style="justify-content:flex-start; margin-top:8px">
          <button class="btn btn--danger" type="button" data-remove-child="1">הסר ילד</button>
        </div>
      `;
      this.els.childrenList.appendChild(row);
      const rm = row.querySelector("[data-remove-child='1']");
      if (rm) on(rm, "click", () => row.remove());
    },

    _collectChildren() {
      const out = [];
      const rows = this.els.childrenList ? Array.from(this.els.childrenList.querySelectorAll(".lcChildRow")) : [];
      for (const r of rows) {
        const firstName = safeTrim(r.querySelector("[data-ch='firstName']")?.value);
        const lastName  = safeTrim(r.querySelector("[data-ch='lastName']")?.value);
        const idNumber  = safeTrim(r.querySelector("[data-ch='idNumber']")?.value).replace(/\D/g, "");
        const birthDate = safeTrim(r.querySelector("[data-ch='birthDate']")?.value);

        // if row is completely empty -> ignore
        const any = firstName || lastName || idNumber || birthDate;
        if (!any) continue;

        // require all
        if (!(firstName && lastName && idNumber && birthDate)) {
          notify("נא למלא לילד: שם פרטי, שם משפחה, ת״ז ותאריך לידה.", "warn");
          return null;
        }

        // age check (<16)
        const bd = new Date(birthDate);
        if (String(bd) === "Invalid Date") {
          notify("תאריך לידה לא תקין (ילד).", "warn");
          return null;
        }
        const now = new Date();
        const ageYears = (now - bd) / (365.25 * 24 * 3600 * 1000);
        if (ageYears >= 16) {
          notify("ילד חייב להיות מתחת לגיל 16.", "warn");
          return null;
        }

        out.push({ firstName, lastName, idNumber, birthDate });
      }
      return out;
    },
    ...
    renderPolicies() {
      ...
      this.els.cfPoliciesTbody.innerHTML = list.map(p => {
        ...
        const hasSecondary = !!(p.secondary && (p.secondary.idNumber || p.secondary.name));
        const kids = Array.isArray(p.children) ? p.children : [];
        const hasKids = kids.length > 0;

        let detailHtml = "";
        if (hasSecondary || hasKids) {
          const sec = hasSecondary ? p.secondary : null;
          const secLine = sec ? (`<span class="lcPolChip"><span class="lcPolChip__k">מבוטח משני:</span> ${escapeHtml(sec.name || "—")} <span class="muted small">(${escapeHtml(sec.idNumber || "")}${sec.phone ? (" • " + escapeHtml(sec.phone)) : ""})</span></span>`) : "";
          let kidsLine = "";
          if (hasKids) {
            const shown = kids.slice(0,3).map(ch => {
              const nm = [ch.firstName, ch.lastName].filter(Boolean).join(" ").trim();
              return escapeHtml(nm || ch.idNumber || "ילד");
            });
            const more = kids.length > 3 ? ` +עוד ${kids.length-3}` : "";
            kidsLine = `<span class="lcPolChip"><span class="lcPolChip__k">ילדים:</span> ${shown.join(" · ")}${more}</span>`;
          }
          detailHtml = `
            <tr class="lcPolDetailRow">
              <td colspan="5">
                <div class="lcPolDetail">
                  ${secLine}
                  ${kidsLine}
                </div>
              </td>
