...
`;
  ...
  function renderSummaryStep(){
    ...
    if(pdfBtnEl) pdfBtnEl.addEventListener('click', () => {
      // Mark PDF as generated for completion badges, but DO NOT force proposal back to "טיוטה"
      // and do not rerender the whole app (prevents UI "getting stuck"/overwritten).
      try{
        wiz.proposal.pdfGenerated = true;
        saveProposals_();
        if(compHost){
          const comp = computeCompletion(wiz.proposal);
          compHost.innerHTML = renderCompletionBadges(comp);
    ...
    const blocks = list.map(ins => {
      ...
      return `
        <div class="sideCard" style="box-shadow:none">
          ...
          <div class="tableWrap summaryTableWrap" style="margin-top:6px">
    ...
    try {
      ...
      <body>
        <div class="page">
          ...
          <table class="t">
            <thead><tr><th>מבוטח</th><th>חברה</th><th>מוצר</th><th>סוג ביטוח</th><th>מטרת ההצעה</th><th>סטטוס הצעה</th><th>פרמיה חודשית</th><th>סכום ביטוח</th><th>סכום פיצוי</th><th>סטטוס</th><th>נספחים לביטול</th></tr></thead>
            <tbody>
              ${oldRows}
              <tr class="sumRow">
                <td colspan="10">סה״כ</td><td class="sumGold">${money(sumOld)}</td>
              </tr>
            </tbody>
          </table>
