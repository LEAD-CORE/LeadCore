...
if (!out.agents.length) {
  ...
  const UI = {
    ...
    init() {
      ...
      // customer documents (full screen)
      this.els.customerDocsFull = $("#customerDocsFull");
      this.els.cdCustomerName = $("#cdCustomerName");
      this.els.cdDocName = $("#cdDocName");
      this.els.cdDocDate = $("#cdDocDate");
      this.els.cdDocUrl = $("#cdDocUrl");
      this.els.cdAddLinkBtn = $("#cdAddLinkBtn");
      this.els.cdClearFormBtn = $("#cdClearFormBtn");
      this.els.cdFile = $("#cdFile");
      this.els.cdUploadBtn = $("#cdUploadBtn");
      this.els.cdDocsTbody = $("#cdDocsTbody");
      this.els.cdEmpty = $("#cdEmpty");
      this.els.cdNotice = $("#cdNotice");
...
this.els.btnUserEditSave = $("#btnUserEditSave");
      ...
      on(this.els.cfDocsBtn, "click", () => {
        const c = getOpenCustomer();
        if (!c) return notify("בחר לקוח כדי לפתוח מסמכים.", "warn");
        this.openCustomerDocsFull(c.id);
      });
...
if(!id){ notify("לא נבחר לקוח לעריכה.", "warn"); return; }
      ...
      this.closeCustomerDocsFull();
        if (el.closest("#customerDocsFull")) return this.closeCustomerDocsFull();
        // Fallback
        this.closeOverlays();
      }));

      
      // Customer Docs events
      on(this.els.cdClearFormBtn, "click", () => {
        this._clearDocsForm();
        this._docsNotice("", "");
      });

      on(this.els.cdAddLinkBtn, "click", async () => {
        const wrap = this.els.customerDocsFull;
        const cid = safeTrim(wrap?.dataset?.customerId);
        if (!cid) return notify("בחר לקוח לפני שמירת מסמך.", "warn");

        const c = (State.data.customers || []).find(x => x.id === cid);
        if (!c) return notify("לקוח לא נמצא.", "warn");

        const name = safeTrim(this.els.cdDocName?.value);
        const date = safeTrim(this.els.cdDocDate?.value) || (new Date()).toISOString().slice(0,10);
        const url = safeTrim(this.els.cdDocUrl?.value);

        if (!name) return this._docsNotice("חסר שם מסמך.", "warn");
        if (!url) return this._docsNotice("חסר קישור למסמך.", "warn");
        ...
        const ok = await this._saveDocsState();
        if (ok) {
          this._docsNotice("נשמר ✔️", "ok");
          this._clearDocsForm();
          if (this.els.cdDocDate) this.els.cdDocDate.value = (new Date()).toISOString().slice(0,10);
          this.renderCustomerDocs(cid);
        }
      });

      on(this.els.cdFile, "change", () => {
        const f = this.els.cdFile?.files?.[0];
        if (!f) {
          if (this.els.cdUploadBtn) this.els.cdUploadBtn.disabled = true;
          return;
        }
        // Upload to Drive requires server support; we keep UX clear and safe.
        if (this.els.cdUploadBtn) this.els.cdUploadBtn.disabled = true;
        this._docsNotice("העלאה ל-Drive דורשת תמיכה בשרת (Apps Script). בינתיים: העלה ל-Drive ידנית והדבק קישור.", "warn");
      });

      on(this.els.cdUploadBtn, "click", () => {
        this._docsNotice("העלאה ל-Drive עדיין לא הופעלה בשרת. השתמש בשמירת קישור.", "warn");
      });

      on(this.els.cdDocsTbody, "click", async (e) => {
        const btn = e.target?.closest?.("button[data-doc-del]");
        if (!btn) return;
        const docId = safeTrim(btn.dataset.docDel);
        const wrap = this.els.customerDocsFull;
        const cid = safeTrim(wrap?.dataset?.customerId);
        if (!cid || !docId) return;

        const c = (State.data.customers || []).find(x => x.id === cid);
        if (!c || !Array.isArray(c.documents)) return;

        c.documents = c.documents.filter(d => safeTrim(d?.id) !== docId);

        const ok = await this._saveDocsState();
        if (ok) {
          this._docsNotice("נמחק ✔️", "ok");
          this.renderCustomerDocs(cid);
        }
      });
...
this.renderPolicies();
    ...
    // Customer Documents • Full Screen
    // ---------------------------
    openCustomerDocsFull(customerId) {
      const c = (State.data.customers || []).find(x => x.id === customerId);
      if (!c || !this.els.customerDocsFull) return;

      this.els.customerDocsFull.dataset.customerId = c.id;
      if (this.els.cdCustomerName) this.els.cdCustomerName.textContent = `${c.firstName} ${c.lastName}`.trim() || "—";

      // init defaults
      if (this.els.cdDocDate && !this.els.cdDocDate.value) this.els.cdDocDate.value = (new Date()).toISOString().slice(0,10);
      this._docsNotice("", "");

      this.renderCustomerDocs(c.id);

      this.els.customerDocsFull.classList.add("is-open");
      this.els.customerDocsFull.setAttribute("aria-hidden", "false");
    },

    closeCustomerDocsFull() {
      if (!this.els.customerDocsFull) return;
      this.els.customerDocsFull.classList.remove("is-open");
      this.els.customerDocsFull.setAttribute("aria-hidden", "true");
      this.els.customerDocsFull.dataset.customerId = "";
      this._clearDocsForm();
      this._docsNotice("", "");
    },

    renderCustomerDocs(customerId) {
      const c = (State.data.customers || []).find(x => x.id === customerId);
      if (!c) return;

      const docs = Array.isArray(c.documents) ? c.documents : [];
      // keep schema on the object
      if (!Array.isArray(c.documents)) c.documents = docs;

      if (this.els.cdDocsTbody) this.els.cdDocsTbody.innerHTML = "";
      if (this.els.cdEmpty) this.els.cdEmpty.hidden = docs.length > 0;

      if (!this.els.cdDocsTbody) return;
      ...
      docs
        ...
        .forEach(d => {
          ...
          tdName.textContent = safeTrim(d?.name) || "—";
          tr.appendChild(tdName);

          const tdDate = document.createElement("td");
          tdDate.textContent = safeTrim(d?.date) || "—";
          tr.appendChild(tdDate);

          const tdType = document.createElement("td");
          tdType.textContent = safeTrim(d?.type) || "קישור";
          tr.appendChild(tdType);

          const tdAct = document.createElement("td");
          tdAct.style.whiteSpace = "nowrap";

          const url = safeTrim(d?.url);
          if (url) {
            const a = document.createElement("a");
            a.href = url;
            a.target = "_blank";
            a.rel = "noopener";
            a.className = "btn btn--tiny";
            a.textContent = "פתח";
            tdAct.appendChild(a);
          }
          ...
          del.textContent = "מחק";
          del.dataset.docDel = safeTrim(d?.id) || "";
          tdAct.appendChild(del);

          tr.appendChild(tdAct);
          this.els.cdDocsTbody.appendChild(tr);
        });
    },
    ...
    _clearDocsForm() {
      ...
      if (this.els.cdFile) this.els.cdFile.value = "";
      if (this.els.cdUploadBtn) this.els.cdUploadBtn.disabled = true;
    },

    _docsNotice(text, kind) {
      if (!this.els.cdNotice) return;
      this.els.cdNotice.textContent = text || "";
      this.els.cdNotice.classList.remove("is-warn", "is-ok");
      if (kind === "warn") this.els.cdNotice.classList.add("is-warn");
      if (kind === "ok") this.els.cdNotice.classList.add("is-ok");
    },

    async _saveDocsState() {
      // update updatedAt + normalize
      ...
      if (r.ok) {
        ...
        this.renderSyncStatus("נשמר", "ok", 1600);
        return true;
      }
      this.renderSyncStatus("שגיאת שמירה", "err", 2200);
      notify("לא הצלחתי לשמור לשיטס. בדוק חיבור/הרשאות.", "warn");
      return false;
    },
openPolicyModal() {
