...
// ---------------------------
  ...
  const Auth = {
    ...
    _loginSubmit() {
      ...
      // Admin default
      if (username === "מנהל מערכת" && pin === "1234") {
      ...
      // Agent: allow matching existing agent name; default PIN is 0000 (can be expanded later in users screen)
      const agents = State.data?.agents || [];
      const matched = agents.find(a => safeTrim(a?.name) === username);
      if (!matched) return this._setError("שם משתמש לא נמצא");

      if (pin !== "0000") return this._setError("קוד כניסה שגוי");
  ...
  const UI = {
    ...
    init() {
      ...
      // Customer file: change assigned agent (admin only)
      on(this.els.cfAgentSelect, "change", async () => {
        try {
          if (!Auth.isAdmin()) {
            notify("אין הרשאה לשנות נציג מטפל.", "warn");
...
if (this.els.cfIdIssueDate) this.els.cfIdIssueDate.textContent = (c.idIssueDate || "—");
      ...
      if (this.els.cfAgentSelect) {
        ...
        this.els.cfAgentSelect.disabled = !Auth.isAdmin();
      }
this.renderPolicies();
...
openPolicyActionModal(action, policyId, policyLabel) {
    ...
    renderUsers() {
      ...
      // In this phase, we use agents list as system users (role: agent).
      // Admin is fixed and not editable here.
      const agents = Array.isArray(State.data.agents) ? State.data.agents : [];
      const rows = [{ name: "מנהל מערכת", role: "Admin", status: "פעיל", locked: true }]
        .concat(agents.map(a => ({ name: safeTrim(a.name), role: "נציג/סוכן", status: "פעיל", locked: false })));

      const visible = rows
        .filter(r => r.name)
        .filter(r => !q ? true : r.name.toLowerCase().includes(q))
        .filter(r => {
          if (filter === "active") return r.status === "פעיל";
          if (filter === "disabled") return r.status !== "פעיל";
          return true;
        });

      this.els.usersTbody.innerHTML = visible.map(r => {
        const actions = r.locked
          ? `<span class="muted">מובנה</span>`
          : `<span class="muted">עריכה תתווסף בהמשך</span>`;
        return `
          <tr>
      ...
      on(this.els.btnAddUser, "click", () => notify("הוספת נציג/סוכן תתווסף בשלב הבא.", "info"));
    },
