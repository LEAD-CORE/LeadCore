...
(() => {
  ...
  const UI = {
    ...
    init() {
      ...
      this.els.cfAgent = $("#cfAgent");
      this.els.cfFilterActiveOnly = $("#cfFilterActiveOnly");
...
on(this.els.btnSearch, "click", () => {
  ...
  this.renderCustomers();
      ...
      // Customer Full Screen handlers
      on(this.els.btnAddPolicy, "click", () => this.openPolicyModal());

      const rerenderPolicies = () => this.renderPolicies();
      on(this.els.cfFilterActiveOnly, "change", rerenderPolicies);
      on(this.els.cfFilterCompany, "change", rerenderPolicies);
      on(this.els.cfFilterType, "change", rerenderPolicies);

      on(this.els.cfAgent, "change", async () => {
        const id = safeTrim(this.els.customerFull?.dataset?.customerId);
        const c = State.data.customers.find(x => x.id === id);
        if (!c) return;
        const next = safeTrim(this.els.cfAgent.value);
    ...
    openCustomerFull(customerId) {
      ...
      this.els.cfAgent.value = c.assignedAgent || "";

      // default filters
      if (this.els.cfFilterActiveOnly) this.els.cfFilterActiveOnly.checked = true;
      if (this.els.cfFilterCompany) this.els.cfFilterCompany.value = "";
      if (this.els.cfFilterType) this.els.cfFilterType.value = "";
    ...
    renderPolicies() {
      ...
      // Options from policies
      const companies = Array.from(new Set(c.policies.map(p => safeTrim(p.company)).filter(Boolean))).sort((a,b)=>a.localeCompare(b,"he"));
      const types = Array.from(new Set(c.policies.map(p => safeTrim(p.type)).filter(Boolean))).sort((a,b)=>a.localeCompare(b,"he"));

      if (this.els.cfFilterCompany) {
        const cur = this.els.cfFilterCompany.value || "";
        this.els.cfFilterCompany.innerHTML =
          `<option value="">כל החברות</option>` +
          companies.map(v => `<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join("");
        this.els.cfFilterCompany.value = cur;
      }

      if (this.els.cfFilterType) {
        const cur = this.els.cfFilterType.value || "";
        this.els.cfFilterType.innerHTML =
          `<option value="">כל הסוגים</option>` +
          types.map(v => `<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join("");
        this.els.cfFilterType.value = cur;
      }

      const activeOnly = !!this.els.cfFilterActiveOnly?.checked;
      const fCompany = safeTrim(this.els.cfFilterCompany?.value);
      const fType = safeTrim(this.els.cfFilterType?.value);

      const list = c.policies.filter(p => {
        if (activeOnly && safeTrim(p.status) !== "פעיל") return false;
        if (fCompany && safeTrim(p.company) !== fCompany) return false;
        if (fType && safeTrim(p.type) !== fType) return false;
        return true;
      });

      const totalActive = c.policies
        .filter(p => safeTrim(p.status) === "פעיל")
        .reduce((s,p)=> s + Number(p.premium||0), 0);

      const filteredTotal = list.reduce((s,p)=> s + Number(p.premium||0), 0);
      const activeCount = c.policies.filter(p => safeTrim(p.status) === "פעיל").length;

      // Sync old field used elsewhere
      c.monthlyPremium = totalActive;

      if (this.els.cfTotalPremium) this.els.cfTotalPremium.textContent = fmtMoney(totalActive);
      if (this.els.cfActiveCount) this.els.cfActiveCount.textContent = String(activeCount);
      if (this.els.cfFilteredTotal) this.els.cfFilteredTotal.textContent = fmtMoney(filteredTotal);
