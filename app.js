...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  const mailBtnTop = $('#mailBtnTop');
  const mailBadgeEl = $('#mailBadge');
  const mailFloatEl = $('#mailFloat');
  const mailCloseBtn = $('#mailCloseBtn');
  const mailMinBtn = $('#mailMinBtn');
  const mailTabInbox = $('#mailTabInbox');
  const mailTabCompose = $('#mailTabCompose');
  const mailPaneInbox = $('#mailPaneInbox');
  const mailPaneCompose = $('#mailPaneCompose');
  const mailListEl = $('#mailList');
  const mailReaderEl = $('#mailReader');
  const mailReaderMetaEl = $('#mailReaderMeta');
  const mailReaderSubjEl = $('#mailReaderSubj');
  const mailReaderBodyEl = $('#mailReaderBody');
  const mailBackBtn = $('#mailBackBtn');
  const mailToEl = $('#mailTo');
  const mailSubjectEl = $('#mailSubject');
  const mailBodyEl = $('#mailBody');
  const mailSendBtn = $('#mailSendBtn');
  const mailClearBtn = $('#mailClearBtn');
  const mailFloatSub = $('#mailFloatSub');
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  function showLogin_(){
    ...
    try{ if(typeof closeMail_ === 'function') closeMail_(); }catch(_){/*noop*/}
  ...
  function showApp_(){
    ...
    try{ if(typeof initInternalMail_ === 'function') initInternalMail_(); }catch(_){/*noop*/}
  ...
  window.gemelLogout = function(){
    ...
    // ---------- INTERNAL MAIL (Floating) ----------
  // LocalStorage-based internal mail (demo). Designed to be swapped to server/Sheets later.
  const MAIL_LS_KEY = 'gemel_mail_v1';
  let mailWinRef_ = null;
  let mailUIWired_ = false;
  let lastUnread_ = 0;
  let activeMailId_ = null;
  ...
  function meName_(){
    return String(state.agentName || '').trim() || 'אורח';
  }
  function inboxFor_(agentName){
    const me = String(agentName||'').trim();
    return loadMail_().filter(m => String(m.to||'').trim() === me)
      .sort((a,b)=> new Date(b.sentAt||0).getTime() - new Date(a.sentAt||0).getTime());
  }
  function unreadCountFor_(agentName){
    return inboxFor_(agentName).filter(m => !m.read).length;
  }
  ...
  function setMailTab_(tab){
    if(!mailTabInbox || !mailTabCompose || !mailPaneInbox || !mailPaneCompose) return;
    const isInbox = (tab === 'inbox');
    mailTabInbox.classList.toggle('active', isInbox);
    mailTabCompose.classList.toggle('active', !isInbox);
    mailPaneInbox.style.display = isInbox ? '' : 'none';
    mailPaneCompose.style.display = isInbox ? 'none' : '';
    if(mailReaderEl){
      // when switching tabs, reset reader view
      if(!isInbox) mailReaderEl.style.display = 'none';
    }
    if(isInbox) renderInbox_();
    if(!isInbox) buildRecipients_();
  }

  function openMailWindow_(){
    // Open a small separate window (not inside the CRM UI)
    const name = 'gemelMailWindow';
    const w = 420, h = 640;
    const left = Math.max(20, (window.screenX || 0) + (window.outerWidth || 1200) - w - 40);
    const top  = Math.max(20, (window.screenY || 0) + 90);
    const features = `popup=yes,width=${w},height=${h},left=${left},top=${top},resizable=yes,scrollbars=yes`;
    try{
      mailWinRef_ = window.open('', name, features);
    }catch(_){
      mailWinRef_ = null;
    }
    if(!mailWinRef_){
      toast('חסום', 'הדפדפן חסם חלון קופץ. אפשר/י Popups לאתר ואז נסה/י שוב.');
      return;
    }
    try{ mailWinRef_.focus(); }catch(_){}

    // Build a lightweight mail app inside the new window (same-origin about:blank)
    const me = meName_();
...
<style>
  :root{
    --bg:#F6F2EA; --panel:#fff; --line:rgba(15,17,22,.12);
    --text:#0F1116; --muted:rgba(15,17,22,.65); --gold:#D6B25E;
    --shadow:0 18px 45px rgba(18,19,25,.10);
    --radius:16px;
    font-family: ui-sans-serif, system-ui, -apple-system, "Noto Sans Hebrew", "Heebo", Arial;
  }
  body{ margin:0; background:linear-gradient(180deg,#FFFEF8,var(--bg)); color:var(--text); }
  .wrap{ padding:14px; }
  .card{
  ...
  .hdr{
    ...
    background:linear-gradient(180deg, rgba(255,255,255,.95), rgba(246,242,234,.85));
  }
  .title{ font-weight:900; }
  .sub{ font-size:12px; color:var(--muted); margin-top:2px; }
  .tabs{ display:flex; gap:8px; padding:10px 12px; border-bottom:1px solid rgba(15,17,22,.10); }
  .tab{
    border:1px solid rgba(214,178,94,.18);
  ...
  .tab.active{ border-color:rgba(214,178,94,.45); box-shadow:0 0 0 3px rgba(214,178,94,.10); }
  ...
  .item.unread{ border-color: rgba(214,178,94,.55); box-shadow:0 0 0 3px rgba(214,178,94,.10); }
  .row{ display:flex; align-items:center; justify-content:space-between; gap:10px; }
  .from{ font-weight:900; }
  .ts{ font-size:12px; color:var(--muted); white-space:nowrap; }
  .preview{ font-size:12px; color:var(--muted); margin-top:6px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .reader{
    border:1px solid rgba(15,17,22,.12);
    border-radius:14px; padding:10px;
    background:rgba(255,255,255,.86);
  }
  .reader h3{ margin:0 0 8px; font-size:14px; }
  .reader .meta{ font-size:12px; color:var(--muted); margin-bottom:10px; }
  ...
  .btn{
    ...
    background:linear-gradient(180deg, rgba(214,178,94,.22), rgba(214,178,94,.14));
  ...
  .hint{ font-size:12px; color:var(--muted); padding:8px 12px 12px; }
</style>
</head>
<body>
  <div class="wrap">
    <div class="card">
      <div class="hdr">
        ...
        <div style="display:flex;gap:8px">
          <button class="btn btnSoft" id="refreshBtn" type="button">רענן</button>
          <button class="btn btnSoft" id="closeBtn" type="button">סגור</button>
        </div>
      </div>

      <div class="tabs">
        <button class="tab active" id="tabInbox" type="button">תיבה נכנסת</button>
        <button class="tab" id="tabCompose" type="button">שלח הודעה</button>
      </div>
      ...
      <div class="grid">
        ...
        <div id="paneCompose" style="display:none">
          <div style="display:grid;gap:10px">
            <div>
              <label>אל</label>
            ...
            <div>
              ...
              <input id="subj" type="text" placeholder="לדוגמה: צריך שיקוף / חזרה ללקוח">
            </div>
            <div>
              <label>הודעה</label>
              <textarea id="body" placeholder="כתוב/י הודעה…"></textarea>
            </div>
            <div class="btnRow">
              <button class="btn btnSoft" id="clearBtn" type="button">נקה</button>
              <button class="btn" id="sendBtn" type="button">שלח</button>
            </div>
            <div class="hint">הודעות נשמרות מקומית בדפדפן. בהמשך אפשר לחבר לגוגל שיטס.</div>
          </div>
        </div>
      </div>
    </div>
...
<script>
(function(){
  const ME = ""; // placeholder replaced below
  const MAIL_LS_KEY = 'gemel_mail_v1';
  let mailWinRef_ = null;
  const USERS_LS_KEY = 'gemel_users_v1';
  ...
  function saveMail(arr){
    try{ localStorage.setItem(MAIL_LS_KEY, JSON.stringify(arr||[])); }catch(e){}
  }
  ...
  function inbox(){
    const me = meName();
    const all = loadMail();
    return all.filter(m => m && String(m.to||'') === me)
      .sort((a,b)=> (b.ts||0)-(a.ts||0));
  }

  function buildToList(){
    const me = meName();
    const users = loadUsers().filter(u => u && u.active !== false).map(u => String(u.displayName||u.username||'').trim()).filter(Boolean);
    const uniq = Array.from(new Set(users)).filter(n => n && n !== me);
    const options = ['<option value="">בחר נציג…</option>'].concat(uniq.map(n => '<option value="'+escapeAttr(n)+'">'+escapeHtml(n)+'</option>'));
    el('toSel').innerHTML = options.join('');
  }
  ...
  function renderInbox(){
    ...
    const list = inbox();
    if(!list.length){
      el('list').innerHTML = '<div class="hint">אין הודעות כרגע.</div>';
      return;
    }
    el('list').innerHTML = list.map(m => {
      const unread = !m.readAt;
      const prev = (m.body||'').slice(0,70);
      return '<div class="item '+(unread?'unread':'')+'" data-id="'+escapeAttr(m.id)+'">'+
        '<div class="row"><div class="from">'+escapeHtml(m.from||'')+'</div><div class="ts">'+escapeHtml(fmt(m.ts||0))+'</div></div>'+
        '<div class="preview">'+escapeHtml((m.subject?m.subject+' • ':'')+prev)+'</div>'+
      '</div>';
    }).join('');
    Array.from(document.querySelectorAll('.item[data-id]')).forEach(it => {
      it.addEventListener('click', () => openMail(it.getAttribute('data-id')));
    });
  }

  function openMail(id){
    const all = loadMail();
    const idx = all.findIndex(x => x && String(x.id||'') === String(id||''));
    if(idx < 0) return;
    const m = all[idx];
    ...
    el('reader').innerHTML =
      '<h3>'+escapeHtml(m.subject || 'ללא נושא')+'</h3>'+
      '<div class="meta">מאת: <b>'+escapeHtml(m.from||'')+'</b> • '+escapeHtml(fmt(m.ts||0))+'</div>'+
      '<div class="body">'+escapeHtml(m.body||'')+'</div>'+
      '<div class="btnRow" style="margin-top:10px">'+
        '<button class="btn btnSoft" id="backBtn" type="button">חזרה</button>'+
        '<button class="btn btnSoft" id="delBtn" type="button">מחק</button>'+
      '</div>';
    el('backBtn').addEventListener('click', () => { el('reader').style.display='none'; activeId=null; renderInbox(); });
    el('delBtn').addEventListener('click', () => {
      const again = loadMail().filter(x => x && String(x.id||'') !== String(activeId||''));
      saveMail(again);
      el('reader').style.display='none';
      activeId=null;
      renderInbox();
  ...
  function send(){
    const to = String(el('toSel').value||'').trim();
    const subject = String(el('subj').value||'').trim();
    const body = String(el('body').value||'').trim();
    if(!to){ alert('בחר נציג'); return; }
    if(!body){ alert('כתוב הודעה'); return; }
    const all = loadMail();
    const msg = { id: 'm_' + Math.random().toString(36).slice(2,10) + '_' + Date.now().toString(36),
      from: meName(), to, subject, body, ts: Date.now(), readAt: null };
    all.unshift(msg);
    saveMail(all);
    el('subj').value=''; el('body').value='';
    alert('נשלח');
    switchTab('inbox');
    renderInbox();
  }
  ...
  function switchTab(which){
    const inboxBtn = el('tabInbox');
    const compBtn = el('tabCompose');
    const paneInbox = el('paneInbox');
    const paneCompose = el('paneCompose');
    if(which === 'compose'){
      compBtn.classList.add('active'); inboxBtn.classList.remove('active');
      paneCompose.style.display=''; paneInbox.style.display='none';
    }else{
      inboxBtn.classList.add('active'); compBtn.classList.remove('active');
      paneInbox.style.display=''; paneCompose.style.display='none';
    }
  }

  el('meSub').textContent = 'מחובר: ' + meName();
  el('closeBtn').addEventListener('click', () => window.close());
  el('refreshBtn').addEventListener('click', () => { buildToList(); renderInbox(); });

  el('tabInbox').addEventListener('click', () => switchTab('inbox'));
  el('tabCompose').addEventListener('click', () => switchTab('compose'));
  el('sendBtn').addEventListener('click', send);
  el('clearBtn').addEventListener('click', () => { el('subj').value=''; el('body').value=''; });
  ...
  window.addEventListener('storage', (e) => {
    ...
    if(e.key === MAIL_LS_KEY){
      // Refresh inbox view live
      renderInbox();
    }
    if(e.key === USERS_LS_KEY){
      buildToList();
    }
  });
})();
    ...
    // Inject ME safely (avoid template confusion)
    const html2 = html.replace('const ME = \"\";', 'const ME = ' + JSON.stringify(me) + ';');

    mailWinRef_.document.open();
    mailWinRef_.document.write(html2);
    mailWinRef_.document.close();
  }

  function closeMail_(){
    try{ if(mailWinRef_ && !mailWinRef_.closed) mailWinRef_.close(); }catch(_){}
    mailWinRef_ = null;
    activeMailId_ = null;
  }
  function toggleMail_(){
    // open (or focus) the separate window
    if(mailWinRef_ && !mailWinRef_.closed){
      try{ mailWinRef_.focus(); }catch(_){}
      return;
    }
    openMailWindow_();
  ...
  function updateMailBadge_(){
    try{
      ...
      const n = unreadCountFor_(meName_());
      lastUnread_ = n;
      if(n > 0){
        mailBadgeEl.textContent = String(n);
        mailBadgeEl.style.display = 'inline-flex';
      }else{
        mailBadgeEl.style.display = 'none';
      }
    }catch(_){}
  }

  function escapeMini_(s){
    return escapeHtml(String(s||''));
  }

  function renderInbox_(){
    if(!mailListEl) return;
    const list = inboxFor_(meName_());
    if(mailReaderEl) mailReaderEl.style.display = 'none';
    mailListEl.style.display = '';
    if(!list.length){
      mailListEl.innerHTML = '<div class="sideHint">אין הודעות כרגע.</div>';
      return;
    }

    mailListEl.innerHTML = list.map(m => {
      const from = escapeMini_(m.from || '—');
      const subj = escapeMini_(m.subject || 'ללא נושא');
      const prev = escapeMini_(String(m.body||'').slice(0,90));
      const t = escapeMini_(fmtMailTime_(m.sentAt));
      const cls = m.read ? 'mailItem' : 'mailItem unread';
      return `
        <div class="${cls}" data-mail-open="${escapeAttr(m.id)}">
          <div class="mailItemTop">
            <div class="mailFrom">${from}</div>
            <div class="mailTime">${t}</div>
          </div>
          <div class="mailSubj">${subj}</div>
          <div class="mailPrev">${prev}${(String(m.body||'').length>90)?'…':''}</div>
        </div>
      `;
    }).join('');

    // wire open
    $$('#mailList [data-mail-open]').forEach(el => {
      el.addEventListener('click', () => openMailMessage_(el.dataset.mailOpen));
    });
  }

  function openMailMessage_(id){
    const all = loadMail_();
    const idx = all.findIndex(x => String(x.id||'') === String(id||''));
    if(idx < 0) return;

    const m = all[idx];
    if(String(m.to||'').trim() !== meName_()) return;
    ...
    if(mailListEl) mailListEl.style.display = 'none';
    if(mailReaderEl) mailReaderEl.style.display = 'flex';
    if(mailReaderMetaEl) mailReaderMetaEl.textContent = `מאת: ${m.from||'—'} • ${fmtMailTime_(m.sentAt)||''}`;
    if(mailReaderSubjEl) mailReaderSubjEl.textContent = m.subject || 'ללא נושא';
    if(mailReaderBodyEl) mailReaderBodyEl.textContent = m.body || '';
  ...
  function buildRecipients_(){
    if(!mailToEl) return;
    // pull active users from system
    const users = (typeof loadUsers_ === 'function' ? loadUsers_() : (typeof getUsers_ === 'function' ? getUsers_() : []))
      .filter(u => u && u.active !== false)
      .map(u => String(u.displayName || u.username || '').trim())
      .filter(Boolean);

    const me = meName_();
    const options = users.filter(n => n && n !== me);
    mailToEl.innerHTML = ['<option value="">בחר נציג…</option>', ...options.map(n => `<option value="${escapeAttr(n)}">${escapeMini_(n)}</option>`)].join('');
  }

  function clearCompose_(){
    if(mailToEl) mailToEl.value = '';
    if(mailSubjectEl) mailSubjectEl.value = '';
    if(mailBodyEl) mailBodyEl.value = '';
  }

  function sendMail_(){
    const from = meName_();
    const to = String(mailToEl?.value || '').trim();
    const subject = String(mailSubjectEl?.value || '').trim();
    const body = String(mailBodyEl?.value || '').trim();

    if(!to){
      toast('חסר', 'בחר נציג יעד');
      return;
    }
    if(!body){
      toast('חסר', 'כתוב/י הודעה');
      return;
    }
    ...
    clearCompose_();
    toast('נשלח', 'הודעה נשלחה בהצלחה');
    setMailTab_('inbox');
    // no sound for sender
  }

  function initInternalMail_(){
    if(mailUIWired_) return;
    mailUIWired_ = true;
    ...
    // button
    if(mailBtnTop){
      mailBtnTop.addEventListener('click', toggleMail_);
    }
    if(mailCloseBtn) mailCloseBtn.addEventListener('click', closeMail_);
    if(mailMinBtn) mailMinBtn.addEventListener('click', () => mailFloatEl?.classList.toggle('min'));

    if(mailTabInbox) mailTabInbox.addEventListener('click', () => setMailTab_('inbox'));
    if(mailTabCompose) mailTabCompose.addEventListener('click', () => setMailTab_('compose'));

    if(mailBackBtn) mailBackBtn.addEventListener('click', () => { renderInbox_(); if(mailReaderEl) mailReaderEl.style.display='none'; if(mailListEl) mailListEl.style.display=''; });

    if(mailSendBtn) mailSendBtn.addEventListener('click', sendMail_);
    if(mailClearBtn) mailClearBtn.addEventListener('click', clearCompose_);

    // Close when pressing Esc (only if open)
    window.addEventListener('keydown', (e) => {
      if(e.key === 'Escape'){
        if(mailFloatEl && mailFloatEl.style.display !== 'none') closeMail_();
      }
    });

    // Listen for cross-tab updates (demo "receiver" simulation)
    window.addEventListener('storage', (e) => {
      try{
        if(e.key !== MAIL_LS_KEY) return;
        if(!state.currentUser) return;
        const nowUnread = unreadCountFor_(meName_());
        if(nowUnread > lastUnread_){
          // new mail arrived
          playMailSound_();
          toast('מייל חדש', 'נכנסה הודעה חדשה');
        }
        lastUnread_ = nowUnread;
        updateMailBadge_();
        // refresh inbox if open and on inbox tab
        if(mailFloatEl && mailFloatEl.style.display !== 'none' && mailTabInbox?.classList.contains('active')){
          renderInbox_();
        }
      }catch(_){}
    });
