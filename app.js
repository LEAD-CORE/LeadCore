/* LEAD CORE • Premium CRM • build 2026-02-02_fix2
...
(() => {
  ...
  // חשוב: זו כתובת ה-Web App שמסתיימת ב-/exec
  // ===========================
  ...
  // ---------------------------
  const Storage = {
    // ברירת מחדל: Sheets (כדי שמחשב חדש תמיד יטען מהשיטס)
    ...
    async loadSheets() {
      ...
      try{
        ...
        if (!json || json.ok !== true) {
          return { ok: false, error: (json && json.error) ? String(json.error) : "שגיאת get" };
        }
    ...
    async saveSheets(state) {
      ...
      try{
        ...
        const json = await res.json().catch(() => null);
        if (!json || json.ok !== true) return { ok: false, error: (json && json.error) ? String(json.error) : "שגיאת put" };
...
on(this.els.btnSearch, "click", () => {
  ...
  const App = {
    async ensureConnection(){
      // בדיקת חיבור רכה: לא מפילה את המערכת
    ...
    async boot() {
      ...
      UI.renderAll();
      UI.goView("dashboard");
      // בדיקת חיבור אוטומטית (לא חוסם)
      await App.ensureConnection();
    },
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      // לא מציגים חלון קופץ למשתמש (רק לקונסול)
      // אפשר להוסיף חיווי קטן בעתיד אם תרצה.
