...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // ---------- INTERNAL MAIL (LocalStorage; ready to move to Sheets later) ----------
  const MAIL_LS_KEY = 'gemel_mail_v1';
  const MAIL_LAST_SEEN_PREFIX = 'gemel_mail_last_seen_'; // per username (iso)

  let mailUI = { mounted:false, tab:'inbox', open:false, selectedId:null };
  let mailPollTimer = null;
  let mailLastNotifiedAt = 0;
  ...
  function mailUid_(){ return 'm_' + Math.random().toString(36).slice(2,10) + '_' + Date.now().toString(36); }

  function meUsername_(){ return String((state.currentUser && state.currentUser.username) || '').trim(); }
  function meName_(){ return String(state.agentName || '').trim(); }

  function userDisplay_(username){
    const u = (typeof findUserByUsername_ === 'function') ? findUserByUsername_(username) : null;
    return u ? (u.displayName || u.username) : String(username||'');
  }

  function isMailReadBy_(mail, username){
    const u = String(username||'').trim();
    if(!u) return true;
    const rb = mail && mail.readBy ? mail.readBy : {};
    return !!rb[u];
  }
  function markMailRead_(id, username){
    const u = String(username||'').trim();
    if(!id || !u) return;
    const list = loadMail_();
    const idx = list.findIndex(x => x && x.id === id);
    if(idx < 0) return;
    const m = list[idx];
    const rb = (m.readBy && typeof m.readBy === 'object') ? m.readBy : {};
    if(!rb[u]){
      rb[u] = true;
      list[idx] = { ...m, readBy: rb };
      saveMail_(list);
    }
  }

  function unreadCount_(username){
    const u = String(username||'').trim();
    if(!u) return 0;
    const list = loadMail_();
    return list.filter(m => m && m.toUsername === u && !isMailReadBy_(m, u)).length;
  }

  function setMailLastSeen_(username, iso){
    const u = String(username||'').trim();
    if(!u) return;
    try{ localStorage.setItem(MAIL_LAST_SEEN_PREFIX + u, String(iso||'')); }catch(_){}
  }
  function getMailLastSeen_(username){
    const u = String(username||'').trim();
    if(!u) return '';
    try{ return String(localStorage.getItem(MAIL_LAST_SEEN_PREFIX + u) || ''); }catch(_){ return ''; }
  }
  ...
  function mountMailWindow_(){
    if(mailUI.mounted) return;
    const root = document.createElement('div');
    root.id = 'mailWindow';
    root.className = 'mailWindow';
    root.innerHTML = `
      <div class="mailHead">
        <div class="mailTitle">
          <span aria-hidden="true" style="width:26px;height:26px;display:inline-flex;align-items:center;justify-content:center">
          ...
          <span>דואר פנימי</span>
        </div>
        <button class="mailClose" type="button" data-mail-close>סגור</button>
      </div>
      <div class="mailTabs">
        <button class="mailTab active" type="button" data-mail-tab="inbox">דואר נכנס</button>
        <button class="mailTab" type="button" data-mail-tab="compose">שליחת הודעה</button>
      </div>
      <div class="mailBody" id="mailBody"></div>
    `;
    document.body.appendChild(root);

    root.querySelector('[data-mail-close]')?.addEventListener('click', () => closeMail_());
    root.querySelectorAll('[data-mail-tab]').forEach(b => {
      b.addEventListener('click', () => {
        mailUI.tab = b.dataset.mailTab || 'inbox';
  ...
  function openMail_(){
    mountMailWindow_();
    const w = document.getElementById('mailWindow');
    if(!w) return;
    w.classList.add('show');
    mailUI.open = true;
    updateMailBadge_();
    setMailLastSeen_(meUsername_(), nowIso_());
    renderMail_();
  }
  function closeMail_(){
    const w = document.getElementById('mailWindow');
  ...
  function toggleMail_(){
    if(!mailUI.open) openMail_();
    else closeMail_();
  }
  ...
  function renderInbox_(){
    const u = meUsername_();
    const list = loadMail_()
      .filter(m => m && m.toUsername === u)
      .slice()
      .sort((a,b) => new Date(b.sentAt||0).getTime() - new Date(a.sentAt||0).getTime());

    if(!list.length){
      return `<div class="mailEmpty">אין הודעות כרגע.</div>`;
    }

    return `
      <div class="mailList">
        ${list.map(m => {
          const unread = !isMailReadBy_(m, u);
          const fromName = escapeHtml(m.fromName || userDisplay_(m.fromUsername) || m.fromUsername || '');
          const subj = escapeHtml(m.subject || '(ללא נושא)');
          const bodyPrev = escapeHtml(String(m.body||'').trim().slice(0,90));
          const t = escapeHtml(fmtMailTime_(m.sentAt));
          return `
            <div class="mailItem ${unread?'unread':''}" data-mail-open="${escapeAttr(m.id)}">
              <div class="mailSubj">${subj}</div>
              <div class="mailMeta">
                <span>מאת: <b>${fromName}</b></span>
                <span>${t}</span>
              </div>
              <div style="margin-top:6px;font-size:12.5px;color:rgba(15,17,22,.68)">${bodyPrev}${String(m.body||'').length>90?'…':''}</div>
            </div>
          `;
        }).join('')}
      </div>
  ...
  function renderMailView_(id){
    const u = meUsername_();
    const list = loadMail_();
    const m = list.find(x => x && x.id === id);
    if(!m){
      return `<div class="mailEmpty">הודעה לא נמצאה.</div>`;
    }
    ...
    const subj = escapeHtml(m.subject || '(ללא נושא)');
    const t = escapeHtml(fmtMailTime_(m.sentAt));
    const body = escapeHtml(String(m.body||'').trim());

    return `
      <div style="display:flex;justify-content:space-between;gap:10px;margin-bottom:10px">
        <button class="btn btnSoft" type="button" data-mail-back>חזרה</button>
        <button class="btn btnSoft" type="button" data-mail-reply="${escapeAttr(m.fromUsername||'')}">השב</button>
      </div>
      <div class="panel" style="margin:0">
        <div style="font-weight:900;font-size:15px;margin-bottom:6px">${subj}</div>
        <div style="font-size:12.5px;color:rgba(15,17,22,.65);margin-bottom:8px">מאת: <b>${fromName}</b> • ${t}</div>
        <div style="white-space:pre-wrap;line-height:1.45">${body || '—'}</div>
      </div>
    `;
  }

  function renderCompose_(prefillTo){
    const meU = meUsername_();
    const users = getUsers_().filter(x => x && x.active !== false && x.username && x.username !== meU);

    return `
      <form class="mailForm" id="mailComposeForm">
        <div>
          <div class="label" style="margin-bottom:6px">אל</div>
          <select class="input" id="mailTo" required>
            <option value="">בחר נציג…</option>
            ${users.map(u => `<option value="${escapeAttr(u.username)}"${prefillTo===u.username?' selected':''}>${escapeHtml(u.displayName || u.username)} (${escapeHtml(u.username)})</option>`).join('')}
          </select>
        </div>
        <div>
          <div class="label" style="margin-bottom:6px">נושא</div>
          <input class="input" id="mailSubj" type="text" placeholder="נושא (אופציונלי)">
        </div>
        <div>
          <div class="label" style="margin-bottom:6px">הודעה</div>
          <textarea class="input" id="mailBodyTxt" style="min-height:140px;resize:vertical" placeholder="כתוב כאן את ההודעה…" required></textarea>
        </div>
        <div class="mailActions">
          <button class="btn btnSoft" type="button" data-mail-clear>נקה</button>
          <button class="btn" type="submit">שלח</button>
        </div>
      </form>
      <div style="margin-top:10px;font-size:12px;color:rgba(15,17,22,.60)">
        טיפ: אפשר להשאיר את חלון המייל פתוח ולהמשיך לעבוד במערכת.
      </div>
    `;
  }

  function sendMail_(toUsername, subject, body){
    const fromUsername = meUsername_();
    const fromName = meName_() || userDisplay_(fromUsername);
    const toU = String(toUsername||'').trim();
    if(!fromUsername || !toU) return { ok:false, msg:'חסר נמען' };
    if(!String(body||'').trim()) return { ok:false, msg:'חסרה הודעה' };
  ...
  function renderMail_(){
    ...
    w.querySelectorAll('[data-mail-tab]').forEach(b => {
      b.classList.toggle('active', (b.dataset.mailTab||'') === mailUI.tab);
    });

    if(mailUI.tab === 'compose'){
      body.innerHTML = renderCompose_();
      const form = document.getElementById('mailComposeForm');
      const toEl = document.getElementById('mailTo');
      const subjEl = document.getElementById('mailSubj');
      const txtEl = document.getElementById('mailBodyTxt');

      body.querySelector('[data-mail-clear]')?.addEventListener('click', () => {
        if(toEl) toEl.value = '';
      ...
      if(form){
        form.addEventListener('submit', (e) => {
          e.preventDefault();
          const toU = String(toEl?.value || '').trim();
          const subj = String(subjEl?.value || '').trim();
          const txt  = String(txtEl?.value || '').trim();
          const res = sendMail_(toU, subj, txt);
          if(!res.ok){
            toast('שגיאה', res.msg || 'לא נשלח');
            return;
          }
    ...
    if(mailUI.selectedId){
      body.innerHTML = renderMailView_(mailUI.selectedId);
      ...
      body.querySelector('[data-mail-reply]')?.addEventListener('click', (e) => {
        const toU = String(e.currentTarget?.dataset?.mailReply || '').trim();
        mailUI.tab = 'compose';
        mailUI.selectedId = null;
        renderMail_();
        const toEl = document.getElementById('mailTo');
        if(toEl && toU) toEl.value = toU;
      });
      return;
    }

    body.innerHTML = renderInbox_();
    body.querySelectorAll('[data-mail-open]').forEach(el => {
      el.addEventListener('click', () => {
        mailUI.selectedId = el.dataset.mailOpen;
        renderMail_();
      });
    });
  }

  function checkIncomingMail_(){
    const u = meUsername_();
    if(!u) return;

    updateMailBadge_();

    const lastSeen = getMailLastSeen_(u);
    const lastSeenT = lastSeen ? new Date(lastSeen).getTime() : 0;

    const list = loadMail_();
    const newestUnread = list.find(m => m && m.toUsername === u && !isMailReadBy_(m, u));
    if(!newestUnread) return;

    const t = new Date(newestUnread.sentAt||0).getTime();
    if(!isNaN(t) && t > lastSeenT && t > mailLastNotifiedAt){
      mailLastNotifiedAt = t;
      playMailSound_();
      const from = newestUnread.fromName || userDisplay_(newestUnread.fromUsername) || '';
      toast('דואר נכנס', 'התקבלה הודעה מ־' + from);
      setMailLastSeen_(u, newestUnread.sentAt || nowIso_());
    }
  }

  function startMailPolling_(){
    stopMailPolling_();
    mailLastNotifiedAt = 0;
    updateMailBadge_();
    mailPollTimer = setInterval(checkIncomingMail_, 1400);
  }
  ...
  function wireMailButton_(){
    const btn = document.getElementById('mailBtn');
    if(!btn || btn.dataset.wired === '1') return;
    btn.dataset.wired = '1';
    btn.addEventListener('click', () => openMailPopup_());
  }

  // Open mail as a separate small window (not a floating panel inside the CRM)
  let mailPopupWin = null;

  function openMailPopup_(){
    // Save current session for the popup (same-origin localStorage)
    try{
      localStorage.setItem('gemel_mail_session_v1', JSON.stringify({
        username: meUsername_(),
        name: meName_(),
        openedAt: nowIso_()
      }));
    }catch(_){}
    ...
    if(mailPopupWin && !mailPopupWin.closed){
      try{ mailPopupWin.focus(); }catch(_){}
      return;
    }
    ...
    mailPopupWin = window.open('', 'gemel_mail_popup', features);

    if(!mailPopupWin){
      toast('חסום', 'הדפדפן חסם חלון קופץ. אפשר לאשר Popups לאתר ואז ללחוץ שוב על המעטפה.');
      return;
    }

    try{
      mailPopupWin.document.open();
      mailPopupWin.document.write(buildMailPopupHtml_());
      mailPopupWin.document.close();
      mailPopupWin.focus();
    }catch(e){
      try{ mailPopupWin.close(); }catch(_){}
      mailPopupWin = null;
      toast('שגיאה', 'לא הצלחתי לפתוח חלון מייל. נסה שוב.');
      return;
    }

    // Mark as seen now (so notifications won't repeat immediately)
    setMailLastSeen_(meUsername_(), nowIso_());
    updateMailBadge_();
  }

  function buildMailPopupHtml_(){
    // IMPORTANT: keep it self-contained (no external files) so it always opens cleanly.
    const safeTitle = 'GEMEL INVEST • דואר פנימי';
    return `<!doctype html>
...
<style>
  ...
  .wrap{height:100%; display:flex; flex-direction:column; padding:12px; gap:10px;}
  .card{
    background: linear-gradient(180deg, rgba(255,255,255,.86), rgba(246,242,234,.76));
  ...
  .btn{
    ...
    background: linear-gradient(180deg, rgba(255,255,255,.86), rgba(246,242,234,.78));
    padding:10px 12px;
    border-radius:14px;
    font-weight:900;
    cursor:pointer;
  ...
  .item.unread{border-color: rgba(214,178,94,.38); box-shadow: 0 0 0 3px rgba(214,178,94,.10);}
  .meta{display:flex; justify-content:space-between; gap:10px; color:var(--muted); font-size:12px; margin-top:4px;}
  .subj{font-weight:900;}
  .empty{color:var(--muted); font-weight:700; padding:10px 0;}
  .toast{
    position:fixed;
    left:12px;
    bottom:12px;
  ...
  <div class="wrap">
    <div class="card">
      <div class="head">
        ...
        <div class="pill"><span id="meName">—</span><span id="badge" style="display:none">• <span id="badgeN">0</span></span></div>
      </div>
      <div class="tabs">
        ...
        <button class="tab" data-tab="compose" type="button">שליחה</button>
      </div>
    </div>
...
(() => {
  ...
  const MAIL_LS_KEY = 'gemel_mail_v1';
  const USERS_LS_KEY = 'gemel_users_v1';
  const LAST_SEEN_PREFIX = 'gemel_mail_last_seen_';
  const SESSION_KEY = 'gemel_mail_session_v1';
  ...
  function loadSession(){
    const s = safeJsonParse(localStorage.getItem(SESSION_KEY) || '');
    return s && s.username ? s : { username:'', name:'' };
  }
  function loadUsers(){
    const arr = safeJsonParse(localStorage.getItem(USERS_LS_KEY) || '');
    return Array.isArray(arr) ? arr : [];
  }
  function loadMail(){
    const arr = safeJsonParse(localStorage.getItem(MAIL_LS_KEY) || '');
    return Array.isArray(arr) ? arr : [];
  }
  function saveMail(arr){
    localStorage.setItem(MAIL_LS_KEY, JSON.stringify(arr || []));
  }
  function setLastSeen(u, iso){
    if(!u) return;
    localStorage.setItem(LAST_SEEN_PREFIX + u, String(iso||''));
  }
  function nowIso(){ return new Date().toISOString(); }
  function uid(){ return 'm_' + Math.random().toString(36).slice(2,10) + '_' + Date.now().toString(36); }

  function esc(s){ return String(s||'').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
  function fmt(iso){
    try{ const d=new Date(iso); return isNaN(d.getTime())?'':d.toLocaleString('he-IL'); }catch(_){ return ''; }
  }
  ...
  let state = { tab:'inbox', selectedId:null };
  const session = loadSession();
  const meU = String(session.username||'').trim();
  const meN = String(session.name||'').trim() || meU || 'אורח';
  $('#meName').textContent = meN;

  function isRead(m){
    const rb = (m && m.readBy && typeof m.readBy === 'object') ? m.readBy : {};
    return !!rb[meU];
  }
  ...
  function unreadCount(){
    if(!meU) return 0;
    const list = loadMail();
    return list.filter(m => m && m.toUsername === meU && !isRead(m)).length;
  }
  function syncBadge(){
    const n = unreadCount();
    const b = $('#badge');
    ...
    document.title = n > 0 ? ('(' + n + ') דואר פנימי • GEMEL INVEST') : ('דואר פנימי • GEMEL INVEST');
  }
  ...
  function render(){
    ...
    const listAll = loadMail().slice().sort((a,b)=>new Date(b.sentAt||0)-new Date(a.sentAt||0));

    if(state.tab === 'compose'){
      const users = loadUsers().filter(u => u && u.active !== false && String(u.username||'').trim() && String(u.username||'').trim() !== meU);
      view.innerHTML = \`
        <div style="display:grid; gap:10px">
          <div class="row" style="gap:8px">
            <select id="toUser" class="input">
              <option value="">בחר נציג…</option>
              \${users.map(u => \`<option value="\${esc(u.username)}">\${esc(u.displayName||u.username)}</option>\`).join('')}
            </select>
          </div>
          <input id="subj" class="input" placeholder="נושא (אופציונלי)"/>
          <textarea id="body" class="input" placeholder="כתוב הודעה…"></textarea>
          <div class="row" style="justify-content:flex-end">
            <button class="btn primary" id="sendBtn" type="button">שלח</button>
          </div>
          <div style="color:var(--muted); font-size:12px; font-weight:700">הודעות נשמרות בדמו בלוקאל (על אותו מחשב/דפדפן). בחיבור לשרת זה יעבוד בין מחשבים.</div>
        </div>
      \`;
      $('#sendBtn')?.addEventListener('click', () => {
        const to = String($('#toUser')?.value||'').trim();
        const subject = String($('#subj')?.value||'').trim();
        const body = String($('#body')?.value||'').trim();
        if(!to){ toast('בחר נציג'); return; }
        if(!body){ toast('כתוב הודעה'); return; }
    ...
    const myList = state.tab === 'sent'
      ? listAll.filter(m => m && m.fromUsername === meU)
      : listAll.filter(m => m && m.toUsername === meU);

    if(state.selectedId){
      const msg = myList.find(m => m && m.id === state.selectedId) || listAll.find(m => m && m.id === state.selectedId);
      if(!msg){
        state.selectedId = null;
        render();
        return;
      }
      if(state.tab === 'inbox'){ markRead(msg.id); setLastSeen(meU, nowIso()); }

      view.innerHTML = \`
        <div style="display:flex; justify-content:space-between; gap:10px; align-items:flex-start; margin-bottom:10px">
          <div>
            <div class="subj">\${esc(msg.subject||'הודעה')}</div>
            <div class="meta">
              <span>\${state.tab==='sent' ? 'אל: ' + esc(msg.toUsername||'') : 'מאת: ' + esc(msg.fromName||msg.fromUsername||'')}</span>
              <span>\${esc(fmt(msg.sentAt||''))}</span>
            </div>
          </div>
          <button class="btn" id="backBtn" type="button">חזרה</button>
        </div>
        <div class="card" style="border-radius:16px; box-shadow:none; border:1px solid rgba(15,15,20,.10)">
          <div class="body" style="white-space:pre-wrap; line-height:1.55">\${esc(msg.body||'')}</div>
        </div>
      \`;
    ...
    if(!myList.length){
      view.innerHTML = \`<div class="empty">אין הודעות כרגע.</div>\`;
      return;
    }
    ...
    view.innerHTML = \`
      <div class="list">
        \${myList.map(m => {
          const unread = (state.tab==='inbox') && !isRead(m);
          const who = state.tab==='sent' ? ('אל: ' + (m.toUsername||'')) : ('מאת: ' + (m.fromName||m.fromUsername||''));
          return \`
            <div class="item \${unread?'unread':''}" data-open="\${esc(m.id)}">
              <div class="subj">\${esc(m.subject||'הודעה פנימית')}</div>
              <div class="meta">
                <span>\${esc(who)}</span>
                <span>\${esc(fmt(m.sentAt||''))}</span>
              </div>
            </div>
          \`;
        }).join('')}
    ...
    $$('[data-open]').forEach(el => {
      el.addEventListener('click', () => {
        state.selectedId = el.getAttribute('data-open') || null;
        render();
      });
    });
  ...
  // react to new mail from other windows
  window.addEventListener('storage', (e) => {
    if(e.key === MAIL_LS_KEY){
      render();
    }
  });
})();
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  function setIdentity_(u){
    ...
    applyPermissions_();
      try{ wireMailButton_(); startMailPolling_(); updateMailBadge_(); }catch(_){ }
  }
  ...
  window.gemelLogout = function(){
    try{ stopMailPolling_(); closeMail_(); }catch(_){ }
    clearSession_();
