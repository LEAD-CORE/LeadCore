...
(() => {
  ...
  // Build marker (used to verify you are on the latest app.js)
  const LEADCORE_BUILD = "2026-02-02_fix_nopopup_01";
// ===========================
...
on(this.els.btnSearch, "click", () => {
  ...
  const App = {
    async ensureConnection(){
      // בדיקת חיבור רכה: לא מפילה את המערכת, רק מעדכנת חיווי/לוג.
      if (typeof Storage?.ensureConnection === "function") {
        return await Storage.ensureConnection();
      }
      return { ok: true };
    },
