...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  function openMail_(){
    if(!mailFloatEl) return;
    mailFloatEl.style.display = 'flex';
    mailFloatEl.classList.remove('min');
    if(mailFloatSub) mailFloatSub.textContent = meName_();
    setMailTab_('inbox');
    updateMailBadge_();
  }
  function closeMail_(){
    if(!mailFloatEl) return;
    mailFloatEl.style.display = 'none';
    activeMailId_ = null;
  }
  function toggleMail_(){
    if(!mailFloatEl) return;
    const shown = mailFloatEl.style.display !== 'none';
    if(shown) closeMail_(); else openMail_();
  }
