...
const App = {
  ...
  // === Move connected-user pill into sidebar (scoped change only) ===
  const moveUserPillToSidebar = () => {
    try{
      const pill = document.getElementById("lcUserPill");
      const slot = document.getElementById("sidebarUserSlot");
      if (!pill || !slot) return false;
      if (pill.dataset && pill.dataset.movedToSidebar === "1") return true;

      slot.appendChild(pill);
      pill.dataset.movedToSidebar = "1";

      // cancel any inline hotfix positioning if exists
  ...
  // attempt on boot
  setTimeout(() => { moveUserPillToSidebar(); }, 0);
