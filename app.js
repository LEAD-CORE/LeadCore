...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  window.gemelLogout = function(){
    ...
    // ---------- INTERNAL MAIL (Separate Window) ----------
  // LocalStorage-based internal mail (demo). Opens in a separate window (not inside the app UI).
  // Designed to be swapped to server/Sheets later.
  ...
  // Clear, audible beep (works well in PWA after a user gesture)
  function playMailSound_(){
    try{
      ...
      g.gain.linearRampToValueAtTime(0.0, now + 0.28);
      o.stop(now + 0.30);
      setTimeout(()=>{ try{ ctx.close(); }catch(_){} }, 450);
    }catch(_){}
  }
  function unlockMailAudio_(){
    // Call on a click/gesture so future sounds are allowed in some browsers
    try{
      const Ctx = window.AudioContext || window.webkitAudioContext;
      if(!Ctx) return;
      const ctx = new Ctx();
  ...
  function mailWindowHtml_(){
    // NOTE: keep this self-contained (no external files), so it works as a separate window.
    return `<!doctype html>
...
<style>
  :root{
    ...
    --text:#0F1116;
    --muted: rgba(15,17,22,.66);
    --gold: rgba(214,178,94,.75);
    --shadow: 0 18px 45px rgba(18,19,25,.14);
    --radius: 18px;
  ...
  *{box-sizing:border-box}
  body{
    ...
    background: radial-gradient(1000px 500px at 30% 0%, rgba(255,255,255,.85) 0%, var(--bg) 55%, #EFE9DD 100%);
    color:var(--text);
    height:100vh;
    overflow:hidden;
  }
  .wrap{height:100vh; display:flex; align-items:stretch; justify-content:center; padding:14px}
  .card{
    width:min(740px, 96vw);
    height: calc(100vh - 28px);
    border-radius: 22px;
    background: linear-gradient(180deg, rgba(255,255,255,.66), rgba(246,242,234,.52));
  ...
  .meBox{display:flex; flex-direction:column}
  ...
  .btn:hover{box-shadow:0 0 0 3px rgba(214,178,94,.10)}
  ...
  .tab.active{background: rgba(255,255,255,.82); box-shadow:0 10px 22px rgba(18,19,25,.08)}
  .body{flex:1; min-height:0; display:grid; grid-template-columns: 310px 1fr}
  .list{
  ...
  .item.unread{border-color: rgba(214,178,94,.30); background: rgba(255,255,255,.74)}
  .row{display:flex; align-items:center; justify-content:space-between; gap:10px}
  .from{font-weight:900; font-size:13px}
  .time{font-size:11px; color:var(--muted); white-space:nowrap}
  .subject{font-size:12.5px; color:rgba(15,17,22,.78)}
  .reader{
    min-height:0;
    overflow:auto;
    padding:12px 14px;
  }
  .empty{color:var(--muted); font-weight:700; padding:18px}
  .hr{height:1px;background:var(--line); margin:10px 0}
  .h2{font-weight:900; font-size:14px}
  .meta{font-size:12px; color:var(--muted); margin-top:4px}
  .msg{white-space:pre-wrap; line-height:1.5; margin-top:10px}
  .compose{
    padding:12px 14px;
    overflow:auto;
  }
  label{font-size:12px; color:var(--muted); font-weight:800; display:block; margin:10px 0 6px}
  ...
  .sendRow{display:flex; justify-content:flex-end; gap:10px; margin-top:12px}
  .pill{
  ...
  @media (max-width: 720px){
    ...
    .list{border-left:none; border-bottom:1px solid var(--line)}
  }
</style>
</head>
<body>
  ...
  <div class="card">
    <div class="top">
      <div class="meBox">
        ...
        <div class="sub">מחובר/ת: <span id="meName">—</span> <span class="pill" id="unreadPill" style="display:none">0 לא נקראו</span></div>
      </div>
      <div class="actions">
        <button class="btn" id="focusAppBtn" type="button">חזרה למערכת</button>
    ...
    <div class="tabs">
      ...
      <button class="tab" id="tabCompose" type="button">כתיבת הודעה</button>
    </div>

    <div class="body" id="paneInbox">
      <div class="list" id="mailList"></div>
      <div class="reader" id="reader">
        <div class="empty">בחר הודעה מהרשימה.</div>
      </div>
    </div>

    <div class="compose" id="paneCompose" style="display:none">
      <label>נמען</label>
      <select id="toSel"></select>

      <label>נושא</label>
      <input id="subj" type="text" placeholder="נושא קצר" />

      <label>תוכן</label>
      <textarea id="bodyTxt" placeholder="כתוב/י הודעה..."></textarea>
...
(function(){
  ...
  function loadUsers(){
    try{
      ...
      return Array.isArray(arr) ? arr.filter(u => u && u.active !== false).map(u => String(u.displayName||u.username||'').trim()).filter(Boolean) : [];
    }catch(_){ return []; }
  }
  function loadMail(){
    try{
  ...
  function inboxFor(me){
    const who = String(me||'').trim();
    return loadMail().filter(m => String(m.to||'').trim() === who)
      .sort((a,b)=> new Date(b.sentAt||0).getTime() - new Date(a.sentAt||0).getTime());
  }
  function unreadCount(me){
    return inboxFor(me).filter(m => !m.read).length;
  }
  function playSound(){
    try{
      const Ctx = window.AudioContext || window.webkitAudioContext;
  ...
  function buildRecipients(){
    const sel = $('toSel');
    if(!sel) return;
    const users = loadUsers().filter(n => n && n !== ME);
    sel.innerHTML = '<option value="">בחר נמען…</option>' + users.map(n => '<option value="'+escapeHtml(n)+'">'+escapeHtml(n)+'</option>').join('');
  }

  function renderInbox(resetReader){
    const list = $('mailList');
    const reader = $('reader');
    if(!list || !reader) return;

    if(!ME){
      list.innerHTML = '<div class="empty">לא נמצא משתמש מחובר. חזור/י למערכת והתחבר/י.</div>';
      reader.innerHTML = '<div class="empty">—</div>';
      return;
    }

    const inbox = inboxFor(ME);
    const u = unreadCount(ME);
    const pill = $('unreadPill');
    if(pill){
      if(u>0){ pill.style.display='inline-flex'; pill.textContent = u + ' לא נקראו'; }
      else pill.style.display='none';
    }

    if(!inbox.length){
      list.innerHTML = '<div class="empty">אין הודעות בתיבה.</div>';
      if(resetReader) reader.innerHTML = '<div class="empty">בחר הודעה מהרשימה.</div>';
      return;
    }

    list.innerHTML = inbox.map(m => {
      const cls = 'item' + (!m.read ? ' unread' : '');
      const subj = m.subject ? m.subject : '(ללא נושא)';
      return '<div class="'+cls+'" data-id="'+escapeHtml(m.id)+'">' +
        '<div class="row"><div class="from">'+escapeHtml(m.from||'—')+'</div><div class="time">'+escapeHtml(fmtTime(m.sentAt))+'</div></div>' +
    ...
    if(resetReader && inbox[0]) openMsg(inbox[0].id);
  }

  function openMsg(id){
    if(!ME) return;
    const all = loadMail();
    const msg = all.find(x => String(x.id) === String(id));
    if(!msg) return;
    ...
    if(!reader) return;
    reader.innerHTML =
      '<div class="h2">'+escapeHtml(msg.subject || '(ללא נושא)')+'</div>' +
      '<div class="meta">מאת: <b>'+escapeHtml(msg.from||'—')+'</b> • '+escapeHtml(fmtTime(msg.sentAt))+'</div>' +
      '<div class="hr"></div>' +
      '<div class="msg">'+escapeHtml(msg.body||'')+'</div>';

    renderInbox(false);
  }

  function send(){
    if(!ME){ alert('אין משתמש מחובר'); return; }
    const to = String($('toSel')?.value||'').trim();
    const subject = String($('subj')?.value||'').trim();
    const body = String($('bodyTxt')?.value||'').trim();

    if(!to){ alert('בחר נמען'); return; }
    if(!body){ alert('כתוב תוכן הודעה'); return; }
  ...
  function setTab(which){
    const inbox = $('paneInbox');
    const comp = $('paneCompose');
    $('tabInbox').classList.toggle('active', which==='inbox');
    $('tabCompose').classList.toggle('active', which!=='inbox');
    if(inbox) inbox.style.display = (which==='inbox') ? '' : 'none';
    if(comp) comp.style.display = (which==='inbox') ? 'none' : '';
    if(which==='inbox') renderInbox(true);
    if(which!=='inbox') buildRecipients();
  }
  ...
  window.addEventListener('message', (ev)=>{
    const d = ev.data || {};
    if(d && d.type === 'gemel-mail-init'){
      setMe(d.me || '');
      // after user opened this window, sounds are typically allowed
      // playSound();
    }
  });

  window.addEventListener('storage', (ev)=>{
    if(ev.key === MAIL_LS_KEY){
      // if a new mail arrived for me and window is open, you can also play sound here:
      try{
        if(ME){
          const u = unreadCount(ME);
          // simple: play sound only when unread increases
          if(typeof window.__lastUnread === 'number' && u > window.__lastUnread) playSound();
          window.__lastUnread = u;
        }
      }catch(_){}
      renderInbox(false);
    }
    if(ev.key === USERS_LS_KEY){
      buildRecipients();
    }
  });

  // Try to init if opener exists (fallback)
  setTimeout(()=>{
    try{
      if(window.opener && !window.opener.closed){
        window.opener.postMessage({ type:'gemel-mail-need-init' }, '*');
      }
    }catch(_){}
  }, 120);
  ...
  function openMailWindow_(){
    try{
      ...
      if(mailWinRef_ && !mailWinRef_.closed){
        ...
        const features = 'width=760,height=820,resizable=yes,scrollbars=yes';
        mailWinRef_ = window.open('', 'gemelMailWindow', features);
        if(!mailWinRef_) {
          toast('חסום', 'הדפדפן חסם חלונות קופצים. אפשר לאפשר Popups לאתר ואז לנסות שוב.');
          return;
        }
        mailWinRef_.document.open();
        mailWinRef_.document.write(mailWindowHtml_());
        mailWinRef_.document.close();
      }

      // init handshake
      try{
        mailWinRef_.postMessage({ type:'gemel-mail-init', me: meName_() }, '*');
      }catch(_){}
    }catch(_){}
  }
  ...
  function closeMail_(){
    try{
      if(mailWinRef_ && !mailWinRef_.closed) mailWinRef_.close();
    }catch(_){}
    mailWinRef_ = null;
  }
  ...
  function wireMail_Once_(){
    try{
      if(mailBtnTop){
        mailBtnTop.addEventListener('click', () => {
          unlockMailAudio_();
          openMailWindow_();
        });
      }

      // Update badge on boot + periodically
      updateMailBadge_();
      setInterval(updateMailBadge_, 4500);

      // Listen for new mail updates from other tabs/windows
      window.addEventListener('storage', (ev) => {
        if(ev.key !== MAIL_LS_KEY) return;
        const nowUnread = unreadCountFor_(meName_());
        // Play sound only when unread count increases
        if(typeof lastUnread_ === 'number' && nowUnread > lastUnread_){
          playMailSound_();
        }
        lastUnread_ = nowUnread;
        updateMailBadge_();

        // If mail window asked for init, answer it
      });

      // Mail window can request init
      window.addEventListener('message', (ev) => {
        const d = ev.data || {};
        if(d && d.type === 'gemel-mail-need-init'){
          try{
            if(mailWinRef_ && !mailWinRef_.closed){
              mailWinRef_.postMessage({ type:'gemel-mail-init', me: meName_() }, '*');
            }
      ...
      // Close mail window when logging out (logout flow calls closeMail_ already)
      // Keep safe.
    }catch(_){}
  }

  // Wire at startup (only once)
  wireMail_Once_();
