  ...
  const UI = {
    ...
    init() {
      ...
      this.els.cfAgentSelect = $("#cfAgentSelect");
            this.els.cfAgentUnder = $("#cfAgentUnder");
...
if (this.els.cfIdIssueDate) this.els.cfIdIssueDate.textContent = (c.idIssueDate || "—");
      ...
      // agent under name (display only)
      if (this.els.cfAgentUnder) {
        const fallbackAgent = (State.data.agents && State.data.agents[0] && State.data.agents[0].name) ? State.data.agents[0].name : "";
        const agentName = c.assignedAgent || fallbackAgent || "—";
        const span = this.els.cfAgentUnder.querySelector(".agentName");
        if (span) span.textContent = agentName;
        else this.els.cfAgentUnder.textContent = `נציג/סוכן מטפל: ${agentName}`;
      }
