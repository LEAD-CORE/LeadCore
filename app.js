...
// Search (live filter on customers view)
    ...
    openCustomerFull(customerId) {
      ...
      const idNum = (c.idNumber || "").trim();

      if (this.els.cfName) this.els.cfName.textContent = fullName;

      // פרטי זיהוי ויצירת קשר: מציגים ת״ז בתוך שורה קיימת (ללא שינוי HTML/CSS)
      ...
      // לא מציגים ת״ז למעלה בצ׳יפ
      if (this.els.cfId) this.els.cfId.textContent = "";
