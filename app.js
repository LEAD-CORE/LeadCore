...
// Search (live filter on customers view)
    ...
    openCustomerFull(customerId) {
      ...
      if (this.els.cfName) this.els.cfName.textContent = fullName;

      // Render ת.ז inside the existing "Name" row (Identity & Contact group)
      // and hide it from the top area (chips / old top-row)
      if (this.els.cfNameLine) {
        this.els.cfNameLine.textContent = idNum ? `${fullName} (ת.ז: ${idNum})` : fullName;
      }

      if (this.els.cfPhone) this.els.cfPhone.textContent = c.phone || "—";

      if (this.els.cfId) {
        const chip = this.els.cfId.closest(".cfChip");
        const line = this.els.cfId.closest(".cfLine");
        if (chip) chip.style.display = "none";
        if (line) line.style.display = "none";
        this.els.cfId.textContent = "";
      }
