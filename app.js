...
function renderMailWindow_(){
  ...
  const js = `
    (function(){
      const MAIL_LS_KEY = ${json.dumps('gemel_mail_v1')};
      const USERS_LS_KEY = ${json.dumps('gemel_users_v1')};

      const $ = (s, r=document) => r.querySelector(s);
      const $$ = (s, r=document) => Array.from(r.querySelectorAll(s));

      function getMe(){
        try{ return window.opener && window.opener.__gemelCurrentUser ? window.opener.__gemelCurrentUser : null; }catch(_){ return null; }
      }
