...
`;
  ...
  function renderPoliciesByInsured(p, kind){
    ...
    const blocks = list.map(ins => {
      ...
      const head = (kind === 'new')
        ? `<tr><th>חברה</th><th>מוצר</th><th>פרמיה</th><th>אחרי הנחה</th><th>הנחה (מלל)</th><th>כיסויים</th><th>שיעבוד</th></tr>`
              ...
              <tbody>
                ${rows.map(x => {
                  if(kind === 'new'){
                    ...
                    const disc = (() => {
                      const d = x && x.discount ? x.discount : null;
                      const note = d && d.note != null ? String(d.note).trim() : '';
                      const tier = d && d.tier != null ? String(d.tier).trim() : '';
                      const v = note || (tier && tier !== '0%' ? tier : '');
