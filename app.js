...
(() => {
  ...
  const LS_REMEMBER_KEY = "LEAD_CORE_REMEMBER_ME_V1";
...
// ---------------------------
  ...
  const defaultState = () => ({
    meta: { updatedAt: null },
    // Users / Session (Admin + Agents)
    users: [
      { id: "u_admin", name: "מנהל מערכת", role: "admin", pin: "1234", active: true },
      { id: "u_yuval", name: "יובל מנדלסון", role: "agent", pin: "", active: true }
    ],
    session: { currentUserId: null },
    // legacy agents list (kept for compatibility; derived from users when possible)
  ...
  function normalizeState(s) {
    ...
    const out = {
      meta: { ...(s?.meta || {}) },
      users: Array.isArray(s?.users) ? s.users : null,
      session: (s?.session && typeof s.session === "object") ? { ...(s.session) } : { ...(base.session) },
      // legacy support
    ...
    // users (if missing, derive from legacy agents)
    if (!out.users) {
      const legacyAgents = Array.isArray(out.agents) ? out.agents : [];
      const derivedAgents = legacyAgents.map((a, idx) => ({
        id: safeTrim(a?.id) || ("u_agent_" + idx),
        name: safeTrim(a?.name) || "נציג/סוכן",
        role: "agent",
        pin: "",
        active: true
      }));
      out.users = [ ...(base.users || []), ...derivedAgents ];
    }

    // normalize users
    out.users = (out.users || []).map((u, idx) => ({
      id: safeTrim(u?.id) || ("u_" + idx),
      name: safeTrim(u?.name) || "משתמש",
      role: safeTrim(u?.role) === "admin" ? "admin" : "agent",
      pin: safeTrim(u?.pin),
      active: (u?.active === false || String(u?.active) === "false") ? false : true
    }));

    // ensure at least one admin exists
    if (!out.users.some(u => u.role === "admin")) {
      out.users.unshift({ id: "u_admin", name: "מנהל מערכת", role: "admin", pin: "1234", active: true });
    }

    // session
    out.session ||= { currentUserId: null };
    if (out.session.currentUserId == null) out.session.currentUserId = null;

    // derive agents list from active agent-users (for assignment + MyFlow)
    const agentUsers = out.users.filter(u => u.role === "agent" && u.active);
    out.agents = agentUsers.map((u, idx) => ({
      id: "a_" + (safeTrim(u.id) || idx),
      name: u.name
    }));
  ...
  const Storage = {
    ...
    _applyRemote(payload, at) {
      // preserve logged-in user across remote updates (polling)
      const __prevUid = safeTrim(State.data?.session?.currentUserId);
      State.data = normalizeState(payload || {});
      // restore session if possible
      try{
        const users = Array.isArray(State.data?.users) ? State.data.users : [];
        const keep = __prevUid && users.some(u=>u && u.id===__prevUid && u.active);
        State.data.session ||= { currentUserId: null };
        if(keep){
          State.data.session.currentUserId = __prevUid;
        } else {
          const admin = users.find(u=>u && u.role==="admin" && u.active);
          if(admin) State.data.session.currentUserId = admin.id;
        }
      }catch(_){}
      State.data.meta ||= {};
    ...
    // ---------------------------
  // Auth (Admin/Agent) • Premium login screen
  // ---------------------------
  const Auth = {
    _booted: false,
    _appStarted: false,

    getUsers(){ return Array.isArray(State.data.users) ? State.data.users : []; },

    getCurrentUser(){
      const id = safeTrim(State.data?.session?.currentUserId);
      if(!id) return null;
      return this.getUsers().find(u => u && u.id === id && u.active) || null;
    },
    ...
    loadSession(){
      try{
        ...
        const id = safeTrim(s?.currentUserId);
        if(!id) return null;
    ...
    saveSession(remember){
      try{
        ...
        localStorage.setItem(LS_SESSION_KEY, JSON.stringify({ currentUserId: safeTrim(State.data?.session?.currentUserId) || null }));
    ...
    show(){
      document.body.classList.add("lc-auth-locked");
      // focus user dropdown first, then PIN
      const userSel = UI.els.authUser;
      const pin = UI.els.authPin;
      if(userSel && !safeTrim(userSel.value)){
        setTimeout(()=>{ try{ userSel.focus(); }catch(_){} }, 40);
      } else if(pin){
        setTimeout(()=>{ try{ pin.focus(); }catch(_){} }, 40);
      }
      UI.updateTopUser();
      UI.applyRoleUi();
      UI.renderAuthUsers();
    ...
    hide(){
      document.body.classList.remove("lc-auth-locked");
    },

    async tryAutoLogin(){
      try{
        const remember = localStorage.getItem(LS_REMEMBER_KEY);
        if(!remember) return false;
      }catch(_){ return false; }
      const s = this.loadSession();
      if(!s || !s.currentUserId) return false;
      const u = this.getUsers().find(x => x && x.id === s.currentUserId && x.active);
      if(!u) return false;
      State.data.session ||= { currentUserId: null };
      State.data.session.currentUserId = u.id;
      UI.updateTopUser();
      UI.applyRoleUi();
      return true;
    },

    loginByCredentials(userId, pin, remember){
      const uidIn = safeTrim(userId);
      const p = safeTrim(pin);
      if(!uidIn) return { ok:false, error:"נא לבחור שם משתמש." };
      if(!p) return { ok:false, error:"נא להזין קוד כניסה." };

      const users = this.getUsers().filter(u => u && u.active);
      const user = users.find(u => u.id === uidIn) || users.find(u => safeTrim(u.name) === uidIn);
      if(!user) return { ok:false, error:"משתמש לא נמצא או לא פעיל." };
      if(safeTrim(user.pin) !== p) return { ok:false, error:"קוד שגוי." };

      State.data.session ||= { currentUserId: null };
      State.data.session.currentUserId = user.id;
    ...
    loginByPin(pin, remember){
      // if no user selected, default to admin
      const admin = this.getUsers().find(u => u && u.role === "admin" && u.active) || this.getUsers()[0];
      return this.loginByCredentials(admin?.id, pin, remember);
    },

    async startAppIfNeeded(){
      if(this._appStarted) return;
      this._appStarted = true;
      await App.boot();
      UI.applyRoleUi();
      UI.renderAgentOptions();
      UI.renderUsers();
    },
    ...
    logout(){
      this.clearSession();
      this.show();
      try{ UI.goView("dashboard"); }catch(_){}
    },
    ...
    async init(){
      ...
      const img = UI.els.authLogoImg;
      const txt = UI.els.authLogoText;
      if(img){
      ...
      // preload users list from Sheets so login screen shows real users
      const prevId = safeTrim(State.data?.session?.currentUserId);
      try{
      ...
      // if there was a previous session, try to restore it (only if still active)
      if(prevId){
        try{
          const users = Array.isArray(State.data?.users) ? State.data.users : [];
          if(users.some(u=>u && u.id===prevId && u.active)){
            State.data.session ||= { currentUserId: null };
            State.data.session.currentUserId = prevId;
          }
        }catch(_){}
      }

      // populate login user dropdown (active users)
      UI.renderAuthUsers();

      on(UI.els.authForm, "submit", async (e) => {
        e.preventDefault();
        const userId = safeTrim(UI.els.authUser?.value);
        const pin = safeTrim(UI.els.authPin?.value);
        const remember = !!UI.els.authRemember?.checked;
        const r = this.loginByCredentials(userId, pin, remember);
        if(!r.ok){
          UI.setAuthHelp(r.error || "שגיאה");
          return;
        }
        UI.setAuthHelp("מתחבר…");
        this.hide();
        await this.startAppIfNeeded();
        UI.renderAll();
        UI.goView("dashboard");
      });

      on(UI.els.btnLogout, "click", () => this.logout());

      this.show();
      this.tryAutoLogin().then(async ok => {
        if(ok){
          this.hide();
          await this.startAppIfNeeded();
          UI.renderAll();
...
// ---------------------------
  ...
  const UI = {
    ...
    init() {
      ...
      // auth
      this.els.authScreen = $("#authScreen");
      this.els.authForm = $("#authForm");
      this.els.authUser = $("#authUser");
      this.els.authPin = $("#authPin");
      this.els.authRemember = $("#authRemember");
      this.els.authHelp = $("#authHelp");
      this.els.authLogoImg = $("#authLogoImg");
      this.els.authLogoText = $(".lcAuth__logoText");

      // top user
      this.els.topUser = $("#topUser");
      this.els.topUserName = $("#topUserName");
      ...
      this.els.modalUser = $("#modalUser");
      this.els.userForm = $("#userForm");
      ...
      // Users (admin) interactions
      on(this.els.btnAddUser, "click", () => this.openUserModal());
      on(this.els.usersSearch, "input", () => this.renderUsers());

      on($("#view-users"), "click", (e) => {
        const btnEdit = e?.target?.closest?.("button[data-edit-user]");
        if (btnEdit) {
          const id = safeTrim(btnEdit.dataset.editUser);
          if (id) this.openUserModal(id);
          return;
        }
        const btnToggle = e?.target?.closest?.("button[data-toggle-user]");
        if (btnToggle) {
          const id = safeTrim(btnToggle.dataset.toggleUser);
          if (id) this.toggleUserActive(id);
          return;
        }
      });

      on(this.els.userForm, "submit", async (e) => {
        e.preventDefault();
      ...
      $$("[data-close='1']").forEach(el => on(el, "click", () => {
        ...
        if (el.closest("#modalUser")) return this.closeUserModal();
...
// Search (live filter on customers view)
    ...
    updateTopUser(){
      ...
      if(!this.els.topUser || !this.els.topUserName) return;
      if(!u){
        this.els.topUser.style.display = "none";
        return;
      }
      this.els.topUser.style.display = "flex";
      const roleLabel = (u.role === "admin") ? "מנהל" : "נציג/סוכן";
      this.els.topUserName.textContent = `מחובר: ${u.name} • ${roleLabel}`;
    },

    applyRoleUi(){
      const isAdmin = Auth.isAdmin();

      // sidebar admin-only button
      $$(".lcAdminOnly").forEach(el => {
        try { el.style.display = isAdmin ? "" : "none"; } catch(_) {}
      });

      // MyFlow: for agent, lock to self and hide agent dropdown
      const mfWrap = $(".lcMyFlowAgent", this.els.viewMyFlow || document);
      if(mfWrap){
        mfWrap.style.display = isAdmin ? "" : "none";
      }

      // New customer: assigned agent field is admin-only; agent gets locked to self
      const newAssign = this.els.newAssignedAgent;
      if(newAssign){
        newAssign.disabled = !isAdmin;
      }
    ...
    setAuthHelp(text){
      if(this.els.authHelp) this.els.authHelp.textContent = safeTrim(text) || "";
    },

    renderAuthUsers(){
      const sel = this.els.authUser;
      if(!sel) return;

      // keep current selection if possible
      const prev = safeTrim(sel.value);

      // rebuild options
      sel.innerHTML = '<option value="" disabled>בחר משתמש…</option>';
      const users = Array.isArray(State.data?.users) ? State.data.users : [];
      const active = users.filter(u => u && u.active);
      // put admin first
      active.sort((a,b) => (a.role === "admin" ? -1 : 1) - (b.role === "admin" ? -1 : 1));

      for(const u of active){
        const opt = document.createElement("option");
        opt.value = u.id;
        opt.textContent = u.name + (u.role === "admin" ? " • מנהל" : " • נציג/סוכן");
        sel.appendChild(opt);
      }

      // select prev or default admin
      const wants = prev && active.some(u => u.id === prev) ? prev : (active.find(u=>u.role==="admin")?.id || active[0]?.id || "");
      if(wants) sel.value = wants;
    },
    ...
    renderUsers(){
      ...
      const rolePill = (role) => {
        ...
        const cls = isAdminRole ? "lcRolePill lcRolePill--admin" : "lcRolePill";
        return `<span class="${cls}">${isAdminRole ? "מנהל" : "נציג/סוכן"}</span>`;
      };

      const statusPill = (active) => {
        return `<span class="lcStatusPill ${active ? "" : "is-off"}">${active ? "פעיל" : "מושבת"}</span>`;
      };
      ...
      this.els.usersTbody.innerHTML = users.map(us => `
        <tr>
          <td>${escapeHtml(us.name)}</td>
          <td>${rolePill(us.role)}</td>
          <td>${statusPill(us.active)}</td>
          <td>${pinShow(us.pin)}</td>
          <td>
            <div class="lcMiniBtns">
              <button class="lcMiniBtn" type="button" data-edit-user="${escapeHtml(us.id)}">עריכה</button>
              <button class="lcMiniBtn ${us.active ? "lcMiniBtn--danger" : ""}" type="button" data-toggle-user="${escapeHtml(us.id)}">${us.active ? "השבת" : "הפעל"}</button>
            </div>
          </td>
        </tr>
      `).join("");
    ...
    openUserModal(editId){
      if(!this.els.modalUser || !this.els.userForm) return;
      const me = Auth.getCurrentUser();
      if(!me || me.role !== "admin") return;

      const isEdit = !!editId;
      this.els.modalUser.classList.add("is-open");
      this.els.modalUser.setAttribute("aria-hidden", "false");

      const title = $("#userModalTitle", this.els.modalUser);
      if(title) title.textContent = isEdit ? "עריכת משתמש" : "הוספת נציג/סוכן";

      this.els.userForm.reset();
      this.els.modalUser.dataset.editId = editId ? String(editId) : "";

      if(isEdit){
        const u = (State.data.users || []).find(x => x && x.id === editId);
        if(u){
          this.els.userForm.elements.name.value = u.name || "";
          this.els.userForm.elements.role.value = (u.role === "admin") ? "admin" : "agent";
          this.els.userForm.elements.pin.value = u.pin || "";
          this.els.userForm.elements.active.value = u.active ? "true" : "false";
        }
      } else {
        this.els.userForm.elements.role.value = "agent";
        this.els.userForm.elements.active.value = "true";
      }

      const first = this.els.userForm.querySelector("input[name='name']");
      if(first) setTimeout(()=>{ try{ first.focus(); }catch(_){} }, 20);
    },

    closeUserModal(){
      if(!this.els.modalUser) return;
      this.els.modalUser.classList.remove("is-open");
      this.els.modalUser.setAttribute("aria-hidden", "true");
      try{ delete this.els.modalUser.dataset.editId; }catch(_){}
    },

    async saveUserFromForm(){
      if(!this.els.userForm) return;
      const me = Auth.getCurrentUser();
      if(!me || me.role !== "admin") return;

      const fd = new FormData(this.els.userForm);
      const name = safeTrim(fd.get("name"));
      const role = safeTrim(fd.get("role")) === "admin" ? "admin" : "agent";
      const pin = safeTrim(fd.get("pin"));
      const active = String(fd.get("active")) !== "false";

      if(!name){
        notify("נא למלא שם מלא.", "warn");
        return;
      }
      if(pin && !/^\d{4,8}$/.test(pin)){
        notify("PIN מומלץ להיות 4–8 ספרות.", "warn");
        return;
      }

      const editId = safeTrim(this.els.modalUser?.dataset?.editId);
      State.data.users ||= [];
      if(editId){
        const u = State.data.users.find(x => x && x.id === editId);
        if(!u){ notify("משתמש לעריכה לא נמצא.", "error"); return; }
        u.name = name;
        u.role = role;
        u.pin = pin;
        u.active = active;
      } else {
        const newUser = { id: "u_" + uid(), name, role, pin, active };
        State.data.users.unshift(newUser);
      }

      State.set(State.data);

      const r = await App.save("עודכן משתמש");
      if(!r.ok){
        notify("שמירה נכשלה: " + (r.error || "שגיאה"), "error");
        return;
      }
      ...
      this.renderMyFlow();
      this.renderAll();
      UI.renderSyncStatus("עודכן", "ok", 1500);
    },
    ...
    async toggleUserActive(userId){
      ...
      const u = (State.data.users || []).find(x => x && x.id === userId);
      if(!u) return;

      if(u.role === "admin" && u.active){
        const activeAdmins = (State.data.users || []).filter(x => x && x.role === "admin" && x.active);
        if(activeAdmins.length <= 1){
          notify("לא ניתן להשבית את המנהל האחרון במערכת.", "warn");
          return;
        }
      }
      ...
      const r = await App.save("עודכן סטטוס משתמש");
      if(!r.ok){
        notify("שמירה נכשלה: " + (r.error || "שגיאה"), "error");
        return;
      }
    ...
    renderAgentOptions(){
      ...
      const fill = (sel, selectedName, lockToMe) => {
        if(!sel) return;
        sel.innerHTML = agents.map(a => `<option value="${escapeHtml(a.name)}">${escapeHtml(a.name)}</option>`).join("");
        const v = lockToMe ? myName : selectedName;
        if(v) sel.value = v;
      };

      fill(this.els.newAssignedAgent, (agents[0] ? agents[0].name : ""), (!isAdmin && !!myName));
      if(this.els.newAssignedAgent){
        if(!isAdmin && myName){
          this.els.newAssignedAgent.value = myName;
        }
      }

      // MyFlow agent selector (admin only)
      if(this.els.myFlowAgent && isAdmin){
        const cur = safeTrim(this.els.myFlowAgent.value) || (agents[0] ? agents[0].name : "");
        this.els.myFlowAgent.innerHTML = agents.map(a =>
          `<option value="${escapeHtml(a.name)}">${escapeHtml(a.name)}</option>`
        ).join("");
        if(cur) this.els.myFlowAgent.value = cur;
      }
    },
    goView(view) {
    ...
    _getActiveAgentName(){
      ...
      const me = Auth.getCurrentUser();
      if(me && me.role === "agent") return me.name || fallback;
...
const App = {
    ...
    async boot() {
      ...
      if (r.ok) {
        const __prevUid = safeTrim(State.data?.session?.currentUserId);
        State.set(r.payload);
        // restore logged-in user if remote payload overwrote session
        try{
          const users = Array.isArray(State.data?.users) ? State.data.users : [];
          const keep = __prevUid && users.some(u=>u && u.id===__prevUid && u.active);
          State.data.session ||= { currentUserId: null };
    ...
    async syncNow() {
      try {
        ...
        // take remote truth (preserve logged-in user)
        const __prevUid = safeTrim(State.data?.session?.currentUserId);
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      UI.init();
      // Auth will start the app only after successful login
      await Auth.init();
    } catch (e) {
