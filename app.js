...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // ---------- INTERNAL MAIL (Local) ----------
  // Stores messages in localStorage. Ready to migrate to Google Sheets later.
  const MAIL_LS_KEY = 'gemel_mail_v1';

  let _mailLastUnread = 0;
  let _mailPollTimer = null;

  function loadMail_(){
    try{
      const raw = localStorage.getItem(MAIL_LS_KEY);
      const arr = raw ? JSON.parse(raw) : null;
  ...
  function saveMail_(arr){
    try{ localStorage.setItem(MAIL_LS_KEY, JSON.stringify(arr || [])); }catch(_){}
  }

  function meUsername_(){ return String(state.currentUser?.username || '').trim(); }
  function meName_(){ return String(state.currentUser?.displayName || state.agentName || '').trim(); }

  function listActiveUsersForMail_(){
    const users = (typeof loadUsers_ === 'function' ? loadUsers_() : []);
    return (users||[])
      .filter(u => u && u.active !== false)
      .map(u => ({ username:String(u.username||'').trim(), name:String(u.displayName||u.username||'').trim() }))
  ...
  function unreadCount_(){
    const u = meUsername_();
    if(!u) return 0;
    const msgs = loadMail_();
    return msgs.filter(m => m && Array.isArray(m.toUsers) && m.toUsers.includes(u) && !(m.readBy && m.readBy[u])).length;
  }

  function markRead_(msgId){
    const u = meUsername_();
    if(!u) return;
    const msgs = loadMail_();
    const m = msgs.find(x => x && x.id === msgId);
    if(!m) return;
    if(!m.readBy) m.readBy = {};
    if(m.readBy[u]) return;
    m.readBy[u] = true;
    saveMail_(msgs);
  }

  function playMailSound_(){
    try{
  ...
  function ensureMailOverlay_(){
    let ov = document.getElementById('mailOverlay');
    if(!ov) return null;
    if(!ov.querySelector('.mailModal')){
      ov.innerHTML = `
        <div class="mailModal" role="dialog" aria-modal="true" aria-label="דואר פנימי">
          <div class="mailHeader">
            <div class="mailTitle">
              <b>דואר פנימי</b>
              <span id="mailSubLine"></span>
            </div>
            <div style="display:flex;align-items:center;gap:10px">
              <div class="mailTabs" id="mailTabs">
                <button type="button" class="mailTab active" data-mailtab="inbox">דואר נכנס</button>
                <button type="button" class="mailTab" data-mailtab="sent">נשלח</button>
                <button type="button" class="mailTab" data-mailtab="compose">כתיבה</button>
              </div>
              <button type="button" class="btn btnSoft" id="mailCloseBtn">סגור</button>
            </div>
          </div>

          <div class="mailBody">
            <div class="mailList" id="mailList"></div>
            <div class="mailPane" id="mailPane"></div>
          </div>
        </div>
      `;
      // close interactions
      ov.addEventListener('click', (e) => { if(e.target === ov) closeMail_(); });
      ov.querySelector('#mailCloseBtn')?.addEventListener('click', closeMail_);
      ov.querySelectorAll('[data-mailtab]').forEach(b => b.addEventListener('click', () => setMailTab_(b.dataset.mailtab)));
    }
    return ov;
  }
...
function openMail_(){
  // Opens in a separate small browser window (independent from the app layout)
  try{
    if(_mailWin && !_mailWin.closed){
      _mailWin.focus();
      _mailWin.postMessage({ type:'gemel_mail_refresh' }, '*');
      return;
    }
  }catch(_){}
  ...
  const w = window.open('', 'GEMEL_INVEST_MAIL', features);
  if(!w){
    toast('חסום', 'הדפדפן חסם חלון קופץ. אפשר לאשר Pop-ups לאתר ואז לנסות שוב.');
    return;
  }
  _mailWin = w;

  // Write the mail app into the new window (same-origin -> shares localStorage)
  renderMailWindow_();
  try{ _mailWin.focus(); }catch(_){}
}

function closeMail_(){
  try{
    if(_mailWin && !_mailWin.closed) _mailWin.close();
  }catch(_){}
  _mailWin = null;
}

window.openMail_ = openMail_; // safe global if needed

function renderMailWindow_(){
  if(!_mailWin || _mailWin.closed) return;
  ...
  const css = `
    ...
    *{ box-sizing:border-box; }
    body{
      margin:0;
      font-family: ui-sans-serif, system-ui, -apple-system, "Noto Sans Hebrew", "Heebo", Arial;
      background: radial-gradient(900px 520px at 35% 0%, rgba(255,255,255,.92), rgba(246,242,234,.96) 60%, rgba(239,233,221,.98));
      color:#0F1116;
    }
    ...
    .card{
      ...
      border-radius: 18px;
      box-shadow: 0 18px 45px rgba(18,19,25,.10);
    ...
    header{
      ...
      gap:10px;
      padding:12px 12px;
      border-bottom:1px solid rgba(15,17,22,.10);
      background: linear-gradient(180deg, rgba(255,255,255,.78), rgba(246,242,234,.60));
    }
    ...
    .btn{
      border:1px solid rgba(15,17,22,.14);
    ...
    .list{
      border-left:1px solid rgba(15,17,22,.10);
      padding:10px;
      overflow:auto;
    ...
    .item.unread{ border-color: rgba(214,178,94,.55); box-shadow: 0 0 0 3px rgba(214,178,94,.12); }
    .rowTop{ display:flex; justify-content:space-between; gap:10px; font-size:12px; opacity:.75; }
    .subj{ margin-top:6px; font-weight:900; font-size:13px; }
    .hint{ font-size:12px; opacity:.65; padding:10px; }
    .field{ width:100%; padding:10px 10px; border-radius: 12px; border:1px solid rgba(15,17,22,.14); background: rgba(255,255,255,.78); font-family:inherit; }
    textarea.field{ min-height:220px; resize:vertical; }
    ...
    @media (max-width: 620px){
      ...
      .list{ border-left:none; border-bottom:1px solid rgba(15,17,22,.10); max-height: 240px; }
    }
  `;
  ...
  const js = `
    (function(){
      ...
      function getMe(){
        // Prefer opener (main app), fallback to localStorage public session (when opener is unavailable)
        try{
          if(window.opener && window.opener.__gemelCurrentUser) return window.opener.__gemelCurrentUser;
        }catch(_){}
        try{
          const raw = localStorage.getItem('gemel_session_pub_v1');
          const obj = raw ? JSON.parse(raw) : null;
          return (obj && obj.username) ? obj : null;
        }catch(_){ return null; }
      }
      function meUsername(){ return String(getMe()?.username||'').trim(); }
      function meName(){ return String(getMe()?.displayName||'').trim(); }
      ...
      function saveMail(arr){
        try{ localStorage.setItem(MAIL_LS_KEY, JSON.stringify(arr||[])); }catch(_){}
        try{ window.opener && window.opener.postMessage({ type:'gemel_mail_changed' }, '*'); }catch(_){}
      }
      function loadUsers(){
        try{
      ...
      function listActiveUsers(){
        ...
        return loadUsers()
          ...
          .map(x => ({ username:String(x.username||'').trim(), name:String(x.displayName||x.username||'').trim() }))
          .filter(x => x.username && x.name && x.username !== u);
      }
      function uid(){
      ...
      function render(){
        ...
        const sub = $('#subLine');
        if(!u){
          sub.textContent = 'לא מחובר/ת — יש להתחבר למערכת הראשית';
        }else{
          sub.textContent = 'מחובר/ת: ' + (name||u) + ' (' + u + ')';
        }

        $$('#tabs .tab').forEach(b => b.classList.toggle('active', b.dataset.tab === tab));

        const msgs = loadMail().slice().sort((a,b)=> new Date(b.createdAt||0)-new Date(a.createdAt||0));
        const inbox = msgs.filter(m => m && u && Array.isArray(m.toUsers) && m.toUsers.includes(u));
        const sent  = msgs.filter(m => m && u && String(m.fromUser||'') === u);
        ...
        if(tab==='compose'){
          ...
          listEl.innerHTML = '<div style="font-weight:900;margin-bottom:8px">בחירת נמענים</div>' +
            '<div class="hint" style="padding:0 0 10px">אפשר לבחור נמען אחד או כמה.</div>' +
            '<div class="chips">' +
            (users.length ? users.map(x => (
              '<label class="chip" title="'+escapeHtml(x.username)+'">' +
                '<input type="checkbox" data-to="'+escapeHtml(x.username)+'">' +
                '<span>'+escapeHtml(x.name)+'</span>' +
              '</label>'
            )).join('') : '<div class="hint">אין משתמשים זמינים כרגע.</div>') +
            '</div>';

          paneEl.innerHTML =
            '<div style="font-weight:900;font-size:14px;margin-bottom:10px">כתיבת הודעה</div>' +
            '<div style="font-size:12px;opacity:.7;margin-bottom:6px">נושא</div>' +
            '<input id="subject" class="field" type="text" placeholder="לדוגמה: עדכון לקוח / תיאום שיחה">' +
            '<div style="font-size:12px;opacity:.7;margin:10px 0 6px">תוכן</div>' +
            '<textarea id="body" class="field" placeholder="כתוב/י כאן..."></textarea>' +
            '<div class="actions">' +
              '<button class="btn" id="clearBtn">נקה</button>' +
              '<button class="btn" id="sendBtn">שלח</button>' +
            '</div>' +
            '<div class="hint">אפשר להשאיר את החלון פתוח ולהמשיך לעבוד במערכת.</div>';

          $('#clearBtn').onclick = () => {
            $('#subject').value = '';
            $('#body').value = '';
            $$('[data-to]').forEach(c => c.checked = false);
          };

          $('#sendBtn').onclick = () => {
            if(!u){
              alert('יש להתחבר למערכת הראשית');
              return;
            }
            const toUsers = $$('[data-to]').filter(c=>c.checked).map(c=>c.getAttribute('data-to'));
            const toNamesMap = Object.fromEntries(listActiveUsers().map(x => [x.username, x.name]));
            const subject = String($('#subject').value||'').trim();
            const body = String($('#body').value||'').trim();
            if(!toUsers.length){
              alert('בחר/י לפחות נמען אחד');
              return;
            }
            if(!body){
              alert('חסר תוכן הודעה');
              return;
            }
            const msg = {
              id: uid(),
              ...
              fromName: name || u,
              toUsers,
              toNames: toUsers.map(x => toNamesMap[x] || x),
              subject,
              body,
              createdAt: new Date().toISOString(),
              readBy: { [u]: true } // sender is "read"
            };
            const all = loadMail();
            all.unshift(msg);
            saveMail(all);
        ...
        if(!list.length){
          listEl.innerHTML = '<div class="hint">'+(tab==='sent' ? 'אין הודעות שנשלחו עדיין.' : 'אין הודעות נכנסות.')+'</div>';
          paneEl.innerHTML = '<div class="hint">—</div>';
          return;
        }

        // list
        listEl.innerHTML = list.map(m => {
          const isUnread = (tab!=='sent') && !(m.readBy && u && m.readBy[u]);
          const from = (tab==='sent')
            ? ('אל: ' + (Array.isArray(m.toNames) ? m.toNames.join(', ') : (Array.isArray(m.toUsers)?m.toUsers.join(', '):'')))
            : (m.fromName || m.fromUser || '—');
          const subj = String(m.subject||'').trim() || '(ללא נושא)';
          return (
            '<div class="item '+(isUnread?'unread':'')+'" data-id="'+escapeHtml(m.id)+'">' +
              '<div class="rowTop"><div>'+escapeHtml(from)+'</div><div>'+escapeHtml(fmtTime(m.createdAt))+'</div></div>' +
        ...
        $$('[data-id]').forEach(el => {
          el.onclick = () => {
            ...
            if(tab!=='sent') markRead(selectedId);
            render();
          };
        });

        if(!selectedId) selectedId = list[0].id;
        const msg = list.find(x => x && x.id === selectedId) || list[0];

        if(!msg){
          paneEl.innerHTML = '<div class="hint">בחר/י הודעה מהרשימה כדי לקרוא.</div>';
          return;
        }

        const subj = String(msg.subject||'').trim() || '(ללא נושא)';
        const fromLine = (tab==='sent')
          ? '<b>אל:</b> ' + escapeHtml((Array.isArray(msg.toNames)&&msg.toNames.length)?msg.toNames.join(', '):(Array.isArray(msg.toUsers)?msg.toUsers.join(', '):'')) 
          : '<b>מאת:</b> ' + escapeHtml(msg.fromName||msg.fromUser||'');
        const toLine = (tab==='sent')
          ? '<b>מאת:</b> ' + escapeHtml(msg.fromName||msg.fromUser||'')
          : '<b>אל:</b> ' + escapeHtml((Array.isArray(msg.toNames)&&msg.toNames.length)?msg.toNames.join(', '):(Array.isArray(msg.toUsers)?msg.toUsers.join(', '):'')); 

        paneEl.innerHTML =
          '<div style="font-weight:900;font-size:15px;margin-bottom:8px">'+escapeHtml(subj)+'</div>' +
          '<div style="font-size:12px;opacity:.75;margin-bottom:6px">'+fromLine+'</div>' +
          '<div style="font-size:12px;opacity:.75;margin-bottom:6px">'+toLine+'</div>' +
          '<div style="font-size:12px;opacity:.75;margin-bottom:10px"><b>תאריך:</b> '+escapeHtml(fmtTime(msg.createdAt))+'</div>' +
          (tab==='inbox' ? '<div class="actions" style="justify-content:flex-start"><button class="btn" id="replyBtn">השב</button></div>' : '') +
          '<div style="white-space:pre-wrap;line-height:1.45;background:rgba(255,255,255,.70);border:1px solid rgba(15,17,22,.10);border-radius:14px;padding:12px;min-height:220px">'+escapeHtml(String(msg.body||''))+'</div>';
        ...
        if(tab==='inbox'){
          ...
          if(replyBtn){
            replyBtn.onclick = () => {
              ...
              selectedId = '';
              render();
              // prefill recipients + subject
              setTimeout(() => {
                const fromU = String(msg.fromUser||'').trim();
                const cb = document.querySelector('[data-to="'+fromU.replace(/"/g,'&quot;')+'"]');
                // better: loop
                $$('[data-to]').forEach(c => { c.checked = (c.getAttribute('data-to')===fromU); });
                $('#subject').value = 'RE: ' + (String(msg.subject||'').trim()||'');
                $('#body').value = '
...
' + String(msg.body||'');
      ...
      $$('#tabs .tab').forEach(b => b.onclick = () => { tab = b.dataset.tab; selectedId=''; render(); });

      window.addEventListener('message', (ev) => {
        if(ev && ev.data && (ev.data.type==='gemel_mail_refresh' || ev.data.type==='gemel_mail_changed')){
          render();
        }
      });

      window.addEventListener('storage', (e) => {
        if(e && e.key===MAIL_LS_KEY) render();
      });
          ...
          <header>
            ...
            <div style="display:flex;align-items:center;gap:10px">
              <div class="tabs" id="tabs">
                <button class="tab active" data-tab="inbox" type="button">דואר נכנס</button>
                <button class="tab" data-tab="sent" type="button">נשלח</button>
                <button class="tab" data-tab="compose" type="button">כתיבה</button>
              </div>
              <button class="btn" id="closeBtn" type="button">סגור</button>
            </div>
          </header>
          <div class="layout">
  ...
  _mailWin.document.open();
  _mailWin.document.write(html);
  _mailWin.document.close();
}

function setMailTab_(tab){
    _mailTab = String(tab||'inbox');
    _mailSelectedId = '';
    // tab UI
    const ov = ensureMailOverlay_();
    ov?.querySelectorAll('[data-mailtab]').forEach(b => {
      b.classList.toggle('active', b.dataset.mailtab === _mailTab);
    });
    renderMail_();
  }

  function fmtMailTime_(iso){
    try{
      const d = new Date(iso||'');
      if(isNaN(d.getTime())) return '';
      return d.toLocaleString('he-IL');
  ...
  function renderMail_(){
    const ov = ensureMailOverlay_();
    if(!ov) return;
    const listEl = ov.querySelector('#mailList');
    const paneEl = ov.querySelector('#mailPane');
    const subLine = ov.querySelector('#mailSubLine');

    const u = meUsername_();
    const name = meName_() || state.agentName || '';
    if(subLine) subLine.textContent = `מחובר/ת: ${name}${u?` (${u})`:''}`;

    const msgs = loadMail_().slice().sort((a,b)=> new Date(b.createdAt||0).getTime() - new Date(a.createdAt||0).getTime());

    const inbox = msgs.filter(m => m && Array.isArray(m.toUsers) && u && m.toUsers.includes(u));
    const sent  = msgs.filter(m => m && String(m.fromUser||'') === u);

    const currentList = (_mailTab === 'sent') ? sent : inbox;

    if(listEl){
      if(_mailTab === 'compose'){
        listEl.innerHTML = `
          <div style="font-weight:900;margin-bottom:8px">בחירת נמענים</div>
          <div class="sideHint" style="margin-bottom:10px">ניתן לבחור נמען אחד או כמה.</div>
          <div id="mailRecipientsList"></div>
        `;
        const users = listActiveUsersForMail_().filter(x => x.username !== u);
        const wrap = listEl.querySelector('#mailRecipientsList');
        if(wrap){
          wrap.innerHTML = users.map(x => `
            <label class="mailChip" title="${escapeAttr(x.username)}">
              <input type="checkbox" data-mailto="${escapeAttr(x.username)}">
              <span>${escapeHtml(x.name)}</span>
            </label>
          `).join('') || `<div class="sideHint">אין משתמשים זמינים כרגע.</div>`;
        }
      }else{
        if(!currentList.length){
          listEl.innerHTML = `<div class="sideHint">${_mailTab==='sent' ? 'אין הודעות שנשלחו עדיין.' : 'אין הודעות נכנסות.'}</div>`;
        }else{
          listEl.innerHTML = currentList.map(m => {
            const isUnread = (_mailTab !== 'sent') && !(m.readBy && u && m.readBy[u]);
            const from = _mailTab==='sent' ? ('אל: ' + (Array.isArray(m.toNames) ? m.toNames.join(', ') : (Array.isArray(m.toUsers)?m.toUsers.join(', '):''))) : (m.fromName||m.fromUser||'—');
            const subj = String(m.subject||'').trim() || '(ללא נושא)';
            return `
              <div class="mailItem ${isUnread?'unread':''}" data-mailid="${escapeAttr(m.id)}">
                <div class="mailItemTop">
                  <div class="mailFrom">${escapeHtml(from)}</div>
                  <div class="mailTime">${escapeHtml(fmtMailTime_(m.createdAt))}</div>
                </div>
                <div class="mailSub">${escapeHtml(subj)}</div>
              </div>
            `;
          }).join('');

          listEl.querySelectorAll('[data-mailid]').forEach(el => {
            el.addEventListener('click', () => {
              _mailSelectedId = el.getAttribute('data-mailid') || '';
              if(_mailTab !== 'sent' && _mailSelectedId) markRead_(_mailSelectedId);
              renderMail_();
              updateMailBadge_();
            });
          });
    ...
    if(paneEl){
      if(_mailTab === 'compose'){
        paneEl.innerHTML = `
          <div style="font-weight:900;font-size:14px;margin-bottom:10px">כתיבת הודעה</div>
          <div class="mailComposeGrid">
            <div>
              <div style="font-size:12px;color:rgba(15,17,22,.65);margin-bottom:6px">נושא</div>
              <input id="mailSubject" class="input" style="width:100%" type="text" placeholder="לדוגמה: עדכון לקוח / תיאום שיחה">
            </div>
            <div>
              <div style="font-size:12px;color:rgba(15,17,22,.65);margin-bottom:6px">תוכן</div>
              <textarea id="mailBodyTxt" class="input" style="width:100%;min-height:220px;resize:vertical" placeholder="כתוב/י כאן..."></textarea>
            </div>
            <div class="mailActions">
              <button type="button" class="btn btnSoft" id="mailClearBtn">נקה</button>
              <button type="button" class="btn btnPrimary" id="mailSendBtn">שלח</button>
            </div>
            <div class="sideHint">טיפ: אפשר להשאיר את החלון פתוח ולהמשיך לעבוד — ההודעות נשמרות מקומית (דמו) עד שנחבר לשרת.</div>
          </div>
        `;
        paneEl.querySelector('#mailClearBtn')?.addEventListener('click', () => {
          paneEl.querySelector('#mailSubject').value = '';
          paneEl.querySelector('#mailBodyTxt').value = '';
          (ensureMailOverlay_()?.querySelectorAll('[data-mailto]')||[]).forEach(ch => { ch.checked = false; });
        });
        paneEl.querySelector('#mailSendBtn')?.addEventListener('click', () => sendMailFromCompose_());
      }else{
        if(!_mailSelectedId && currentList.length) _mailSelectedId = currentList[0].id;
        const msg = currentList.find(x => x && x.id === _mailSelectedId) || null;

        if(!msg){
          paneEl.innerHTML = `<div class="sideHint">בחר/י הודעה מהרשימה כדי לקרוא.</div>`;
        }else{
          const subj = String(msg.subject||'').trim() || '(ללא נושא)';
          const fromLine = _mailTab==='sent' ? `<b>אל:</b> ${escapeHtml((Array.isArray(msg.toNames)&&msg.toNames.length)?msg.toNames.join(', '): (Array.isArray(msg.toUsers)?msg.toUsers.join(', '):''))}` : `<b>מאת:</b> ${escapeHtml(msg.fromName||msg.fromUser||'')}`;
          const toLine = _mailTab==='sent' ? `<b>מאת:</b> ${escapeHtml(msg.fromName||msg.fromUser||'')}` : `<b>אל:</b> ${escapeHtml((Array.isArray(msg.toNames)&&msg.toNames.length)?msg.toNames.join(', '): (Array.isArray(msg.toUsers)?msg.toUsers.join(', '):''))}`;

          paneEl.innerHTML = `
            <div class="mailReadHead">
              <div style="font-weight:900;font-size:15px">${escapeHtml(subj)}</div>
              <div class="row">${fromLine}</div>
              <div class="row">${toLine}</div>
              <div class="row"><b>תאריך:</b> ${escapeHtml(fmtMailTime_(msg.createdAt))}</div>
              <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:6px">
                ${_mailTab==='inbox' ? `<button type="button" class="btn btnSoft" id="mailReplyBtn">השב</button>` : ``}
                <button type="button" class="btn btnSoft" id="mailClosePaneBtn">סגור</button>
              </div>
            </div>
            <div class="mailContent">${escapeHtml(String(msg.body||''))}</div>
          `;
          paneEl.querySelector('#mailClosePaneBtn')?.addEventListener('click', () => { _mailSelectedId=''; renderMail_(); });
          paneEl.querySelector('#mailReplyBtn')?.addEventListener('click', () => {
            // jump to compose prefilled
            setMailTab_('compose');
            const ov2 = ensureMailOverlay_();
            // pre-select sender
            const fromU = String(msg.fromUser||'').trim();
            if(fromU){
              const boxes = ov2 ? Array.from(ov2.querySelectorAll("[data-mailto]")) : [];
              const box = boxes.find(b => String(b.getAttribute("data-mailto")||"").trim() === fromU);
              if(box) box.checked = true;
            }
            setTimeout(() => {
              const subjEl = ov2?.querySelector('#mailSubject');
              const bodyEl = ov2?.querySelector('#mailBodyTxt');
              if(subjEl) subjEl.value = 'Re: ' + (String(msg.subject||'').trim() || '');
  ...
  function sendMailFromCompose_(){
    const ov = ensureMailOverlay_();
    if(!ov) return;

    const u = meUsername_();
    if(!u){
      toast('שגיאה', 'לא מזוהה משתמש מחובר');
      return;
    }

    const users = listActiveUsersForMail_();
    const toUsernames = Array.from(ov.querySelectorAll('[data-mailto]'))
      .filter(ch => ch && ch.checked)
      .map(ch => String(ch.getAttribute('data-mailto')||'').trim())
      .filter(Boolean);

    if(!toUsernames.length){
      toast('חסר', 'בחר/י לפחות נמען אחד');
      return;
    }

    const subject = String(ov.querySelector('#mailSubject')?.value || '').trim();
    const body = String(ov.querySelector('#mailBodyTxt')?.value || '').trim();

    if(!body){
      toast('חסר', 'תוכן ההודעה ריק');
      return;
    }

    const toNames = toUsernames.map(un => (users.find(x => x.username===un)?.name || un));

    const msg = {
      id: 'm_' + Math.random().toString(36).slice(2,10) + '_' + Date.now().toString(36),
      fromUser: u,
      fromName: meName_() || u,
      toUsers: toUsernames,
      toNames,
      ...
      body,
      createdAt: new Date().toISOString(),
      readBy: { [u]: true } // sender considered read
    };
    ...
    // reset form
    ov.querySelector('#mailSubject').value = '';
    ov.querySelector('#mailBodyTxt').value = '';
    ov.querySelectorAll('[data-mailto]').forEach(ch => { ch.checked = false; });
  ...
  function startMailPolling_(){
    stopMailPolling_();
    _mailLastUnread = unreadCount_();
    _mailPollTimer = setInterval(() => {
      const now = unreadCount_();
      if(now > _mailLastUnread){
        playMailSound_();
        toast('דואר', `נכנסה הודעה חדשה (${now})`);
      }
      _mailLastUnread = now;
      updateMailBadge_();
            // keep mail window live if open
      try{ if(_mailWin && !_mailWin.closed){ _mailWin.postMessage({ type:'gemel_mail_refresh' }, '*'); } }catch(_){ }
}, 4000);
  }
  function stopMailPolling_(){
    if(_mailPollTimer){
      clearInterval(_mailPollTimer);
      _mailPollTimer = null;
    }
  
  // Sync from mail window
  window.addEventListener('message', (ev) => {
    if(ev && ev.data && ev.data.type === 'gemel_mail_changed'){
      updateMailBadge_();
    }
  });
}
  ...
  function upsertUser_(payload){
    ...
    if(!isEdit){
      ...
      if(state.currentUser && state.currentUser.username === prevUsername){
        ...
        window.__gemelCurrentUser = state.currentUser;
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  const AUTH_SESSION_PUBLIC_KEY = 'gemel_session_pub_v1';
  ...
  function setSession_(payload){
    ...
    // Also store a minimal public session in localStorage so the mail popup can read it (even if window.opener is blocked)
    try{
      const p = payload || {};
      const pub = { username: p.username || '', role: p.role || '', displayName: p.displayName || '' };
      localStorage.setItem(AUTH_SESSION_PUBLIC_KEY, JSON.stringify(pub));
    }catch(_){}
  }
  function clearSession_(){
    try{ sessionStorage.removeItem(AUTH_SESSION_KEY); }catch(_){}
    try{ localStorage.removeItem(AUTH_SESSION_PUBLIC_KEY); }catch(_){}
  }
  ...
  function loginAttempt_(username, password){
    ...
    // Mail: badge + new-message polling
    updateMailBadge_();
    startMailPolling_();
  ...
  window.gemelLogout = function(){
    ...
    window.__gemelCurrentUser = null;
...
function renderKpisSettings(){
  ...
  if(mailBtn){
    mailBtn.addEventListener('click', () => openMail_());
  }
...
`;
  ...
  function openPrintView(proposal){
    ...
    try {
      ...
      `;
      }).join('') || `<tr><td colspan="8" class="mutedCell">אין פוליסות</td></tr>`;
      ...
      <body>
        <div class="page">
          ...
          <table class="t">
            <thead><tr><th>מבוטח</th><th>חברה</th><th>מוצר</th><th>פרמיה חודשית</th><th>סכום ביטוח</th><th>סכום פיצוי</th><th>סטטוס</th><th>נספחים לביטול</th></tr></thead>
            <tbody>
              ${oldRows}
              <tr class="sumRow">
                <td colspan="7">סה״כ</td><td class="sumGold">${money(sumOld)}</td>
              </tr>
            </tbody>
          </table>
