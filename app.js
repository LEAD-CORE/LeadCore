...
(() => {
  ...
  const UI = {
    ...
    init() {
      ...
      this.els.policySecondaryId = $("#policySecondaryId");
      this.els.policySecondaryPreview = $("#policySecondaryPreview");
      this.els.btnAddChild = $("#btnAddChild");
      this.els.policyChildrenList = $("#policyChildrenList");
      this.els.tplPolicyChildRow = $("#tplPolicyChildRow");
...
// Search (live filter on customers view)
      ...
      // Policy modal: secondary insured lookup + children rows
      this._policyLookupTimer = null;

      const updateSecondaryPreview = () => {
        const idNum = safeTrim(this.els.policySecondaryId?.value);
        const preview = this.els.policySecondaryPreview;
        if (!preview) return;

        // reset
        this.els.policyForm.dataset.secondaryCustomerId = "";
        preview.classList.remove("is-ok");
        preview.classList.add("is-empty");

        if (!idNum) {
          preview.textContent = "לא נבחר";
          return;
        }

        const found = (State.data.customers || []).find(x => safeTrim(x.idNumber) === idNum);
        if (!found) {
          preview.textContent = "לא נמצא במאגר הלקוחות";
          return;
        }

        this.els.policyForm.dataset.secondaryCustomerId = found.id;
        preview.textContent = `${found.firstName} ${found.lastName} • ${found.phone || "—"}`;
        preview.classList.remove("is-empty");
        preview.classList.add("is-ok");
      };

      if (this.els.policySecondaryId) {
        on(this.els.policySecondaryId, "input", () => {
          clearTimeout(this._policyLookupTimer);
          this._policyLookupTimer = setTimeout(updateSecondaryPreview, 180);
        });
      }

      const addChildRow = () => {
        const tpl = this.els.tplPolicyChildRow;
        const host = this.els.policyChildrenList;
        if (!tpl || !host) return;
        const node = tpl.content.firstElementChild.cloneNode(true);
        const removeBtn = node.querySelector(".policyChildRemove");
        if (removeBtn) on(removeBtn, "click", () => node.remove());
        host.appendChild(node);
      };

      if (this.els.btnAddChild) on(this.els.btnAddChild, "click", addChildRow);
...
// Policy modal submit
        ...
        // Secondary insured (by TZ -> customer in DB)
        const secondaryIdNumber = safeTrim(fd.get("secondaryIdNumber"));
        let secondaryInsured = null;
        if (secondaryIdNumber) {
          const sc = (State.data.customers || []).find(x => safeTrim(x.idNumber) === secondaryIdNumber);
          if (!sc) {
            notify("מבוטח משני לא נמצא במאגר הלקוחות (בדוק ת״ז).", "warn");
            return;
          }
          secondaryInsured = { customerId: sc.id, idNumber: sc.idNumber, firstName: sc.firstName, lastName: sc.lastName, phone: sc.phone || "" };
        }

        // Children under 16
        const childrenUnder16 = [];
        const rows = Array.from(this.els.policyChildrenList?.querySelectorAll?.(".policyChildRow") || []);
        const today = new Date();
        const maxBirth = new Date(today.getFullYear() - 16, today.getMonth(), today.getDate()); // must be after this date
        for (const row of rows) {
          const get = (k) => safeTrim(row.querySelector(`[data-child="${k}"]`)?.value);
          const child = {
          ...
          const anyFilled = !!(child.firstName || child.lastName || child.idNumber || child.birthDate);
          if (!anyFilled) continue;

          if (!child.firstName || !child.lastName || !child.idNumber || !child.birthDate) {
          ...
          if (isNaN(bd.getTime())) {
            notify("תאריך לידה לילד אינו תקין.", "warn");
            return;
          }
          // under 16 => birthDate must be אחרי/שווה maxBirth
          if (bd < maxBirth) {
            notify("ילדים מתחת לגיל 16 בלבד. בדוק תאריך לידה.", "warn");
            return;
          }

          childrenUnder16.push(child);
        }
        ...
        c.policies ||= [];
        c.policies.unshift({ id: "p_" + uid(), type, company, premium, status: "active", renewAt, secondaryInsured, childrenUnder16 });
        c.updatedAt = nowISO();
...
this.renderPolicies();
    ...
    openPolicyModal() {
      ...
      // reset policy extras (scoped; no side-effects)
      try {
        if (this.els.policyForm) this.els.policyForm.dataset.secondaryCustomerId = "";
        if (this.els.policySecondaryId) this.els.policySecondaryId.value = "";
        if (this.els.policySecondaryPreview) {
          this.els.policySecondaryPreview.textContent = "לא נבחר";
          this.els.policySecondaryPreview.classList.add("is-empty");
          this.els.policySecondaryPreview.classList.remove("is-ok");
        }
        if (this.els.policyChildrenList) this.els.policyChildrenList.innerHTML = "";
      } catch (_) {}
