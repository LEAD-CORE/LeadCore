...
(() => {
  ...
  const UI = {
    ...
    init() {
      ...
      this.els.btnCustomersSearch = $("#btnCustomersSearch");
      ...
      // Search (לקוחות בלבד)
      if (this.els.btnCustomersSearch) this.els.btnCustomersSearch.addEventListener("click", () => this.customerSearchGo());
      if (this.els.customersSearch) this.els.customersSearch.addEventListener("keydown", (e) => {
        if (e.key === "Enter") { e.preventDefault(); this.customerSearchGo(); }
      });
      // Form submit
    ...
    customerSearchGo() {
      // הולך לתפריט לקוחות + מחפש לפי שם מלא / טלפון / ת״ז
      const qRaw = safeTrim(this.els.customersSearch?.value);
      const q = qRaw.toLowerCase();

      // אם אין חיפוש, פשוט עובר ללקוחות
      ...
      const matchIndex = customers.findIndex(c => {
        ...
        const idn = String(c.idNumber || "").replace(/\D/g, "");
        const qDigits = qRaw.replace(/\D/g, "");

        // עדיפות: ת״ז/טלפון מדויק, אחרת התאמה בשם מלא (כולל)
        if (qDigits && (qDigits === phone || qDigits === idn)) return true;
        return fullName.includes(q);
      });

      if (matchIndex === -1) {
        alert("לא נמצא לקוח תואם. נסה שם מלא / טלפון / ת״ז.");
        return;
      }

      // מקפיצים לראש הרשימה
      const [c] = customers.splice(matchIndex, 1);
      customers.unshift(c);

      // רנדר + גלילה לראש
      this.renderCustomers();

      // היילייט רגעי לשורה הראשונה
      const firstRow = this.els.customersTbody?.querySelector("tr");
      if (firstRow) {
        firstRow.style.outline = "3px solid rgba(58,108,255,.35)";
        firstRow.style.borderRadius = "12px";
        setTimeout(() => { 
          firstRow.style.outline = ""; 
          firstRow.style.borderRadius = ""; 
        }, 900);
      }

      const tableWrap = this.els.customersTbody?.closest(".tableWrap");
