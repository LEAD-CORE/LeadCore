...
this.els.btnUserEditSave = $("#btnUserEditSave");
      ...
      // Customer file: export customer details to PDF (print-to-PDF for best Hebrew/RTL)
      const _buildCustomerPrintHtml = (c) => {
        const fullName = `${safeTrim(c?.firstName)} ${safeTrim(c?.lastName)}`.trim() || "—";
        const idn = safeTrim(c?.idNumber || c?.tz || c?.identity || c?.nationalId) || safeTrim(c?.id) || "—";
        const phone = safeTrim(c?.phone) || "—";
        const email = safeTrim(c?.email) || "—";
        const address = safeTrim(c?.address) || "—";
        const birthDate = safeTrim(c?.birthDate) || "—";
        const occupation = safeTrim(c?.occupation) || "—";
        const hmo = safeTrim(c?.hmo) || "—";
        const smoker = safeTrim(c?.smoker) || "—";
        const updatedAt = safeTrim(c?.updatedAt) || "—";

        const policies = Array.isArray(c?.policies) ? c.policies : [];
        const polRows = policies.length
          ? policies.map(p => {
              const type = safeTrim(p?.type) || "—";
              const company = safeTrim(p?.company) || "—";
              const premium = Number(p?.premium || 0) || 0;
              const status = safeTrim(p?.status) || "—";
              const renewAt = safeTrim(p?.renewAt) || "—";
              return `
                <tr>
                  <td>${escapeHtml(company)}</td>
                  <td>${escapeHtml(type)}</td>
                  <td>₪${escapeHtml(premium.toLocaleString("he-IL"))}</td>
                  <td>${escapeHtml(status)}</td>
                  <td>${escapeHtml(renewAt)}</td>
                </tr>`;
            }).join("")
          : `<tr><td colspan="5" style="text-align:center; padding:14px; color:#64748b;">אין ביטוחים ברשימה</td></tr>`;
...
<html lang="he" dir="rtl">
  <head>
    ...
    <title>LeadCore • ${escapeHtml(fullName)} • PDF</title>
    <style>
      :root{ --ink:#0a1220; --muted:#475569; --line:#e2e8f0; --card:#ffffff; }
      *{ box-sizing:border-box; }
      body{ margin:0; font-family: Arial, Helvetica, sans-serif; color:var(--ink); background:#fff; }
      .wrap{ padding:24px; }
      .top{ display:flex; align-items:flex-start; justify-content:space-between; gap:16px; border-bottom:1px solid var(--line); padding-bottom:14px; margin-bottom:16px; }
      .brand{ font-weight:900; font-size:14px; color:#2b4b8a; }
      ...
      .k{ font-size:12px; color:#2b4b8a; font-weight:900; }
      .v{ margin-top:6px; font-size:13px; font-weight:900; }
      .v.ltr{ direction:ltr; unicode-bidi: plaintext; text-align:left; }
      .sectionTitle{ margin:18px 0 10px; font-weight:950; color:#2b4b8a; }
      table{ width:100%; border-collapse:collapse; }
      th, td{ border-bottom:1px solid var(--line); padding:10px 8px; font-size:12px; text-align:right; vertical-align:top; }
      th{ color:#2b4b8a; font-weight:950; background:#f8fafc; }
      @media print{ .wrap{ padding:0; } .top{ margin-bottom:10px; } }
    </style>
  </head>
  <body>
    <div class="wrap">
      <div class="top">
        <div>
          <div class="brand">LeadCore CRM</div>
          <h1>${escapeHtml(fullName)}</h1>
          <div class="sub">ת"ז: ${escapeHtml(idn)} • עודכן לאחרונה: ${escapeHtml(updatedAt)}</div>
        </div>
        <div class="sub">נוצר מהמערכת</div>
      </div>

      <div class="grid">
        <div class="card"><div class="k">טלפון</div><div class="v ltr">${escapeHtml(phone)}</div></div>
        <div class="card"><div class="k">מייל</div><div class="v ltr">${escapeHtml(email)}</div></div>
        <div class="card"><div class="k">כתובת</div><div class="v">${escapeHtml(address)}</div></div>
        <div class="card"><div class="k">תאריך לידה</div><div class="v">${escapeHtml(birthDate)}</div></div>
        <div class="card"><div class="k">עיסוק</div><div class="v">${escapeHtml(occupation)}</div></div>
        <div class="card"><div class="k">קופת חולים</div><div class="v">${escapeHtml(hmo)}</div></div>
        <div class="card"><div class="k">מעשן</div><div class="v">${escapeHtml(smoker)}</div></div>
        <div class="card"><div class="k">נציג מטפל</div><div class="v">${escapeHtml(safeTrim(c?.assignedAgent || c?.agent || "—"))}</div></div>
      </div>

      <div class="sectionTitle">ביטוחים</div>
      <div class="card">
        <table>
          <thead>
            <tr>
    ...
    <script>
      ...
      on(this.els.cfPdfBtn, "click", () => {
        const c = getOpenCustomer();
        if (!c) return notify("לא נבחר לקוח.", "warn");
        const html = _buildCustomerPrintHtml(c);
        const w = window.open("", "_blank", "noopener,noreferrer");
        if (!w) return notify("הדפדפן חסם חלון קופץ. אפשר לאפשר Pop-ups ואז לנסות שוב.", "warn");
        try {
          w.document.open();
