...
// Search (live filter on customers view)
    ...
    openModal() {
      ...
      this.updateCreateCustomerUX_();

      this.updateCreateCustomerUX_();
      try { const root=this.els.modalCustomer; root.querySelectorAll('.lcStep').forEach((x,i)=>x.classList.toggle('is-active', i===0)); root.querySelector('#step-personal')?.scrollIntoView({block:'start'}); } catch(_){ }
    ...
    // ===== Premium Create Customer UX =====
    bindCreateCustomerUX_(){
      if (this._createUxBound) return;
      this._createUxBound = true;
      const root = this.els.modalCustomer;
      ...
      root.querySelectorAll("[data-step-go]").forEach(btn=>{
        on(btn, "click", () => {
          const sel = btn.getAttribute("data-step-go");
          const target = sel ? root.querySelector(sel) : null;
          if(target){
            target.scrollIntoView({ behavior: "smooth", block: "start" });
            // active state
            root.querySelectorAll(".lcStep").forEach(x=>x.classList.remove("is-active"));
            btn.classList.add("is-active");
          }
        });
      });

      const update = () => this.updateCreateCustomerUX_();
      on(form, "input", update);
      on(form, "change", update);

      // light phone formatting on blur (safe)
      const phoneEl = form.querySelector("input[name='phone']");
      if(phoneEl){
        on(phoneEl, "blur", () => {
          const v = safeTrim(phoneEl.value);
          if(!v) return;
          // keep digits only, then format 05X-XXXXXXX if possible
          const d = v.replace(/\D/g,"");
          if(d.length === 10 && d.startsWith("05")){
            phoneEl.value = d.slice(0,3) + "-" + d.slice(3);
          }else{
            phoneEl.value = v;
          }
          this.updateCreateCustomerUX_();
    ...
    updateCreateCustomerUX_(){
      const root = this.els.modalCustomer;
      const form = this.els.customerForm;
      if(!root || !form) return;

      const get = (name) => safeTrim(form.querySelector(`[name="${name}"]`)?.value || "");
      const first = get("firstName");
      const last  = get("lastName");
      const phone = get("phone");
      const idn   = get("idNumber");
      const agent = get("assignedAgent");

      const fullName = safeTrim((first + " " + last).trim()) || "לקוח חדש";
      ...
      setTxt("createPreviewName", fullName);
      setTxt("createPreviewPhone", phone || "—");
      setTxt("createPreviewId", idn || "—");
      setTxt("createPreviewAgent", agent || "—");
      setTxt("createAgentLine", agent || "—");
      ...
      const keyFields = [
        ...
        ["כתובת", get("address")],
        ["אימייל", get("email")],
        ["תאריך לידה", get("birthDate")],
        ["עיסוק", get("occupation")],
        ["קופת חולים", get("hmo")],
        ["נציג מטפל", agent]
      ];

      const filled = keyFields.filter(x => !!x[1]).length;
      const pct = Math.round((filled / keyFields.length) * 100);

      const pb = root.querySelector("#createProgressBar");
      if(pb) pb.style.width = pct + "%";
      setTxt("createProgressText", pct + "%");

      // status chip
      const status = required.every(x => !!x[1]) ? "מוכן לשמירה" : "טיוטה";
      setTxt("createStatusChip", status);

      // missing list (show only the important ones)
      const missing = keyFields.filter(x => !x[1]).map(x => x[0]);
      const ul = root.querySelector("#createMissingList");
      if(ul){
        ul.innerHTML = missing.slice(0,6).map(t => `<li>${escapeHtml(t)}</li>`).join("") || "<li>הכל נראה טוב ✅</li>";
      }
    },
closeModal() {
