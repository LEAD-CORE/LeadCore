...
// Set your GOFI module URL (GitHub Pages) here when ready
  ...
  function setRoute(r){
    ...
    // Mail is a docked panel (not a full route)
    if(r === 'mail'){ toggleMailPanel_(); return; }
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  // ---------- Internal Mail (LocalStorage) ----------
  const MAIL_LS_KEY = 'gemel_mail_v1';
  const MAIL_SEEN_KEY = 'gemel_mail_seen_v1'; // per-session baseline for new-mail detection
  let mailTab = 'inbox';
  let mailOpen = false;
  ...
  function mailRecipientsForUsers_(names){
    return (names||[]).map(n => ({ name:String(n||'').trim(), read:false, readAt:'' })).filter(x => x.name);
  }
  function normalizeName_(s){ return String(s||'').replace(/\s+/g,' ').trim(); }

  function unreadCountForAgent_(agent){
    const me = normalizeName_(agent || state.agentName);
    if(!me) return 0;
    const list = loadMail_();
    let n = 0;
    for(const m of list){
      const rec = (m && Array.isArray(m.recipients)) ? m.recipients : [];
      const hit = rec.find(r => normalizeName_(r.name) === me);
      if(hit && hit.read !== true) n++;
    }
    return n;
  }

  function latestInboundTsForAgent_(agent){
    const me = normalizeName_(agent || state.agentName);
    const list = loadMail_();
    let t = 0;
    for(const m of list){
  ...
  function updateMailBadge_(){
    const n = unreadCountForAgent_(state.agentName);
    const mailBtn = document.querySelector('.navItem[data-route="mail"]');
    if(mailBadgeEl){
      mailBadgeEl.textContent = String(n);
      mailBadgeEl.style.display = n > 0 ? 'inline-flex' : 'none';
    }
    if(mailBtn){
      mailBtn.classList.toggle('mailActive', n > 0);
    }
  }

  function toggleMailPanel_(forceOpen){
    mailOpen = (typeof forceOpen === 'boolean') ? forceOpen : !mailOpen;
    try{
      document.body.classList.toggle('mailOpen', !!mailOpen);
    }catch(_){}
    if(mailPanelEl){
      if(mailOpen){
        mailPanelEl.removeAttribute('hidden');
      }else{
        mailPanelEl.setAttribute('hidden','');
      }
    }
    const mailBtn = document.querySelector('.navItem[data-route="mail"]');
    if(mailBtn) mailBtn.classList.toggle('active', !!mailOpen);
    renderMailPanel_();
  }

  if(mailCloseBtn){
    mailCloseBtn.addEventListener('click', () => toggleMailPanel_(false));
  }

  function setMailTab_(t){
    mailTab = (t === 'compose') ? 'compose' : 'inbox';
    $$('.mailTab').forEach(b => b.classList.toggle('active', b.dataset.mailtab === mailTab));
    renderMailPanel_();
  }

  // wire tab buttons once
  document.addEventListener('click', (e) => {
    const btn = e.target && e.target.closest ? e.target.closest('.mailTab') : null;
    if(!btn) return;
    const t = String(btn.dataset.mailtab||'').trim();
    if(!t) return;
    setMailTab_(t);
  });

  function markRead_(msgId){
    const me = normalizeName_(state.agentName);
    if(!me) return;
    const list = loadMail_();
    const m = list.find(x => x && x.id === msgId);
    if(!m) return;
    if(!Array.isArray(m.recipients)) m.recipients = [];
    const r = m.recipients.find(x => normalizeName_(x.name) === me);
    if(r && r.read !== true){
      r.read = true;
  ...
  function renderInbox_(){
    const me = normalizeName_(state.agentName);
    const list = loadMail_()
      .filter(m => m && Array.isArray(m.recipients) && m.recipients.some(r => normalizeName_(r.name) === me))
      .slice()
      .sort((a,b)=> new Date(b.createdAt||0).getTime() - new Date(a.createdAt||0).getTime());

    if(!list.length){
      return `<div class="sideHint">אין הודעות כרגע.</div>`;
    }

    const itemHtml = (m) => {
      const rec = (m.recipients||[]).find(r => normalizeName_(r.name) === me) || {};
      const unread = rec.read !== true;
      return `
        <div class="mailItem ${unread?'unread':''}" data-mail-open="${escapeAttr(m.id)}">
          <div class="mailMeta">
            <div><b>${escapeHtml(m.from||'')}</b></div>
            <div>${escapeHtml(fmtMailTime_(m.createdAt||''))}</div>
          </div>
          <div class="mailSubject">${escapeHtml(m.subject||'(ללא נושא)')}</div>
          <div class="mailBodyText" style="max-height:56px;overflow:hidden;mask-image:linear-gradient(#000, transparent)">${escapeHtml(m.body||'')}</div>
        </div>
      `;
    };

    return `<div class="mailList">${list.map(itemHtml).join('')}</div>`;
  }

  function renderCompose_(){
    const users = (typeof loadUsers_ === 'function' ? loadUsers_() : (typeof getUsers_ === 'function' ? getUsers_() : []))
      .filter(u => u && u.active !== false)
      .map(u => normalizeName_(u.displayName||u.username||''))
      .filter(Boolean);

    const me = normalizeName_(state.agentName);
    const list = users.filter(n => n); // allow sending to self if wanted

    return `
      <div class="mailCompose">
        <div class="mailRecipients" id="mailRecipientsBox">
          <div class="rTop">
            <div class="rTitle">נמענים (אפשר לבחור כמה)</div>
          </div>
          <input type="text" class="rSearch" id="mailRecSearch" placeholder="חיפוש נציג...">
          <div class="rList" id="mailRecList">
            ${list.map(n => `
              <label>
                <input type="checkbox" class="mailRecChk" value="${escapeAttr(n)}">
                <span>${escapeHtml(n)}${(n===me)?' (אני)':''}</span>
              </label>
            `).join('')}
          </div>
        </div>
        ...
        <div class="mailField">
          <div class="lbl">נושא</div>
          <input id="mailSubject" type="text" placeholder="לדוגמה: תזכורת/עדכון לקוח">
        </div>

        <div class="mailField">
          <div class="lbl">תוכן הודעה</div>
          <textarea id="mailText" placeholder="כתוב/י כאן את ההודעה..."></textarea>
        </div>
  ...
  function renderMailPanel_(){
    if(!mailPanelEl || !mailBodyEl) return;

    // Ensure correct tab UI state
    $$('.mailTab').forEach(b => b.classList.toggle('active', b.dataset.mailtab === mailTab));

    mailBodyEl.innerHTML = (mailTab === 'compose') ? renderCompose_() : renderInbox_();

    // Wire compose actions
    const sendBtn = $('#mailSendBtn', mailPanelEl);
    const clearBtn = $('#mailClearBtn', mailPanelEl);
    const search = $('#mailRecSearch', mailPanelEl);
    const recList = $('#mailRecList', mailPanelEl);

    if(search && recList){
      search.addEventListener('input', () => {
        const q = normalizeName_(search.value).toLowerCase();
        $$('.mailRecChk', recList).forEach(chk => {
          const label = chk.closest('label');
          const name = normalizeName_(chk.value).toLowerCase();
          if(label) label.style.display = (!q || name.includes(q)) ? '' : 'none';
        });
      });
    }
    ...
    if(clearBtn){
      clearBtn.addEventListener('click', () => {
        const subj = $('#mailSubject', mailPanelEl);
        const txt = $('#mailText', mailPanelEl);
        if(subj) subj.value = '';
        if(txt) txt.value = '';
        $$('.mailRecChk', mailPanelEl).forEach(chk => chk.checked = false);
      });
    }

    if(sendBtn){
      sendBtn.addEventListener('click', () => {
        const picked = $$('.mailRecChk', mailPanelEl).filter(chk => chk.checked).map(chk => normalizeName_(chk.value));
        const subj = normalizeName_($('#mailSubject', mailPanelEl)?.value || '');
        const body = String($('#mailText', mailPanelEl)?.value || '').trim();
        ...
        if(!body){
          toast('חסר', 'כתוב/י תוכן הודעה');
          return;
        }

        const msg = {
          id: 'm_' + Math.random().toString(36).slice(2,10) + '_' + Date.now().toString(36),
          from: normalizeName_(state.agentName || ''),
          to: picked.slice(),
          subject: subj || '(ללא נושא)',
          body,
          createdAt: new Date().toISOString(),
          recipients: mailRecipientsForUsers_(picked)
        };
        ...
        // Clear fields
        const subjEl = $('#mailSubject', mailPanelEl);
        const bodyEl = $('#mailText', mailPanelEl);
        if(subjEl) subjEl.value = '';
        if(bodyEl) bodyEl.value = '';
        $$('.mailRecChk', mailPanelEl).forEach(chk => chk.checked = false);

        toast('נשלח', 'ההודעה נשלחה לנמענים שנבחרו');
        updateMailBadge_();
        setMailTab_('inbox');
      });
    }
    ...
    // Wire inbox open
    $$('[data-mail-open]', mailPanelEl).forEach(card => {
      card.addEventListener('click', () => {
        const id = card.dataset.mailOpen;
        if(!id) return;
        openMailMessage_(id);
      });
    });
  ...
  function openMailMessage_(msgId){
    const me = normalizeName_(state.agentName);
    const list = loadMail_();
    const m = list.find(x => x && x.id === msgId);
    if(!m) return;
    ...
    // Replace body area with a single message view (still inside panel)
    if(!mailBodyEl) return;
    mailBodyEl.innerHTML = `
      <div class="mailItem" style="cursor:default">
        <div class="mailMeta">
          <div><b>מאת:</b> ${escapeHtml(m.from||'')}</div>
          <div>${escapeHtml(fmtMailTime_(m.createdAt||''))}</div>
        </div>
        <div class="mailMeta" style="margin-top:-2px">
          <div><b>אל:</b> ${escapeHtml((m.to||[]).join(', '))}</div>
          <div></div>
        </div>
        <div class="mailSubject" style="margin-top:8px">${escapeHtml(m.subject||'(ללא נושא)')}</div>
        <div class="mailBodyText" style="margin-top:8px">${escapeHtml(m.body||'')}</div>

        <div class="mailActions" style="margin-top:12px">
          <button type="button" class="btn btnSoft" id="mailBackBtn">חזרה לדואר נכנס</button>
        </div>
      </div>
    `;
    const back = $('#mailBackBtn', mailPanelEl);
    if(back) back.addEventListener('click', () => { mailTab='inbox'; renderMailPanel_(); });
  }

  // ---------- New mail detection + sound ----------
  function playMailSound_(){
    try{
      ...
      o1.frequency.setValueAtTime(660, now + 0.12);
      ...
      o1.start(now);
      o1.stop(now + 0.24);
      o1.onended = () => { try{ ctx.close(); }catch(_){} };
    }catch(_){}
  }

  function pollNewMail_(){
    const me = normalizeName_(state.agentName);
    if(!me) return;

    const key = MAIL_SEEN_KEY + ':' + me;
    let seen = 0;
    try{ seen = Number(sessionStorage.getItem(key) || '0') || 0; }catch(_){}

    const latest = latestInboundTsForAgent_(me);
    const unread = unreadCountForAgent_(me);

    // If never set, initialize (don't beep on first load)
    if(!seen){
      try{ sessionStorage.setItem(key, String(latest || Date.now())); }catch(_){}
      updateMailBadge_();
      return;
    }

    if(latest > seen){
      try{ sessionStorage.setItem(key, String(latest)); }catch(_){}
      updateMailBadge_();

      // If there is at least one unread message, notify
      if(unread > 0){
        playMailSound_();
        toast('מייל חדש', 'התקבלה הודעה חדשה בדואר הפנימי');
        // if mail is open and on inbox, refresh list
        if(mailOpen && mailTab === 'inbox'){
          renderMailPanel_();
        }
      }
    }else{
      // keep badge in sync anyway
      updateMailBadge_();
    }
  }

  // Start polling after login (render() is called anyway, but we want periodic updates)
  setInterval(() => {
    try{
      if(state && state.currentUser) pollNewMail_();
    }catch(_){}
  }, 3500);
