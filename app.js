...
(() => {
  ...
  // ---------------------------
  // Non-blocking toast (instead of alert on boot)
  // ---------------------------
  const toast = (title, text = "", type = "ok", ms = 3200) => {
  ...
  const normalizeGsUrl = (raw) => {
    ...
    if (!/^https?:\/\//i.test(cleaned) && cleaned.includes("script.google.com")) {
  ...
  const Storage = {
    ...
    async loadSheets() {
      const raw = normalizeGsUrl(this.gsUrl);
      if (!raw) return { ok: false, error: "אין כתובת Web App" };
      ...
      try {
        ...
        if (!res.ok) return { ok: false, error: `שגיאת רשת (${res.status})` };
...
on(this.els.btnSearch, "click", () => {
  ...
  this.renderCustomers();
      ...
      on(this.els.customerForm, "submit", async (e) => {
        ...
        if (!customer.firstName || !customer.lastName || !customer.phone) {
          toast("חסר מידע", "נא למלא שם פרטי, שם משפחה וטלפון.", "warn");
          return;
        }
      ...
      on(this.els.gsUrl, "change", () => {
        Storage.gsUrl = normalizeGsUrl(this.els.gsUrl.value);
      ...
      on(this.els.btnTestConn, "click", async () => {
        ...
        toast(r.ok ? "חיבור תקין ✔" : "חיבור נכשל", r.ok ? "" : (r.error || "שגיאה"), r.ok ? "ok" : "err");
      });
      ...
      on(this.els.btnResetLocal, "click", () => {
        if (!UI.els._resetArmed) { UI.els._resetArmed = true; toast("איפוס מקומי", "לחץ שוב בתוך 5 שניות כדי לאפס את ה-Local.", "warn", 5000); setTimeout(()=>UI.els._resetArmed=false, 5200); return; } UI.els._resetArmed = false;
        localStorage.removeItem(Storage.localKey);
        State.set(defaultState());
        this.renderAll();
        toast("אופס ✅", "האחסון המקומי אופס.", "ok");
      });
    },
  ...
  const App = {
    async boot() {
      // restore settings
      Storage.gsUrl = normalizeGsUrl(localStorage.getItem("LEAD_CORE_GS_URL") || "");
      Storage.mode = (localStorage.getItem("LEAD_CORE_MODE") || "local");
      if (Storage.mode !== "local" && Storage.mode !== "sheets") Storage.mode = "local";
      if (UI.els.gsUrl) UI.els.gsUrl.value = Storage.gsUrl;

      if (UI.els.modeLocal) UI.els.modeLocal.classList.toggle("is-active", Storage.mode === "local");
      if (UI.els.modeSheets) UI.els.modeSheets.classList.toggle("is-active", Storage.mode === "sheets");
// load
