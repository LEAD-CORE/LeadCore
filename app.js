...
(() => {
  ...
  const SLA_RULES = [
    { action: "חזרה לשיקוף לקוח", days: 1 },
    { action: "טיפול גבייה", days: 7 },
    { action: "ממתין לחתימות מבוטח+סוכן", days: 2 },
    { action: "שירות שיחה חוזרת ללקוח", days: 1 },

    // Keep a few legacy options (safe defaults)
    { action: "איסוף מסמכים", days: 2 },
    { action: "הצעת מחיר", days: 3 },
    { action: "תיאום פגישה", days: 3 },
    { action: "חידוש פוליסה", days: 14 },
    { action: "מעקב לקוח", days: 7 }
  ];
  ...
  const computeDueAtFromAction = (action, baseDate = new Date()) => {
    ...
    return toISODateTimeLocal(base);
  };
...
if (!out.agents.length) {
  ...
  const Storage = {
    ...
    startLiveSync() {
      ...
      // scheduled task checker (time-based)
      this._dueTimer = setInterval(() => { try { this.checkDueTasks(); } catch (_) {} }, 15000);
      setTimeout(() => { try { this.checkDueTasks(); } catch (_) {} }, 800);
    },
  ...
  const UI = {
    ...
    init() {
      ...
      this.els.kpiDueList = $("#kpiDueList");
      this.els.viewDashboard = $("#view-dashboard");
...
this.els.btnUserEditSave = $("#btnUserEditSave");
      ...
      // Dashboard due tasks: open customer file from scheduled list
      on(this.els.viewDashboard, "click", (e) => {
        const btn = e?.target?.closest?.("button[data-open-customer]");
        if (!btn) return;
        const id = safeTrim(btn.dataset.openCustomer);
        if (id) this.openCustomerFull(id);
      ...
      try {
        if (this.els.cfNextAction) {
          this.els.cfNextAction.innerHTML = SLA_RULES.map(r =>
            `<option value="${escapeHtml(r.action)}">${escapeHtml(r.action)} • ${escapeHtml(String(r.days))} ימים</option>`
          ).join("");
        ...
        if (this.els.cfDueAt) {
          const raw = safeTrim(c.dueAt);
          const fallback = computeDueAtFromAction(safeTrim(c.nextAction) || "מעקב לקוח", new Date());
          const val = raw ? (raw.includes("T") ? raw.slice(0,16) : `${raw}T09:00`) : fallback;
          this.els.cfDueAt.value = val;
        }
...
this.renderPolicies();
    ...
    _slaBucket(dueAt){
      ...
      // Time-aware: overdue/now => red, today later => amber, future => green
      if (due.getTime() <= now.getTime()) return "red";
      if (this._sameDay(due, now)) return "amber";
      return "green";
    },
    ...
    // ===========================
    // Scheduled tasks (customer dueAt + nextAction)
    // ===========================
    _scopeCustomersForUser(customers){
      const list = Array.isArray(customers) ? customers : [];
      // Agent sees only his customers; manager sees all
      try {
        if (Auth.current && Auth.current.role === "agent") {
          const agentName = this._getActiveAgentName();
          return list.filter(c => safeTrim(c.assignedAgent) && safeTrim(c.assignedAgent) === agentName);
        }
      } catch (_) {}
      return list;
    },

    _getDueTaskItems(customers, opts = {}){
      const soonHours = Number(opts.soonHours || 0); // include upcoming window
      const now = new Date();
      const soonMs = soonHours > 0 ? soonHours * 60 * 60 * 1000 : 0;

      return (Array.isArray(customers) ? customers : [])
        .map(c => {
          const action = safeTrim(c.nextAction);
          const dueAt = safeTrim(c.dueAt);
          const due = dueAt ? this._parseDateSafe(dueAt) : null;
          if (!action || !due) return null;
          const ts = due.getTime();
          const isDue = ts <= now.getTime();
          const isSoon = !isDue && soonMs ? (ts <= (now.getTime() + soonMs)) : false;
          if (!isDue && !isSoon) return null;
          const name = safeTrim(`${c.firstName || ""} ${c.lastName || ""}`) || "לקוח ללא שם";
          return { id: c.id, name, action, dueAt, dueTs: ts, isDue };
        })
        .filter(Boolean)
        .sort((a,b) => (a.isDue === b.isDue ? 0 : a.isDue ? -1 : 1) || (a.dueTs - b.dueTs) || a.name.localeCompare(b.name, "he"));
    },

    checkDueTasks(){
      // Avoid repeating the same notification during a session
      if (!this._dueNotified) this._dueNotified = new Set();
      const customers = this._scopeCustomersForUser(State.data.customers || []);
      const dueNow = this._getDueTaskItems(customers, { soonHours: 0 }).filter(x => x.isDue);

      // Show at most 1 notification per tick
      const hit = dueNow.find(it => !this._dueNotified.has(`${it.id}|${it.dueAt}`));
      if (!hit) return;

      this._dueNotified.add(`${hit.id}|${hit.dueAt}`);
      notify(`⏰ משימה ללקוח: ${hit.name} • ${hit.action}`, "warn");
    },
    ...
    renderMyFlow(){
      ...
      const fmtDue = (v) => {
        ...
        if (!d) return "—";
        try { return d.toLocaleString("he-IL", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit", year: "numeric" }); }
        catch (_) { return String(v).replace("T"," "); }
      };
...
openPolicyActionModal(action, policyId, policyLabel) {
    ...
    renderDashboard() {
      ...
      // Scheduled tasks KPI (due/soon)
      const scopeCustomers = this._scopeCustomersForUser(customers);
      const dueItems = this._getDueTaskItems(scopeCustomers, { soonHours: 24 });
      const dueNowCount = dueItems.filter(x => x.isDue).length;

      // KPI value = how many due/overdue right now
      if (this.els.kpiUpdated) this.els.kpiUpdated.textContent = dueNowCount ? `${dueNowCount} לקוחות` : "—";

      // Small list inside KPI card
      if (this.els.kpiDueList) {
        const top = dueItems.slice(0, 4);

        const fmtDue = (iso) => {
          const d = this._parseDateSafe(iso);
          if (!d) return "—";
          try { return d.toLocaleString("he-IL", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit", year: "numeric" }); } catch(_) { return String(iso).replace("T"," "); }
        };

        if (!top.length) {
          this.els.kpiDueList.innerHTML = `<div class="muted small">אין משימות מתוזמנות ב-24 השעות הקרובות.</div>`;
        } else {
          this.els.kpiDueList.innerHTML = top.map(it => `
            <div class="kpiTask">
              <div class="kpiTask__meta">
                <div class="kpiTask__name">${escapeHtml(it.name)}</div>
                <div class="kpiTask__sub">${escapeHtml(it.action)} • ${fmtDue(it.dueAt)}</div>
              </div>
              <button type="button" class="btn kpiTask__btn" data-open-customer="${escapeHtml(it.id)}">פתיחה</button>
            </div>
          `).join("");
        }
      }
