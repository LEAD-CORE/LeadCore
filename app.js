...
;
  ...
  const Storage = {
    ...
    // resolved (final) URL after Google redirect (script.google.com -> script.googleusercontent.com)
    _resolvedUrl: null,
    async _ensureResolvedUrl() {
      // Some browsers block POST if it redirects; resolve once via ping so we POST to the final URL directly.
      if (this._resolvedUrl) return this._resolvedUrl;
      try {
        const u = new URL(this.gsUrl);
        u.searchParams.set("action", "ping");
        // cache-bust
        u.searchParams.set("_", String(Date.now()));
        const r = await fetch(u.toString(), { method: "GET", redirect: "follow", cache: "no-store" });
        this._resolvedUrl = r.url || this.gsUrl;
      } catch (_) {
        this._resolvedUrl = this.gsUrl;
      }
    ...
    async loadSheets() {
      ...
      const base = await this._ensureResolvedUrl();
...
const submitBtn = this.els.customerForm?.querySelector?.("button[type='submit']");
        ...
        // ✅ Close create-customer modal and open customer file immediately
        try{
            this.closeModal && this.closeModal();
            this.openCustomerFull && this.openCustomerFull(customer.id);
