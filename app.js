...
(() => {
  ...
  const UI = {
    ...
    init() {
      ...
      this.els.secondaryIdInput = $("#secondaryIdInput");
      this.els.secondaryPreview = $("#secondaryPreview");
...
// Search (live filter on customers view)
      ...
      // Policy modal submit
        ...
        const secondaryId = safeTrim(fd.get("secondaryId"));
        let secondarySnapshot = null;
        if (secondaryId) {
          const hit = (State.data.customers || []).find(x => safeTrim(x.idNumber) === secondaryId) || null;
          if (!hit) {
            notify("מבוטח משני: לא נמצא לקוח עם הת״ז הזו.", "warn");
            return;
          }
          const nm = safeTrim(hit.fullName) || [safeTrim(hit.firstName), safeTrim(hit.lastName)].filter(Boolean).join(" ").trim();
          secondarySnapshot = { idNumber: secondaryId, name: nm || "מבוטח משני", phone: safeTrim(hit.phone) };
        }

        // Children under 16 (optional, multiple)
        const children = [];
        const rows = this.els.childrenList ? Array.from(this.els.childrenList.querySelectorAll(".childRow")) : [];
        const today = new Date();
        const maxBirth = new Date(today.getFullYear() - 16, today.getMonth(), today.getDate());

        for (const row of rows) {
          const firstName = safeTrim(row.querySelector('[data-child="first"]')?.value);
          const lastName  = safeTrim(row.querySelector('[data-child="last"]')?.value);
          const idNumber  = safeTrim(row.querySelector('[data-child="id"]')?.value);
          const birthDate = safeTrim(row.querySelector('[data-child="birth"]')?.value);
          ...
          if (!(firstName && lastName && idNumber && birthDate)) {
            notify("ילדים: נא למלא את כל השדות בכל שורה (שם פרטי/שם משפחה/ת״ז/תאריך לידה).", "warn");
            return;
          }

          const bd = new Date(birthDate);
          if (isNaN(bd.getTime())) {
            notify("ילדים: תאריך לידה לא תקין.", "warn");
            return;
          }
          // must be under 16 => birthDate > maxBirth
          if (bd <= maxBirth) {
            notify("ילדים: ניתן להוסיף רק ילדים מתחת לגיל 16.", "warn");
            return;
          }

          children.push({ firstName, lastName, idNumber, birthDate });
        }
        ...
        c.policies ||= [];
        c.policies.unshift({ id: "p_" + uid(), type, company, premium, status: "active", renewAt, secondary: secondarySnapshot, children });
        c.updatedAt = nowISO();
      ...
      // Policy modal extras: Secondary insured lookup + children list (SCOPED)
      const findCustomerByIdNumber = (idNum) => {
        const z = safeTrim(idNum);
        if (!z) return null;
        return (State.data.customers || []).find(c => safeTrim(c.idNumber) === z) || null;
      };

      const updateSecondaryPreview = () => {
        if (!this.els.secondaryIdInput || !this.els.secondaryPreview) return;
        const z = safeTrim(this.els.secondaryIdInput.value);
        if (!z) { this.els.secondaryPreview.value = "—"; return; }
        const hit = findCustomerByIdNumber(z);
        if (!hit) { this.els.secondaryPreview.value = "לא נמצא לקוח עם הת״ז הזו"; return; }
        const nm = safeTrim(hit.fullName) || [safeTrim(hit.firstName), safeTrim(hit.lastName)].filter(Boolean).join(" ").trim();
        const ph = safeTrim(hit.phone);
        this.els.secondaryPreview.value = `${nm || "לקוח"}${ph ? " • " + ph : ""}`;
      };

      const addChildRow = (preset = null) => {
        if (!this.els.childrenList) return;
        const row = document.createElement("div");
        row.className = "childRow";
        row.innerHTML = `
          <input class="input" data-child="first" placeholder="שם פרטי" value="${preset && preset.firstName ? escapeHtml(preset.firstName) : ""}">
          <input class="input" data-child="last"  placeholder="שם משפחה" value="${preset && preset.lastName ? escapeHtml(preset.lastName) : ""}">
          <input class="input" data-child="id"    inputmode="numeric" placeholder="ת״ז" value="${preset && preset.idNumber ? escapeHtml(preset.idNumber) : ""}">
          <input class="input" data-child="birth" type="date" value="${preset && preset.birthDate ? escapeHtml(preset.birthDate) : ""}">
          <button class="btn childRemove" type="button">הסר</button>
        `;
        const rm = row.querySelector(".childRemove");
        if (rm) on(rm, "click", () => row.remove());
        this.els.childrenList.appendChild(row);
      };

      if (this.els.secondaryIdInput) {
        on(this.els.secondaryIdInput, "input", () => updateSecondaryPreview());
        on(this.els.secondaryIdInput, "change", () => updateSecondaryPreview());
      }

      if (this.els.btnAddChild) {
        on(this.els.btnAddChild, "click", () => addChildRow());
      }
...
this.renderPolicies();
    ...
    openPolicyModal() {
      ...
      // reset scoped extras (no side effects elsewhere)
      if (this.els.secondaryPreview) this.els.secondaryPreview.value = "—";
      if (this.els.childrenList) this.els.childrenList.innerHTML = "";
      if (this.els.secondaryIdInput) this.els.secondaryIdInput.value = "";
...
openPolicyActionModal(action, policyId, policyLabel) {
    ...
    renderPolicies() {
      ...
      this.els.cfPoliciesTbody.innerHTML = list.map(p => {
        ...
        const status = normPolicyStatus(p.status);
        const statusLabel = status==="cancelled" ? "בוטל" : (status==="swapped" ? "שוחלף" : (status==="pending_cancel" ? "ממתין לביטול" : ""));
        const statusHtml = (p.status && status!=="active") ? (`<div class="muted small">סטטוס: ${escapeHtml(statusLabel)}</div>`) : "";

        // Option A: details sub-row (secondary insured + children)
        ...
        let detailsRow = "";
        if (hasSecondary || hasChildren) {
          const parts = [];

          if (hasSecondary) {
            const sn = escapeHtml(p.secondary.name || "מבוטח משני");
            const sid = escapeHtml(p.secondary.idNumber || "");
            const sph = escapeHtml(p.secondary.phone || "");
            parts.push(`<span class="lc-pol-chip">👤 <strong>מבוטח משני:</strong> ${sn}${sid ? ` (${sid})` : ""}${sph ? ` • ${sph}` : ""}</span>`);
          }

          if (hasChildren) {
            const kids = p.children.slice(0, 3).map(ch => {
              const nm = [safeTrim(ch.firstName), safeTrim(ch.lastName)].filter(Boolean).join(" ").trim();
              return escapeHtml(nm || ch.idNumber || "ילד");
            });
            const more = p.children.length > 3 ? ` +${p.children.length - 3}` : "";
            parts.push(`<span class="lc-pol-chip">🧒 <strong>ילדים:</strong> ${kids.join(", ")}${more}</span>`);
          }

          detailsRow = `
            <tr class="lc-policy-subrow">
              <td colspan="5">
                <div class="lc-pol-meta">${parts.join("")}</div>
              </td>
            </tr>
          `;
        }
        ...
        return `
          <tr>
            <td>${escapeHtml(p.type || "")}${statusHtml}</td>
