...
/* ===== Internal Mail (Popup Window) ===== */
(function(){
  const MAIL_KEY = 'gemel_mail_messages_v1';
  const LAST_SEEN_KEY = 'gemel_mail_last_seen_unread_v1';
  ...
  function saveMsgs(arr){
    try{ localStorage.setItem(MAIL_KEY, JSON.stringify(arr||[])); }catch(_){}
  }
  function unreadCountFor(agentName){
    const me = String(agentName||'').trim();
    if(!me) return 0;
    const msgs = loadMsgs();
    return msgs.filter(m => m && m.to === me && m.read !== true).length;
  }
  ...
  function setBadge(n){
    ...
    el.style.display = nn > 0 ? 'inline-block' : 'none';
  }
  ...
  function beep(){
    try{
      ...
      o.start();
      setTimeout(()=>{ o.stop(); ctx.close().catch(()=>{}); }, 170);
    }catch(_){}
  }

  function pollBadgeAndSound(){
    try{
      const st = window.__gemelState;
      const me = st && st.agentName ? st.agentName : '';
      const count = unreadCountFor(me);
      setBadge(count);

      const last = Number(localStorage.getItem(LAST_SEEN_KEY) || '0') || 0;
      if(count > last){
        beep();
      }
  ...
  function openMailPopup(){
    const st = window.__gemelState;
    const me = st && st.agentName ? String(st.agentName||'').trim() : '';
    if(!me){
      try{ window.toast && window.toast('חסר', 'התחבר/י כדי להשתמש במייל'); }catch(_){}
      return;
    }
    try{ localStorage.setItem('gemel_mail_current_agent', me); }catch(_){}

    const w = window.open('', 'gemel_mail_popup', 'width=430,height=610,resizable=yes,scrollbars=yes');
    if(!w){
      try{ window.toast && window.toast('חסום', 'הדפדפן חסם חלון קופץ (Popups)'); }catch(_){}
      return;
    }
...
<title>מייל פנימי • GEMEL INVEST</title>
<style>
  :root{ --bg:#F6F2EA; --panel:#fff; --line:rgba(15,15,20,.12); --text:#0F1116; --muted:rgba(15,17,22,.65); --gold:#D6B25E; }
  *{ box-sizing:border-box; font-family: ui-sans-serif, system-ui, -apple-system, "Noto Sans Hebrew", "Heebo", Arial; }
  body{ margin:0; background: radial-gradient(900px 500px at 30% 0%, #FFFEF8 0%, var(--bg) 62%, #EFE9DD 100%); color:var(--text); }
  .wrap{ padding:14px; }
  .card{ background: rgba(255,255,255,.88); border:1px solid rgba(214,178,94,.18); border-radius:18px; box-shadow:0 18px 45px rgba(18,19,25,.10); overflow:hidden; }
  .head{ padding:12px 12px 10px; border-bottom:1px solid var(--line); display:flex; align-items:center; justify-content:space-between; gap:10px; background: linear-gradient(180deg, rgba(255,255,255,.80), rgba(246,242,234,.72)); }
  .title{ font-weight:900; }
  .me{ font-size:12px; color:var(--muted); font-weight:700; }
  ...
  select,input,textarea{ width:100%; border:1px solid rgba(15,17,22,.16); border-radius:12px; padding:10px 10px; background: rgba(255,255,255,.90); outline:none; }
  textarea{ min-height:90px; resize:vertical; }
  .btnRow{ display:flex; gap:10px; justify-content:flex-end; }
  button{ border:1px solid rgba(214,178,94,.22); background: rgba(255,255,255,.72); padding:10px 12px; border-radius:12px; font-weight:900; cursor:pointer; }
  button.primary{ background: linear-gradient(180deg, rgba(214,178,94,.22), rgba(214,178,94,.10)); }
  .list{ border-top:1px solid var(--line); padding:12px; max-height: 320px; overflow:auto; }
  .item{ border:1px solid rgba(15,17,22,.10); background: rgba(255,255,255,.78); border-radius:14px; padding:10px; margin-bottom:10px; }
  .meta{ display:flex; align-items:center; justify-content:space-between; gap:10px; font-size:12px; color:var(--muted); }
  .subj{ font-weight:900; margin-top:6px; }
  .body{ white-space:pre-wrap; margin-top:6px; font-size:13px; }
  .pill{ font-size:11px; font-weight:900; padding:2px 8px; border-radius:999px; background: rgba(214,178,94,.16); border:1px solid rgba(214,178,94,.22); }
...
<body>
  <div class="wrap">
    <div class="card">
      <div class="head">
        <div>
          <div class="title">מייל פנימי</div>
          <div class="me" id="meLine"></div>
        </div>
        <div class="muted" id="unreadLine"></div>
      </div>
      ...
      <div class="grid">
        ...
        <div class="row">
          <label>נושא</label>
          <input id="subj" type="text" placeholder="לדוגמה: עדכון לקוח / שיקוף"/>
        </div>
        <div class="row">
          <label>תוכן</label>
        ...
        <div class="btnRow">
          <button id="markAll">סמן הכל כנקרא</button>
          <button class="primary" id="sendBtn">שלח</button>
        </div>
      </div>
...
(function(){
  const MAIL_KEY = 'gemel_mail_messages_v1';
  function loadMsgs(){ try{ const raw = localStorage.getItem(MAIL_KEY); const arr = raw ? JSON.parse(raw) : []; return Array.isArray(arr)?arr:[]; }catch(_){ return []; } }
  function saveMsgs(arr){ try{ localStorage.setItem(MAIL_KEY, JSON.stringify(arr||[])); }catch(_){} }
  function uid(){ return 'm_' + Math.random().toString(36).slice(2,10) + '_' + Date.now().toString(36); }
  function nowIso(){ return new Date().toISOString(); }
  function esc(s){ return String(s||'').replace(/[&<>"]/g, c=>({ '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;' }[c])); }

  const me = String(localStorage.getItem('gemel_mail_current_agent') || '').trim();
  document.getElementById('meLine').textContent = me ? ('מחובר: ' + me) : 'לא מחובר';

  function loadUsers(){
    try{
      const raw = localStorage.getItem('gemel_users_v1');
      const arr = raw ? JSON.parse(raw) : [];
      if(Array.isArray(arr) && arr.length){
        return arr.filter(u => u && u.active !== false).map(u => String(u.displayName||u.username||'').trim()).filter(Boolean);
      }
    }catch(_){}
    return [];
  }
  ...
  function renderTo(){
    const list = loadUsers().filter(n => n && n !== me);
    const sel = document.getElementById('toSel');
    sel.innerHTML = '<option value="">בחר נציג...</option>' + list.map(n => '<option value="'+esc(n)+'">'+esc(n)+'</option>').join('');
  }

  function renderList(){
    const box = document.getElementById('list');
    const msgs = loadMsgs().filter(m => m && (m.to===me || m.from===me)).sort((a,b)=> new Date(b.createdAt||0)-new Date(a.createdAt||0));
    const unread = msgs.filter(m => m.to===me && m.read !== true).length;
    document.getElementById('unreadLine').textContent = unread ? ('לא נקראו: ' + unread) : 'אין הודעות חדשות';

    if(!msgs.length){
      box.innerHTML = '<div class="muted">אין הודעות עדיין.</div>';
      return;
    }

    box.innerHTML = msgs.map(m => {
      const incoming = (m.to===me);
      const pill = incoming ? (m.read?'<span class="pill">נקרא</span>':'<span class="pill">חדש</span>') : '<span class="pill">נשלח</span>';
      return '<div class="item">' +
        '<div class="meta"><div>' + (incoming ? ('מאת: <b>'+esc(m.from)+'</b>') : ('אל: <b>'+esc(m.to)+'</b>')) + '</div><div>'+pill+'</div></div>' +
        '<div class="subj">' + esc(m.subject||'(ללא נושא)') + '</div>' +
        '<div class="body">' + esc(m.body||'') + '</div>' +
        '<div class="meta" style="margin-top:8px"><div>'+esc(new Date(m.createdAt||'').toLocaleString('he-IL'))+'</div>' +
        (incoming && !m.read ? '<button data-read="'+esc(m.id)+'">סמן נקרא</button>' : '') +
        '</div>' +
      '</div>';
    }).join('');

    box.querySelectorAll('button[data-read]').forEach(btn => {
      btn.addEventListener('click', () => {
        const id = btn.getAttribute('data-read');
        const all = loadMsgs();
        const x = all.find(mm => mm && String(mm.id)===String(id));
        if(x){ x.read = true; saveMsgs(all); renderList(); }
      });
    });
  }

  document.getElementById('sendBtn').addEventListener('click', () => {
    const to = String(document.getElementById('toSel').value||'').trim();
    const subject = String(document.getElementById('subj').value||'').trim();
    const body = String(document.getElementById('body').value||'').trim();
    if(!to){ alert('בחר נציג'); return; }
    if(!body){ alert('כתוב הודעה'); return; }

    const all = loadMsgs();
    all.unshift({ id: uid(), from: me, to, subject: subject || '(ללא נושא)', body, createdAt: nowIso(), read: false });
    saveMsgs(all);
  ...
  document.getElementById('markAll').addEventListener('click', () => {
    ...
    let changed = false;
    all.forEach(m => {
      if(m && m.to===me && m.read !== true){ m.read=true; changed=true; }
    });
  ...
  function wireMailDock(){
    const btn = document.getElementById('mailDockBtn');
    if(btn && !btn.__wired){
      btn.__wired = true;
      btn.addEventListener('click', openMailPopup);
    }
  }

  window.gemelMail = { openMailPopup, pollBadgeAndSound, wireMailDock };

  document.addEventListener('DOMContentLoaded', () => {
    wireMailDock();
    pollBadgeAndSound();
    setInterval(pollBadgeAndSound, 2000);
  });
})();
