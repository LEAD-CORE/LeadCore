...
(() => {
  ...
  const UI = {
    ...
    init() {
      ...
      this.els.cfDetPhone = $("#cfDetPhone");
      this.els.cfDetEmail = $("#cfDetEmail");
      this.els.cfDetAddress = $("#cfDetAddress");
      this.els.cfDetId = $("#cfDetId");
      this.els.cfDetName = $("#cfDetName");
    ...
    openCustomerFull(customerId) {
      ...
      if (this.els.cfDetName) this.els.cfDetName.textContent = `${c.firstName} ${c.lastName}`.trim() || "—";
      if (this.els.cfDetEmail) this.els.cfDetEmail.textContent = c.email || "—";
      if (this.els.cfDetAddress) this.els.cfDetAddress.textContent = c.address || "—";
      if (this.els.cfDetPhone) this.els.cfDetPhone.textContent = c.phone || "—";
      if (this.els.cfDetId) this.els.cfDetId.textContent = c.idNumber || "—";
    ...
    renderPolicies() {
      ...
      const active = list.filter(p => (safeTrim(p.status) || "פעיל") === "פעיל");
      ...
      const rows = list.map(p => {
        ...
        const st = safeTrim(p.status) || "פעיל";
        const isActive = (st === "פעיל");
        const stBadge = isActive ? "" : ` <span class="tag">${escapeHtml(st)}</span>`;
        return `
          <tr>
            <td>${escapeHtml(p.type || "")}${stBadge}</td>
            ...
            <td style="text-align:left; white-space:nowrap">
              ...
              <button class="btn" data-switchpol="${escapeHtml(p.id)}">שחלוף פוליסה</button>
            </td>
          </tr>
        `;
      }).join("");

      this.els.cfPoliciesTbody.innerHTML = rows || `
      ...
      $$("button[data-cancelpol]", this.els.cfPoliciesTbody).forEach(btn => {
        on(btn, "click", async () => {
          ...
          const p = (c.policies || []).find(x => x.id === pid);
          if (!p) return;
          const cur = safeTrim(p.status) || "פעיל";
          p.status = (cur === "בוטל") ? "פעיל" : "בוטל";
          // אם בוטל, לא יכול להיות גם שחלוף
          if (p.status === "בוטל") p.switchedAt = null;
          c.updatedAt = nowISO();
          this.renderPolicies();
          const r = await App.save("עודכן סטטוס פוליסה");
          if (!r.ok) notify("שמירה נכשלה: " + (r.error || "שגיאה"), "error");
          this.renderAll();
        });
      });

      $$("button[data-switchpol]", this.els.cfPoliciesTbody).forEach(btn => {
        on(btn, "click", async () => {
          const pid = btn.dataset.switchpol;
          const p = (c.policies || []).find(x => x.id === pid);
          if (!p) return;
          const cur = safeTrim(p.status) || "פעיל";
          p.status = (cur === "שחלוף") ? "פעיל" : "שחלוף";
          // אם שחלוף, לא יכול להיות גם בוטל
          if (p.status === "שחלוף") p.cancelledAt = null;
          c.updatedAt = nowISO();
          this.renderPolicies();
          const r = await App.save("עודכן סטטוס פוליסה");
          if (!r.ok) notify("שמירה נכשלה: " + (r.error || "שגיאה"), "error");
