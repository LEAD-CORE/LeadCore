...
(() => {
  ...
  const State = {
    ...
    // Local changes (user edits) -> bump updatedAt now
    set(next) {
    ...
    // Server changes (loaded from Sheets) -> keep server updatedAt as-is
    replaceFromServer(next) {
      this.data = normalizeState(next);
      this.data.meta ||= {};
      // do NOT overwrite meta.updatedAt here
    }
  };
...
on(this.els.btnSearch, "click", () => {
  ...
  const LiveSync = {
    ...
    inFlight: false,
    serverAt: null,
    intervalMs: 5000, // every 5s

    setServerAt(at) {
      this.serverAt = at || null;
    },
    ...
    start() {
      ...
      // When user returns to tab, refresh immediately
    ...
    async tick() {
      ...
      try {
        ...
        // First time: set marker only
        if (!this.serverAt && at) {
          this.serverAt = at;
          return;
        }

        if (at && this.serverAt && at === this.serverAt) return;

        // Apply remote update
        this.serverAt = at || this.serverAt;

        State.replaceFromServer(r.payload);
        Storage.saveBackup(State.data);

        // Refresh UI
        UI.renderAll();
        if (UI.els.customerFull && UI.els.customerFull.classList.contains("is-open")) {
          UI.renderPolicies();
        }
