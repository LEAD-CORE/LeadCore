...
// ---------------------------
  ...
  const Auth = {
    ...
    show(){
      ...
      if(pin){ setTimeout(()=>{ try{ pin.focus(); }catch(_){} }, 40); }
      UI.updateTopUser();
      UI.applyRoleUi();
    },
    ...
    loginByPin(pin, remember){
      ...
      const users = this.getUsers().filter(u => u && u.active && safeTrim(u.pin));
      const user = users.find(u => safeTrim(u.pin) === p);
      if(!user) return { ok:false, error:"קוד שגוי או משתמש לא פעיל." };
    ...
    init(){
      ...
      on(UI.els.authForm, "submit", async (e) => {
        ...
        const r = this.loginByPin(pin, remember);
        if(!r.ok){
