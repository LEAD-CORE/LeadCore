...
;
  ...
  try {
    ...
    if (!res.ok) {
      return { ok: false, error: `שגיאת רשת GET (${res.status})` };
    }

    let json;
    try { json = JSON.parse(raw); } catch (_) {
      return { ok: false, error: "תגובה לא תקינה מהשרת (לא JSON)" };
    }

    if (!json || json.ok !== true) return { ok: false, error: (json && (json.error || json.msg)) || "שגיאת get" };
...
async saveSheets(state) {
  ...
  try {
    ...
    if (!res.ok) {
      return { ok: false, error: `שגיאת רשת PUT (${res.status})` };
    }
    ...
    if (!json || json.ok !== true) return { ok: false, error: (json && (json.error || json.msg)) || "שגיאת put" };
...
if (!r.ok) {
  // Rollback: remove the optimistic customer so it won't "disappear" only after refresh
  State.data.customers = State.data.customers.filter(x => x.id !== customer.id);
  notify("שמירה נכשלה: " + (r.error || "שגיאה"), "error");
  return;
}

// Success: close modal and open customer file immediately
this.closeModal();
