    ...
    async saveSheets(state) {
      ...
      const attempts = [
        // (1) action=put + {payload: state, state: state}
        {
          ...
          url: (() => { const u=new URL(baseUrl); u.searchParams.set("action","put"); return u; })(),
          headers: { "Content-Type": "application/json;charset=utf-8" },
          body: JSON.stringify({ payload: state, state })
        },
        ...
        // (3) action=save (לפעמים כך נקרא בסקריפטים אחרים)
        {
          ...
          url: (() => { const u=new URL(baseUrl); u.searchParams.set("action","save"); return u; })(),
          headers: { "Content-Type": "application/json;charset=utf-8" },
          body: JSON.stringify({ payload: state, state })
        }
      ];
...
async save(activityText) {
    ...
    async setMode(mode) {
      if (FORCE_GSHEETS && mode !== "sheets") {
        notify("מצב Local מבוטל. המערכת נעולה על Google Sheets.");
        return;
      }
      Storage.mode = mode;
      if(!FORCE_GSHEETS) localStorage.setItem("LEAD_CORE_MODE", mode);
