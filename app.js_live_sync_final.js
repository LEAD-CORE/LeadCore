...
on(this.els.btnSearch, "click", () => {
  ...
  const App = {
    async boot() {
      ...
      // Live Sync: עדכון אוטומטי בין מחשבים (בלי רענון)
      this.startLiveSync();
    },
...
,
    // ---------------------------
    // Live Sync (Polling) - עדכון אוטומטי בלי רענון
    // ---------------------------
    _liveTimer: null,
    _liveLastAt: null,
    _liveBusy: false,
    ...
    startLiveSync() {
      // עובד רק במצב Google Sheets
      if (this._liveTimer) clearInterval(this._liveTimer);

      // קובע נקודת התחלה: אם כבר נטענו מהשרת – נשתמש ב-updatedAt או בזמן נוכחי
      if (!this._liveLastAt) {
        this._liveLastAt = State.data?.meta?.updatedAt || null;
      }
      ...
      const tick = async () => {
        try {
          ...
          // משתמשים ב-at מהשרת (עדיף), ואם אין – נופלים ל-updatedAt מתוך ה-payload
          const serverAt = r.at || r.payload?.meta?.updatedAt || null;

          // אם זו הפעם הראשונה – רק נשמור חותמת זמן
          if (!this._liveLastAt) {
            this._liveLastAt = serverAt;
            return;
          }

          // אין שינוי
          if (serverAt && this._liveLastAt === serverAt) return;

          // שינוי נמצא -> עדכון state + UI
          this._liveLastAt = serverAt;

          const openId = UI.els.drawerCustomer?.dataset?.customerId || "";
          State.set(r.payload);

          // רינדור עדין: נשמור על המסך הנוכחי
          UI.renderAll();

          // אם תיק לקוח פתוח, נוודא שעדיין מציג את אותו לקוח
          if (openId) {
            UI.openDrawer(openId);
          }

          UI.renderSyncStatus();
        } catch (e) {
          // שקט – בלי פופ-אפים
          // console.log("live sync error", e);
        }
      };

      // תדירות: 4 שניות (מאוזן, לא מכביד)
      this._liveTimer = setInterval(tick, 4000);

      // וגם כשחוזרים לטאב – נבדוק מיד
