...
if (!out.agents.length) {
  ...
  const UI = {
    ...
    init() {
      ...
      this.els.cfSlaAction = $("#cfSlaAction");
      this.els.cfSlaDueAt = $("#cfSlaDueAt");
...
this.els.btnUserEditSave = $("#btnUserEditSave");
      ...
      // Customer File • SLA (action + dueAt)
      on(this.els.cfSlaSaveBtn, "click", async () => {
        const c = getOpenCustomer();
        if (!c) return notify("לא נבחר לקוח.", "warn");

        const action = safeTrim(this.els.cfSlaAction?.value);
        const dueRaw = safeTrim(this.els.cfSlaDueAt?.value); // datetime-local: YYYY-MM-DDTHH:MM

        if (!action) return notify("בחר פעולה ל-SLA.", "warn");
        if (!dueRaw) return notify("בחר תאריך ושעה יעד.", "warn");

        c.nextAction = action;
        c.dueAt = dueRaw;

        const r = await this.save(`עודכן SLA ללקוח: ${action}`);
        if (!r || !r.ok) return;
...
// Search (live filter on customers view)
    ...
    openCustomerFull(customerId) {
      ...
      // Customer File • SLA fields
      if (this.els.cfSlaAction) {
        const v = safeTrim(c.nextAction || "");
        this.els.cfSlaAction.value = v;
        if (this.els.cfSlaAction.value !== v) this.els.cfSlaAction.value = "";
      }
      if (this.els.cfSlaDueAt) {
        const d = this._parseDateSafe(c.dueAt);
        if (d) {
          const pad = (n) => String(n).padStart(2, "0");
          const v = `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
          this.els.cfSlaDueAt.value = v;
        } else {
          this.els.cfSlaDueAt.value = "";
        }
      }
