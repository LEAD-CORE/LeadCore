...
on(this.els.cfPdfBtn, "click", async () => {
  const c = getOpenCustomer();
  if (!c) return notify("לא נבחר לקוח.", "warn");
  try{
    await this.exportCustomerPdf(c);
  } catch (e){
    console.error(e);
    notify("לא הצלחתי להפיק PDF. נסה רענון (Hard Refresh) ונסה שוב.", "warn");
  }
});
...
this.renderPolicies();
    ...
    // ===========================
// PDF Export (Customer File)
// ===========================
async exportCustomerPdf(c){
  if (!c) return;
  // Ensure libs loaded
  const hasH2C = typeof window.html2canvas === "function";
  const hasJsPDF = !!(window.jspdf && window.jspdf.jsPDF);
  if (!hasH2C || !hasJsPDF){
    notify("חסר רכיב PDF בדפדפן. רענן את העמוד ונסה שוב.", "warn");
    return;
  }

  const panel = this.els.customerFull?.querySelector?.(".customerFull__panel");
  if (!panel) return notify("תיק הלקוח לא פתוח.", "warn");

  const makeClone = () => {
    const clone = panel.cloneNode(true);

    // Remove buttons/actions that are not needed in PDF
    clone.querySelectorAll("button, .iconBtn").forEach(btn => {
      // keep only the close icon? remove all to keep it clean
    ...
    const body = clone.querySelector(".customerFull__body");
    if (body){
    ...
    // prevent sticky headers messing with capture
    clone.querySelectorAll("[style*='position: sticky'], .is-sticky, .sticky").forEach(el => {
      try { el.style.position = "static"; } catch(_){}
    });
  ...
  // tiny delay so fonts/layout settle
  ...
  const pxPerPt = canvas.width / targetW; // how many canvas pixels fit in one PDF point

  const sliceHpx = Math.floor((pageH - margin * 2) * pxPerPt);
  ...
  while (y < totalHpx){
    const h = Math.min(sliceHpx, totalHpx - y);
    tmp.height = h;
    ...
    const sliceData = tmp.toDataURL(imgType, imgQuality);
    const sliceHpt = h / pxPerPt; // convert slice height to PDF points

    if (pageIndex > 0) pdf.addPage();
    pdf.addImage(sliceData, "JPEG", margin, margin, targetW, sliceHpt);

    y += h;
    pageIndex++;
  }

  const safe = (s) => String(s || "").replace(/[^\p{L}\p{N}_-]+/gu, "_").replace(/_+/g, "_").replace(/^_|_$/g, "");
  const name = `${safe((c.firstName||"").trim())}_${safe((c.lastName||"").trim())}`.replace(/^_+|_+$/g,"") || "לקוח";
  const idPart = safe(c.idNumber || c.id || "");
  const fileName = `תיק_לקוח_${name}${idPart ? "_" + idPart : ""}.pdf`;
