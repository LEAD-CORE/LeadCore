...
const submitBtn = this.els.customerForm?.querySelector?.("button[type='submit']");
        ...
        if (!r.ok) {
          // rollback so the UI doesn't show a customer that wasn't really saved to Sheets
          try { State.data.customers = (State.data.customers || []).filter(x => x.id !== customer.id); } catch(_){}
          UI.renderSyncStatus("שמירה נכשלה: " + (r.error || "שגיאה"), "err", 4500);
          return;
        }

        // ✅ Success: close modal and open the customer file immediately
        this.closeModal();
